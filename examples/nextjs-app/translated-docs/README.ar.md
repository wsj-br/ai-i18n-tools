...
يفرض إدخال مساحة العمل [`overrides`](../../../pnpm-workspace.yaml) (`ai-i18n-tools: workspace:*`) `ai-i18n-tools` على نسخة مساحة العمل المحلية على الرغم من أن هذا المثال يعلن عن نطاق علامة إقحام منشورة. لا توجد حاجة إلى خطوة بناء أو ربط منفصلة — بعد تغيير مصادر المكتبة، قم بتشغيل `pnpm run build` في جذر المستودع وسيلتقط المثال `dist/` المحدث تلقائيًا.
