...
**Audience:** Contributors and AI agents working inside the `ai-i18n-tools` **repository**. Start with [`AGENT.md`](../AGENT.md) for rules and workflows; this file lives in `dev/` and is **not** the primary integration guide for downstream apps.

**Consumers:** If you are adding the **published npm package** to another project, use [`docs/ai-i18n-tools-context.md`](../docs/ai-i18n-tools-context.md) (shipped on npm under `docs/`), the [documentation site](https://wsj-br.github.io/ai-i18n-tools/), and [Images and screenshots](../docs/guide/images-and-screenshots/) for locale-specific raster assets in translated docs.
...
- `docs[].protectAttributes` / `protectKeys` extend skip lists for `.astro` and MDX (see [Configuration](../docs/reference/configuration.md)).
...
├── api/llm-client.ts      Provider-agnostic LLM client (OpenRouter/OpenAI/Ollama) with model fallback and rate-limit handling
