...
## Translation execution policy for agents

- For changes to user-facing CLI, log, or dashboard strings, run `pnpm i18n:self`. It automatically extracts and translates the tool UI strings.
- Do not run documentation translation (`pnpm i18n:sync`, `sync`, or `translate-docs`) as part of normal agent work. Run it only when the user explicitly requests a documentation-translation refresh.
...
| Path | Role |
|------|------|
| `README.md` | Concise GitHub/npm landing (value prop, install, quick start, links into docs) |
| `docs/index.md` | VitePress docs homepage — independent from README; links into `/guide/` and `/reference/` |
...
When changing English documentation, edit the English source under `docs/` (or `README.md` for the GitHub landing) only. Agents must not run `pnpm i18n:sync`, `sync`, or `translate-docs`, and must not modify generated locale trees or theme JSON unless the user explicitly asks for a documentation-translation refresh. `README.md` and `docs/index.md` are maintained independently; update each according to its audience when shared facts change.
...
- Locale trees under `docs/<locale>/` are generated output. Do not hand-fix or regenerate locale copies for brace escaping.
...
- Never hand-edit or regenerate markdown links under `docs/<locale>/` after English changes.
...
pnpm i18n:sync        # translate docs (including docs/index.md) + theme JSON; run only when explicitly requested
...
If you need to copy, move or remove file (cp, mv, rm) in the agent shell, use the full path (/bin/cp, /bin/mv, /bin/rm) to skip the alias expansion or use the full path to the command.
...
dev/                  Maintainer internals: CHANGELOG.md, DEVEL.md; ad-hoc tools under `dev/scripts/`
...
- When changing English docs or examples, do not run `sync`, `pnpm i18n:sync`, or `translate-docs`, and do not hand-write locale copies. Documentation translation is a user-directed, explicit maintenance task.
...
- `translated-docs/` and `docs/<locale>/` — pipeline output; never edit or regenerate them unless the user explicitly requests documentation translation.
