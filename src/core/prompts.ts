...
export const PROMPTS: PromptStrings = {
  document: {
    coreRules:
      "Rules: Keep headers (###), inline `code`, variables, URLs, line breaks, markdown formatting, placeholders {{X}} unchanged. Preserve exactly (do not translate or alter): {{ADM_OPEN_N}}, {{ADM_END_N}}, {{URL_PLACEHOLDER_N}}, {{HTML_ANCHOR_N}}, {{DOC_HEADING_ID_N}}, {{GLOSSARY_FORCE_N}}, {{IT}}, {{IU}}, {{SE}}, {{SU}}, {{ST}} (for GLOSSARY_FORCE, N is any non-negative integer; copy the full token character-for-character). Copy each placeholder character-for-character, including underscores in DOC_HEADING_ID (never replace them with hyphens). Translate only title/description in front matter. If a <glossary> block appears below, you must preserve each suggested target wording when the source matches or contains that term (use that wording exactly; do not paraphrase or substitute synonyms). Maintain coherence for all other phrasing.",

    markdownPreservation:
      "Markdown structure: Preserve heading levels (#\u2013######), list markers and indentation, blockquotes (>), horizontal rules, and meaningful line breaks. Keep **bold**, *italic*, and `inline code` spans intact with balanced delimiters (do not drop closing **, *, or backticks). Every **bold** span in the source must have a corresponding **bold** span in the translation \u2014 even when the bolded word translates to a short conjunction, particle, suffix, or single word in the target language; never remove bold to simplify the sentence. For **`code`** compound patterns (bold wrapping inline code), preserve both the outer ** delimiters and the inner backticks. In [visible text](url), ![alt](path), and HTML like <img \u2026> / <a \u2026>, translate only the visible link text or alt; keep URLs, paths, angle-bracket links, and attribute names unchanged. Preserve GFM pipe tables (| cells |).",
...
Do not change keys, add keys, remove keys, or return XML tags/markdown/code fences/extra text.`,
  ...
  ui: {
    systemPrompt: [
      ...
      "- Keep placeholders unchanged: {{variable}}, {0}, %s, %d, :value, and {{GLOSSARY_FORCE_N}} (any non-negative integer N; copy exactly)",
