...
Input:
### Section title
Body line with \`CODE\` and {{HTM_0}}.

Output:
### [Translated section title]
[Translated body line with \`CODE\` and {{HTM_0}}.]`,
...
Do not change keys, add keys, remove keys, or return XML tags/markdown/code fences/extra text.`,
  ...
  proofreadUI: {
    systemPrompt: [
      ...
      "",
      "TASK:",
      "- Review each string on its own for spelling, grammar, and mistakes that change the meaning.",
      '- The locale under review decides spelling. For en-GB, British spelling is correct: flag American spellings (for example "prioritize"), and do not suggest American spellings for strings that are already British.',
      '- Do NOT nitpick style when the string is already clear: capitalization, "ID" vs "id", hyphenation, ellipsis "..." vs "…", a missing final period on a fragment, or how many spaces follow a symbol or emoji.',
      "- A string that contains {{count}} or another number placeholder may be the singular or the plural form on purpose, with the other form stored as a different string. Do not change singular to plural or plural to singular.",
      '- suggestedText must be a correction of that same string, not a different message, a neighboring string, or a rewrite that drops information. If you cannot propose that full-string correction, return "issues": [].',
      "- CLI examples, log prefixes, and help lines are intentional fragments. Do not merge them with another sentence or replace them with different copy.",
      "- When a <glossary> block is present: flag terminology **only** if the string uses a **wrong or inconsistent variant** of a term (wrong synonym, misspelling, or forbidden alternate branding). **Never** emit a warning whose sole purpose is “consistency” or “preferred spelling” when the string **already** uses the correct product name, package name, or glossary wording as intended — including proper repetition of that name in a sentence. If the glossary lists more than one spelling, a string that matches any of them is acceptable.",
      ...
      "",
      "PLACEHOLDERS (must stay byte-for-byte identical in suggestedText when you propose a fix, including repeated tokens):",
      "- {{variable}}, {{count}}, {{GLS_N}} (N is any non-negative integer)",
      "- {0}, %s, %d, :value-style tokens when present",
      "- Keep the same number of each placeholder. Do not drop, duplicate, or rename one.",
      ...
      "- Length must equal the number of input strings N.",
      '- Each array element MUST be one JSON object with keys "index" and "issues". "index" is the integer input position (0 .. N-1). Include every index exactly once, even when issues is empty. A short array is discarded, so do not omit empty slots.',
      '- Each "issues" value is an array of objects with keys: "severity" ("error" | "warning"), "message" (string), and "suggestedText" (the full corrected string). Omit the issue when you cannot give suggestedText.',
