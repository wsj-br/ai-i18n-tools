...
- When prose names or describes user-visible interface elements (menus, dialogs, buttons, notifications), phrase them with the same conventional labels users see in mainstream applications in that locale so documentation matches the product.`,
    ...
    coreRules:
      "Rules: Keep headers (###), variables, URLs, line breaks, markdown formatting, placeholders {{X}} unchanged. Preserve exactly (do not translate or alter): {{ADM_OPEN_N}}, {{ADM_END_N}}, {{ADM_TCLOSE_N}}, {{URL_N}}, {{BLD_N}}, {{ILC_N}}, {{HTM_N}}, {{ANC_N}}, {{HDG_N}}, {{MDX_N}}, {{JXA_N}}, {{GLS_N}}, {{IT}}, {{IU}}, {{SE}}, {{SU}}, {{ST}} (for GLS and similar tokens, N is any non-negative integer; copy the full token character-for-character). Copy each placeholder character-for-character; do not change underscores, hyphens, or digits inside {{...}} tokens. Use every {{…}} token from the input exactly once; do not reuse, drop, or renumber tokens. Keep numbered tokens ({{URL_N}}, {{HTM_N}}, {{ILC_N}}, {{BLD_N}}, {{GLS_N}}, and similar) in the same relative order. Emphasis markers {{SE}}, {{SU}}, {{IT}}, {{IU}}, {{ST}} may move with natural word order, but each type’s count must still match. Do not invent new {{…}} tokens; glossary target words are plain text, not wrapped in {{…}}. Keep numbered-token order even when the target language is written right-to-left (placeholders are markup, not prose). Some segments end with appendix lines `||JXA_N: …||` (one or more): preserve each marker’s index N and closing `||`; translate only the human-readable text between the colon and the closing `||`. YAML front matter is not translated (omitted from segments). Admonition lines only mask the directive prefix (for example `:::note `); translate any title text that appears after {{ADM_OPEN_N}} on the same line. If a <glossary> block appears below, you must preserve each suggested target wording when the source matches or contains that term (use that wording exactly; do not paraphrase or substitute synonyms). Maintain coherence for all other phrasing.",
