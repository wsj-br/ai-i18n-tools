...
O ai-i18n-tools inclui seu próprio extrator de strings. Se você usou anteriormente `i18next-scanner`, `babel-plugin-i18next-extract` ou similar, pode remover essas dependências de desenvolvimento após a migração. Os arquivos **JSON de namespace** i18next existentes podem permanecer no [pipeline JSON](/pt-BR/guide/json#i18next-namespace-files). Os dicionários Intlayer `.content.ts` podem ser importados com `migrate-intlayer` — consulte [Migrando do Intlayer](/pt-BR/guide/migrating-from-intlayer).
...
**PATH manual** — execute-os a partir da **raiz do projeto** (o diretório que contém `node_modules/.bin`). Os subdiretórios continuam funcionando depois, desde que você mantenha essa entrada `PATH` no lugar; reexecutar a exportação de uma pasta aninhada falha porque `$PWD` não aponta mais para a raiz do projeto.
...
Com pnpm, execute `pnpm setup` uma vez por máquina (e abra um novo shell) se os comandos globais estiverem faltando — pnpm requer seu diretório bin global em `PATH`. Uma instalação global usa a versão globalmente fixada. Para fixação de versão por projeto, prefira direnv ou PATH manual para que `node_modules/.bin` seja resolvido para a dependência do projeto.

**`package.json` scripts** — quando o npm ou pnpm executa um script, ele adiciona `node_modules/.bin` ao `PATH`, para que o nome do comando simples funcione dentro dos scripts sem alterações no PATH do shell. Prefira `sync` em vez de encadear manualmente as etapas de tradução — a ordem e os sinalizadores de recursos são fáceis de errar quando executados manualmente:
...
Em seguida, execute, por exemplo, `pnpm run i18n:sync`. Consulte [Scripts `package.json` recomendados](/pt-BR/guide/quick-start#recommended-packagejson-scripts) para o conjunto completo recomendado.
...
- **Raiz do repositório** — o pnpm não vincula o próprio `bin` do pacote raiz a `node_modules/.bin`. O `.envrc` deste checkout adiciona `bin/` a `PATH`, de modo que o wrapper `bin/ai-i18n-tools` confirmado (ao lado do shim `bin/ai-i18n-tools.mjs` publicado) fornece o comando direto após `direnv allow`. Sem o direnv, use `node bin/ai-i18n-tools.mjs …`, `./bin/ai-i18n-tools …` ou scripts `pnpm i18n:*` raiz (ou um alias de shell / `pnpm add -g .` — consulte [Guia de Desenvolvimento](https://github.com/wsj-br/ai-i18n-tools/blob/main/dev/DEVEL.md#running-the-cli-during-development)).
