...
Crie `localeLoaders` com `makeLocaleLoadersFromManifest(uiLanguages, sourceLocale, …)` para que as chaves permaneçam alinhadas com `targetLocales` após `generate-ui-languages`. Consulte `docs/guide/ui-strings/i18next-runtime.md` (conexão em tempo de execução), `examples/nextjs-app/`, `examples/console-app/` e `examples/astro-website/` (`makeT` personalizado sem i18next).
