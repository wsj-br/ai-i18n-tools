...
Após a restauração, `translate-docs` também rejeita segmentos onde os placeholders de tags HTML foram reutilizados ou descartados (de modo que as tags restauradas não correspondem mais ao mapa de origem) ou onde o modelo inventou tokens de chaves duplas restantes que não estavam na origem (por exemplo, um token estilo glossário inventado). As verificações pré-restauração exigem o mesmo multiconjunto de tokens <code v-pre>{{…}}</code> e a mesma subsequência ordenada de tokens numerados (<code v-pre>{{HTM_N}}</code>, <code v-pre>{{URL_N}}</code>, …); marcadores de ênfase como <code v-pre>{{SE}}</code> podem se mover com a ordem natural das palavras quando suas contagens por tipo ainda correspondem. Essas falhas usam o mesmo caminho de fallback do modelo que os tokens internos oficiais restantes.
