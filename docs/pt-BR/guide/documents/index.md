...
Para **consistência terminológica** entre UI e documentos, defina `glossary.uiGlossary` para o seu caminho `strings.json` — `translate-docs` reutiliza traduções de UI existentes como dicas em prompts de LLM quando termos correspondentes aparecem em um segmento. `glossary.userGlossary` opcional adiciona substituições CSV para termos de produto (compartilhados com `translate-ui` e `proofread-ui`). Gere um CSV inicial com `glossary-generate`, edite linhas na guia **Glossário** do Painel de Tradução, ou veja [Configuração — `glossary`](/pt-BR/reference/configuration#glossary) e [Glossário](/pt-BR/guide/translation-dashboard/glossary).
...
Após a restauração, `translate-docs` também rejeita segmentos onde os espaços reservados de tags HTML foram reutilizados ou descartados (de modo que as tags restauradas não correspondem mais ao mapa de origem) ou onde o modelo inventou tokens de chaves duplas restantes que não estavam na origem (por exemplo, um token de estilo de glossário inventado). Essas falhas usam o mesmo caminho de fallback do modelo que os tokens internos oficiais restantes.
