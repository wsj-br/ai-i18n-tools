...
Após o retorno do modelo, `translate-docs` restaura os mapas e valida o segmento: tokens de chaves duplas sem ênfase devem manter a mesma subsequência ordenada da fonte protegida (marcadores de ênfase como <code v-pre>{{SE}}</code> podem se mover com a ordem das palavras quando as contagens por tipo correspondem), os tipos de tags HTML restauradas devem corresponder à fonte desprotegida, e qualquer identificador de chaves duplas restante já deve ter existido na fonte (portanto, tokens inventados falham). O prompt do documento também pede aos modelos para copiar cada token uma vez, manter a ordem dos tokens numerados e não inventar novos wrappers de chaves duplas; as verificações mecânicas permanecem autoritárias.
