...
| | `src/glossary/` | Carregamento de glossário, dicas de termos e arquivos opcionais de contexto de projeto para prompts |
| | `src/utils/` | Logger, hashing, analisador de ignorados, tabelas de largura de exibição, carregador `.env` |
| **Tempo de execução do seu aplicativo** | `src/runtime/` | Ajudantes i18next e utilitários de exibição — exportados como `'ai-i18n-tools/runtime'` ([Ajudantes de tempo de execução](/pt-BR/guide/runtime-helpers)) |
...
Tudo o que se destina ao uso programático é reexportado de `src/index.ts` ([API Programática](/pt-BR/reference/programmatic-api)).
...
- Identifique os idiomas de origem e de destino (pelo nome de exibição de `localeDisplayNames` ou `ui-languages.json`).
- Preponha uma diretiva de script quando o local de destino tiver um sistema de escrita esperado (script BCP-47 explícito ou um padrão de idioma como `hi` → Devanagari, `ar` → Árabe, `ja` → Kana/Kanji japonês).
- Envie um array JSON de strings e solicite um array JSON de traduções em troca.
- Inclua dicas de glossário quando disponíveis.

`LlmClient.translateUIBatch` tenta cada modelo em ordem, recorrendo a erros de análise, rede ou script incorreto (incluindo fallback romanizado/latino para locais de script nativo). A CLI constrói essa lista por local de destino a partir de `localeModels`, `uiModels` opcional e `translationModels` (consulte [Provedores e modelos](/pt-BR/guide/providers-and-models#model-fallback-chain)).
...
É necessário um arquivo de configuração válido com **pelo menos um bloco `docs[]`**. Para cada bloco, ele coleta arquivos `.md` / `.mdx` em `contentPaths`, aplica as regras `.translate-ignore` do projeto (mesma ideia da tradução de documentos) e, opcionalmente, restringe a uma subárvore com `--path` / `--file`. Cada arquivo é transformado com `applyHeadingAnchorsToMarkdown`: para cada **cabeçalho ATX simples** (`# …` a `###### …`) fora dos blocos de código cercados, os IDs de cabeçalho existentes de qualquer forma são substituídos pelo estilo selecionado. Os estilos HTML escrevem `<a id="slug"></a>` na linha acima de um cabeçalho sem sufixo; `--slug-style mdx-comment` escreve `{/* #slug */}` na linha do cabeçalho (e remove uma âncora HTML anterior). Os slugs sempre vêm do texto do cabeçalho atual. `--remove` remove âncoras HTML, sufixos clássicos `{#id}` e IDs de comentários MDX sem escrever novos. Os algoritmos de slug correspondem a ecossistemas comuns — `github` (padrão), `bitbucket`, `gitlab`, `pymdown` (sinalizadores opcionais de normalização Unicode / codificação percentual), `azure-devops`, mais `mdx-comment` (slug do github + saída de comentário MDX) — para que os IDs de âncora permaneçam consistentes com as ferramentas existentes (doctoc, PyMdown, Docusaurus, etc.). `--dry-run` relata edições potenciais sem escrever.

Após a passagem da fonte, o mesmo comando reposiciona esses IDs em inglês no markdown traduzido existente de cada localidade (`resolveTranslatedOutputPath`). Títulos traduzidos nunca são re-slugged; um `{#id}` / `{/* #id */}` no meio do título é movido de volta para o sufixo (ou âncora HTML) que o estilo selecionado espera. Arquivos de localidade ausentes são ignorados.
...
1. **Sufixos de ID de cabeçalho** (`{#id}` clássico / `{/* #id */}` MDX no final de um cabeçalho ATX) — removidos completamente da linha e **não** enviados para o modelo. Após a restauração, eles são fixados de volta ao final do cabeçalho correspondente para que o Docusaurus ainda veja um ID válido. Comentários no meio do cabeçalho permanecem no texto e são tratados pelas camadas MDX / `{#…}` posteriores.
2. **Tags e comentários HTML** (`<strong>`, `<!-- ... -->`, etc.) - tags HTML minúsculas de uma lista de permissões conhecida são substituídas por tokens ```{{HTM_N}}```. Tags JSX capitalizadas (`<Highlight>`, `<Tabs>`, `</Tab>`) são tratadas separadamente pela camada MDX (etapa 5).
3. **Marcadores de advertência** (`:::note`, `:::`) - apenas o prefixo da diretiva na linha de abertura é substituído por ```{{ADM_OPEN_N}}```; qualquer título na mesma linha é deixado para o modelo traduzir. Restaurado com o texto original exato.
4. **Âncoras de documento** (HTML `<a id="…">`, `{#…}` Docusaurus remanescente no meio do cabeçalho) - preservadas literalmente.
5. **Constructos apenas MDX** (`src/processors/mdx-placeholders.ts`):
   - **Comentários MDX** (`{/* … */}`, incluindo um `{/* #my-id */}` no meio do cabeçalho que não era um sufixo de fim de linha) substituídos por ```{{MDX_N}}```.
   - **Tags JSX capitalizadas** (`<Highlight>`, `<Tabs>`, `<TabItem>`, `<TOCInline />`, `</Highlight>`) - preservadas como ```{{MDX_N}}``` com atributos de string traduzíveis (`label`, `tooltip`, `aria-label`) reescritos para ```{{JXA_N}}``` dentro da tag, a menos que o nome do atributo apareça em `docs[].protectAttributes`; `label:` dentro de literais de objeto `<Tabs values={[ { label: '…' } ]}>` (ignoráveis via `docs[].protectKeys`) e `<TabItem value="…">` (quando nenhum atributo `label` existe, ignorando valores slug-like minúsculos) também são extraídos. Anexados ao segmento como linhas `||JXA_N: …||`, mesclados de volta por `restoreMdx`.
   - **Expressões de chave MDX** (`{frontMatter.title}`, <code v-pre>style={{…}}</code>) - correspondência sensível à profundidade, substituída por ```{{MDX_N}}```.
6. **URLs Markdown** (`](url)`, `src="…"`) - restauradas de um mapa após a tradução.
7. **Spans de código inline** (`` `code` ``) e **código inline envolto em negrito** (`**`code`**`) - preservados.
8. **Ênfase Markdown** (opcional, ativada automaticamente para localidades CJK/RTL) - delimitadores de ênfase mascarados.

Após o retorno do modelo, o `translate-docs` restaura os mapas e valida o segmento: o mesmo multiconjunto de tokens de chaves duplas deve estar presente, os tokens estruturais (<code v-pre>{{HTM_N}}</code>, marcadores de advertência) devem manter sua subsequência ordenada (tokens de conteúdo como <code v-pre>{{ILC_N}}</code> / <code v-pre>{{URL_N}}</code> / <code v-pre>{{SE}}</code> podem se mover com a ordem das palavras), os tipos de tags HTML restauradas devem corresponder à fonte desprotegida, e qualquer identificador de chaves duplas restante já deve ter existido na fonte (portanto, tokens inventados falham). O prompt do documento também pede aos modelos para copiar cada token uma vez, manter a ordem dos tokens estruturais e não inventar novos wrappers de chaves duplas; as verificações mecânicas permanecem autoritárias.

A proteção de atributos/chaves compartilhados para modelos Astro e MDX JSX é implementada em `src/processors/expression-attribute-protection.ts` e controlada por bloco por `docs[].protectAttributes` e `docs[].protectKeys` (consulte [protectAttributes / protectKeys](/pt-BR/reference/configuration#protectattributes-protectkeys)).
...
As chamadas de modelo faturadas (traduções aceitas e retentativas descartadas) são armazenadas em `api_calls`. Após um comando que registrou chamadas, as linhas de detalhes com mais de sete dias corridos em UTC são consolidadas nas `api_totals` mensais. `usage` e a visualização Uso e custos do painel combinam ambas as tabelas. Consulte [Uso e custos](/pt-BR/guide/translation-dashboard/usage).

As notas do glossário `Context` e as `glossary.contextFiles` são identificadas por fingerprint (`prompt_context_hash`) e injetadas na UI, na documentação, em JSON, SVG e nos prompts de revisão. Alterar essa orientação invalida os segmentos em cache correspondentes e as linhas de rastreamento de arquivos na próxima execução. As linhas de cache de `user-edited` do painel são mantidas. Alterar apenas uma tradução preferida do glossário mantém o cache existente intacto até `--force` / `--force-update`.

O comando `translate-docs` também usa o **rastreamento de arquivos** para que as fontes inalteradas com saídas existentes e atualizadas possam ignorar o trabalho completamente. `--check-cache` reabre os locais com um sistema de escrita esperado para que os segmentos armazenados em cache sejam revalidados; `--force-update` reexecuta o processamento de arquivos para cada local, ainda usando o cache de segmento; `--force` limpa o rastreamento de arquivos e ignora as leituras do cache de segmento para a tradução da API. Quando todos os modelos configurados falham na validação AST em um segmento de markdown, `translate-docs` pode dividir progressivamente o segmento e tentar novamente partes menores (`docs[].segmentSplitting.qualityRetrySplit`, padrão ativado). Consulte [Documentos — comportamento e sinalizadores de cache](/pt-BR/guide/documents/cli-options#cache-behaviour-and-translate-docs-flags) para a tabela completa de sinalizadores.

**Formato de prompt em lote:** `translate-docs --prompt-format` seleciona XML (`<seg>` / `<t>`) ou formatos de array/objeto JSON apenas para `LlmClient.translateDocumentBatch`; extração, placeholders e validação permanecem inalterados. Consulte [Formato de prompt em lote](/pt-BR/guide/documents/cli-options#batch-prompt-format).
...
- **Fallback do modelo**: tenta cada modelo na lista resolvida em ordem; faz fallback em caso de falhas de solicitação ou análise. Cada localidade de destino obtém sua própria cadeia resolvida: `localeModels(locale)` primeiro quando configurado, depois `uiModels` (apenas pipelines de UI), depois `translationModels`. A tradução de Documentos, JSON e SVG cria um cliente por localidade com a cadeia não-UI. O comando `bench-models` em vez disso constrói um cliente de modelo único por ID configurado (união de `translationModels`, `uiModels` e `localeModels`; `translationModels: [id]`, sem fallback) para que possa cronometrar e precificar cada modelo independentemente.
- **Tempo limite da solicitação**: `requestTimeout` (segundos) ou `requestTimeoutMs` no provedor ativo, caso contrário, as mesmas chaves no topo da configuração (padrão 45 segundos), aborta cada solicitação via `AbortSignal.timeout`. O mesmo valor se aplica a `GET /models` quando a CLI carrega uma lista de modelos de um provedor para `check-models` (qualquer provedor). O filtro opcional de pré-voo que descarta IDs de modelo desconhecidos é executado apenas quando o provedor ativo é o OpenRouter.
- **Extras do OpenRouter** (somente quando `openrouter` está ativo): roteamento de throughput via campo de solicitação `provider`, cabeçalhos `HTTP-Referer` / `X-Title` e custo exato em USD lido de `usage.cost`. O uso de tokens é relatado para cada provedor. Quando o provedor omite `usage.cost`, o custo em USD é calculado a partir de `providers.<name>.modelPricing` ou do padrão `pricing` de todo o provedor e armazenado na linha `api_calls`. Um custo relatado pelo provedor nunca é substituído.
- **Log de tráfego de depuração**: se `debugTrafficFilePath` estiver definido, anexa JSON de solicitação e resposta a um arquivo (programático). O `--debug-failed` da CLI escreve `FAILED-TRANSLATION` arquivos em `cacheDir` com o prompt do sistema/usuário, a resposta bruta do assistente e erros de validação para tentativas falhas de verificação de tradução de UI, documentos, JSON e SVG. Falhas de API do provedor / corpo vazio são impressas no console em vez de despejar um arquivo somente de prompt.
...
`init` escreve configurações iniciais de `initConfigTemplates`: `ui-markdown` (UI + markdown de aplicativo opcional), `ui-docusaurus`, `ui-starlight`, `ui-vitepress` (documentos VitePress + `vitepressThemeCatalog`), `ui-nextra` (documentos Nextra + `nextraDictionaryPath`), `ui-astro-website` (UI Astro simples; adicione `docs[]` para tradução de página `.astro`), `ui-json-bundles` (somente `json[]` JSON). Consulte [Início rápido — Inicializar](/pt-BR/guide/quick-start#step-1-initialise).
