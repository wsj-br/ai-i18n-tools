...
| `AI_I18N_LANG` | Idioma da interface do usuário da própria ferramenta (ajuda da CLI, logs, painel). Substituído por `-L` / `--ui-lang`; substitui a configuração `uiLanguage`. Consulte [Idioma da interface do usuário da ferramenta](/guide/tool-ui-language). |
