...
將 `console-app` 替換為任何範例資料夾名稱。每個範例都宣告一個與目前套件版本相符的已發佈插入符號範圍 `ai-i18n-tools`，並從 npm 安裝 CLI。每個範例的 README 都包含相同的程式碼片段，並填入了資料夾名稱。
