...
CLI 將**檔案追蹤**資料儲存於 SQLite（每個檔案 × 地區的來源雜湊）以及**區段**列（每個可翻譯區塊的雜湊 × 地區）。正常執行時，當追蹤的雜湊與目前來源相符、輸出檔案已存在，**且**輸出的修改時間至少與來源一樣新時，會完全跳過該檔案。否則會處理該檔案並使用區段快取，使未變更的文字不會呼叫 API。當儲存的文字未通過文字系統驗證時，區段快取命中也會被拒絕。使用 `--check-cache` 可針對具有預期文字系統的地區（`hi`、`ja`、`zh-Hans`、`ar`、……）繞過檔案層級的跳過，使羅馬化或錯誤文字系統的快取列可以在不使用 `--force-update` 的情況下重新嘗試。

| 旗標                          | 效果                                                                                                                                                                                                                                                              |
|-------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| *(預設)*                   | 當追蹤與磁碟上的輸出相符時跳過未變更的檔案；其餘檔案使用區段快取。錯誤文字系統的快取命中會被拒絕，且僅針對正在處理的檔案重新翻譯。 |
...
| `--debug-failed`              | 全域。為每次失敗的翻譯檢查（錯誤的腳本、解析或品質）在 `cacheDir` 下寫入詳細的 `FAILED-TRANSLATION` 日誌，包含羅馬拼音化的 `zh-Hans`/`hi` 等後備方案——不僅是在所有模型都失敗時。提供者 API 錯誤會列印在控制台上，而不是檔案中。也適用於 `sync` / `sync-ui` / `cleanup`。 |
| `--check-cache`               | 即使檔案追蹤會跳過，仍對具有強制原生文字系統的地區重新驗證快取的區段。沒有預期文字系統的地區仍然跳過。區段快取仍然適用。                                                                               |
