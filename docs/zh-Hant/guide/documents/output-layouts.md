...
- `docsOutput.style = "vitepress"` — 版面配置與 `doc-system` 相同，但 `localeSubpath` 為空；保留 BCP-47 語系資料夾名稱（`localePathLowercase` 預設為 `false`）。請參閱 [VitePress 整合](/zh-Hant/guide/integrations/vitepress)。
- `docsOutput.style = "nextra"` — 版面配置與 `doc-system` 相同，但 `localeSubpath` 為空；英文原始檔位於語系資料夾下（例如 `content/en/`）。請參閱 [Nextra 整合](/zh-Hant/guide/integrations/nextra)。
- `docsOutput.style = "fumadocs"` — 版面配置與 `doc-system` 相同，但 `localeSubpath` 為空；英文原始檔使用點後綴檔案（預設）或當 `fumadocsParser` 為 `"dir"` 時使用語系資料夾。請參閱 [Fumadocs 整合](/zh-Hant/guide/integrations/fumadocs)。
...
如需在平面佈局中建立跨頁錨點連結，請參閱[錨點連結](/zh-Hant/guide/documents/anchor-links)。
