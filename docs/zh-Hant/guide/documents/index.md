...
還原後，`translate-docs` 也會拒絕以下段落：其中 HTML 標籤佔位符被重複使用或丟棄（因此還原後的標籤不再與來源映射相符），或者模型產生了來源中不存在的剩餘雙大括號標記（例如虛構的詞彙表風格標記）。還原前的檢查要求具有相同的 <code v-pre>{{…}}</code> 標記多重集，以及相同的編號標記有序子序列（<code v-pre>{{HTM_N}}</code>、<code v-pre>{{URL_N}}</code>、…）；當每種類型的數量仍然相符時，如 <code v-pre>**</code> 之類的強調標記可以隨自然語序移動。這些失敗情況會使用與剩餘官方內部標記相同的模型後備路徑。
