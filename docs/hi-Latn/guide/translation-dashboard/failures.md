...
UI ke saath file-based debugging ke liye, retries ke dauraan `cacheDir` ke tahat `FAILED-TRANSLATION` detail likhne ke liye `translate-docs --debug-failed` ka upyog karen — [Cache behaviour and `translate-docs` flags](/hi-Latn/guide/documents/cli-options#cache-behaviour-and-translate-docs-flags) dekhen.
...
Pre-translation static checks ke liye [Markdown issues](/hi-Latn/guide/translation-dashboard/markdown-issues) dekhen.
