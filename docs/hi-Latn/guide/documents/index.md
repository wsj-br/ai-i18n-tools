...
[Docusaurus](/hi-Latn/guide/integrations/docusaurus) sites par, `docusaurusCatalogDir` ko apne `write-translations` catalog folder (jaise ki `docs-site/i18n/en`) par bhi set karein. Phir `translate-docs` mein shell JSON bhi shaamil hai - navbar, footer, aur theme strings.
...
[Astro Starlight](/hi-Latn/guide/integrations/astro#astro-starlight) sites par, page bodies aapke Starlight content root (aam taur par `src/content/docs/`) par `docsOutput.style: "astro-starlight"` ka upyog `docsRoot` ke saath karte hain. `translate-docs` English tree ke bagal mein `src/content/docs/<locale>/` ke tahat localized markdown/MDX likhta hai. Starlight kai locales ke liye built-in UI strings bhejta hai — koi alag theme catalog pipeline nahi; optional UI overrides `src/content/i18n/en.json` ke liye `docs[]` block par `jsonPathTemplate` ka upyog kar sakte hain.
...
[SVG](/hi-Latn/guide/svg-translation/) files ko [`translate-svg`](/hi-Latn/reference/cli-commands/content#translate-svg) ke madhyam se translate kiya jaata hai jab `features.translateSVG` enable hota hai - na ki `docs[]` / `contentPaths` ke madhyam se.
...
UI aur docs ke beech **terminology consistency** ke liye, `glossary.uiGlossary` ko apne `strings.json` path par set karein — `translate-docs` existing UI translations ko LLM prompts mein hints ke roop mein dobara upyog karta hai jab segment mein matching terms dikhai dete hain. Optional `glossary.userGlossary` product terms ke liye CSV overrides jodta hai (`translate-ui` aur `proofread-ui` ke saath shared). `glossary-generate` ke saath ek starter CSV generate karein, Translation Dashboard **Glossary** tab mein rows edit karein, ya [Configuration — `glossary`](/hi-Latn/reference/configuration#glossary) aur [Glossary](/hi-Latn/guide/translation-dashboard/glossary) dekhein.

<a id="per-locale-model-overrides"></a>
### Pratyaik sthanik model override

`translate-docs` aur `sync` ke docs step models ko **prati target locale** resolve karte hain: `localeModels(locale)` pehle jab configure kiya jaata hai, phir provider ki global `translationModels` chain. Iska upyog tab karein jab kisi vishesh bhasha ko aapki default fallback list se alag model ki zaroorat ho - udaaharan ke liye, `pt-BR` documentation ke liye Gemini ko prefer karna jab global chain Portuguese ke saath struggle karti hai. Dekhein [Providers and models](/hi-Latn/guide/providers-and-models#model-fallback-chain) aur [Configuration - `localeModels`](/hi-Latn/reference/configuration#provider-and-providers).
...
| --- | --- |
| Docusaurus site | `init -t ui-docusaurus`, `docsOutput.style = "docusaurus"` - [Docusaurus](/hi-Latn/guide/integrations/docusaurus) |
| VitePress site | `init -t ui-vitepress` + `vitepressThemeCatalog` theme ke liye - [VitePress](/hi-Latn/guide/integrations/vitepress) |
| Nextra site | `init -t ui-nextra` + `nextraDictionaryPath` dictionary ke liye (sidebar `_meta.ts` automatic hai) - [Nextra](/hi-Latn/guide/integrations/nextra) |
| Fumadocs site | `init -t ui-fumadocs` + `fumadocsUiCatalog` UI ke liye (sidebar `meta.json` automatic hai) - [Fumadocs](/hi-Latn/guide/integrations/fumadocs) |
| Astro Starlight | `init -t ui-starlight` - [Astro Starlight](/hi-Latn/guide/integrations/astro#astro-starlight) |
| Flat documents (README, changelogs, aadi) | `docsOutput.style = "flat"` - [Output layouts](/hi-Latn/guide/documents/output-layouts), optional [language switcher](/hi-Latn/guide/documents/language-switcher) |
...
| Product terminology aur UI/doc consistency | [Configuration — `glossary`](/hi-Latn/reference/configuration#glossary), [Glossary](/hi-Latn/guide/translation-dashboard/glossary) |
...
Theme dictionary strings ke liye `docs[].nextraDictionaryPath` set karein - [Nextra integration](/hi-Latn/guide/integrations/nextra) dekhein. Sidebar `_meta.ts` labels automatic roop se collect kiye jaate hain.
...
UI overrides ke liye `docsOutput.fumadocsUiCatalog` set karein - [Fumadocs integration](/hi-Latn/guide/integrations/fumadocs) dekhein. Sidebar `meta.json` labels automatic roop se collect kiye jaate hain.
...
- `provider` aur `providers` — `init` ek default provider block (`openrouter` jab tak aap `-P <provider>` pass na karein) scaffold karta hai; kam se kam ek provider configure karein aur uski API key set karein `translate-docs` ya `sync` se pehle (Ollama ko koi key nahi chahiye). [Provider and API key](/hi-Latn/guide/quick-start#provider-and-api-key) aur [LLM providers and models](/hi-Latn/guide/providers-and-models) dekhein.
- `sourceLocale` - source language (`docusaurus.config.js` mein `defaultLocale` se match hona chahiye).
- `targetLocales` - BCP-47 locale codes ka array (jaise ki `["de", "fr", "es"]`).
...
- `glossary.uiGlossary` - `strings.json` ka path taaki document segments ko aapke UI catalog se terminology hints mil sakein ([Configuration — `glossary`](/hi-Latn/reference/configuration#glossary) dekhein).
- `glossary.userGlossary` - fixed product-term translations ke liye optional CSV; UI pipelines dwara bhi upyog kiya jata hai aur [Glossary](/hi-Latn/guide/translation-dashboard/glossary) dashboard tab mein editable hai.
...
**Yadi aapko is tarah ki validation failure milti hai, to source-language text ko saral banana pasand karein** - paragraph ko vibhajit karein, ek udaharan ko fenced code block mein le jaayen, ya kam layered bold/code pairs ke saath usi vichar ka varnan karein - bajaye iske ki har model aur locale se ghane inline markup ko poori tarah se reproduce karne ki ummeed karein.
