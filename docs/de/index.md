...
- **UI-Strings** – Extrahiert `t("…")`-Aufrufe aus JS/TS (und optional `.astro`-Dateien) und generiert flache, pro-Locale-JSON für i18next oder statische SSG-Lookups.
- **Dokumente** – Übersetzt Markdown-, MDX- und `.astro`-Seiten, die in `docs[].contentPaths` aufgeführt sind, unter Verwendung von `translate-docs`. Funktioniert mit **VitePress**, **Starlight**, **Docusaurus**, **Nextra**, **Fumadocs**, Astro-basierten Websites oder jedem statischen Site-Generator, der aus Markdown/MDX/`.astro`-Quelldateien liest.
...
Um den reinen `ai-i18n-tools`-Befehl in Bash/Zsh einzugeben, stellen Sie das lokale Bin-Verzeichnis vor `PATH` (siehe [Verwenden der CLI](/guide/installation#using-the-cli) für PowerShell, direnv und Windows-Hinweise):
...
Bevorzugen Sie `sync` gegenüber dem manuellen Verketten von `extract`, `translate-ui`, `translate-svg`, `translate-docs` und `translate-json` – Reihenfolge und Feature-Flags sind bei manueller Ausführung leicht falsch einzustellen. Siehe [Empfohlene `package.json`-Skripte](/guide/quick-start#recommended-packagejson-scripts) in der Schnellstartanleitung.
...
Verbinden Sie dann i18next in Ihrer App mit den Hilfsprogrammen von `'ai-i18n-tools/runtime'`. Die vollständige Einrichtung finden Sie unter [Schritt 4: i18next zur Laufzeit verbinden](/guide/ui-strings/i18next-runtime) in der Anleitung für UI-Strings.
...
Bearbeiten Sie `ai-i18n-tools.config.json`: Legen Sie `docs[].contentPaths` auf Markdown-, MDX- und/oder `.astro`-Quellen fest; `docs[].outputDir` und `docs[].docsOutput.style` (`"docusaurus"`, `"astro-starlight"`, `"vitepress"`, `"nextra"`, `"fumadocs"`, `"flat"` usw.). Vollständige Feldreferenz: [Dokumente](/guide/documents/).
...
`init -t ui-fumadocs` erstellt `docsOutput.style: "fumadocs"` mit dem Punkt-Parser (Standard) oder dem Verzeichnis-Parser für Nextra-ähnliche Gebietsschema-Ordner. `translate-docs` sammelt und übersetzt automatisch `meta.json`-Seitenleistenbeschriftungen; setzen Sie `docsOutput.fumadocsUiCatalog`, um auch UI-Überschreibungen in `lib/layout.shared.ts` zu übersetzen – alles im selben `sync`-Lauf, ohne JSON-Sidecars. Siehe [Fumadocs-Integration](/guide/fumadocs-integration) und [examples/fumadocs-docs](https://github.com/wsj-br/ai-i18n-tools/tree/main/examples/fumadocs-docs/).

<a id="astro-plain-astro--starlight"></a>
### Astro (reines Astro & Starlight)

**Astro Starlight** – `init -t ui-starlight` und dann `translate-docs`. Starlight-UI-Überschreibungen können `src/content/i18n/en.json` mit `jsonPathTemplate` in einem separaten `docs[]`-Block verwenden, wenn dies erforderlich ist ([Dokumente – für die Dokumentation initialisieren](/guide/documents/#step-1-initialise-for-documentation)).
...
Gerüst-UI mit `init -t ui-astro-website`. Für hartkodiertes HTML in `.astro`-Seiten aktivieren Sie `features.translateDocs` und fügen Sie einen `docs[]`-Block mit `docsOutput.style: "astro-starlight"` hinzu (siehe [Astro-Webseiten (Parsen und Ersetzen)](/guide/ui-strings/astro-website#astro-website-pages-parse-and-replace)). Halten Sie `targetLocales`, `i18n.locales` in `astro.config.mjs` und `ui-languages.json` aufeinander abgestimmt (Astro-Routen verwenden Kleinbuchstaben-Codes wie `pt-br`; flache Bundle-Dateinamen folgen der Konfigurationsschreibweise, z. B. `pt-BR.json`).

Verbinden Sie `t()` zur Build-Zeit ohne i18next, es sei denn, Sie fügen Client-Islands hinzu – siehe [Astro-Website UI-Strings (SSG)](/guide/ui-strings/astro-website#astro-website-ui-strings-ssg) und die `src/i18n/t.ts` des Beispiels.
...
| `extractInterpolationNamesForWrap(key)`                                  | Parst <code v-pre>{{var}}</code>-Namen aus einem Quellschlüssel für `wrapT` / Schlüsselkürzung-Rückfalloption.                                                              |
...
| `interpolateTemplate(str, vars)` | Low-Level <code v-pre>{{var}}</code>-Substitution an einem einfachen String (wird intern verwendet; Anwendungscode sollte stattdessen `t()` nutzen). |
...
Für einfache HTML-Apps versehen Sie Elemente mit reinen `data-i18n` / `data-i18n-title` / `data-i18n-placeholder`-Markierungen (der Quelltext wird einmalig aus dem textContent / title / placeholder des Elements selbst entnommen); `mark-html` fügt sie für Sie ein und `extract` erfasst sie dann in `strings.json`. Siehe [HTML zur Übersetzung markieren](/guide/ui-strings/plain-html#marking-html-for-translation).

Vollständige Listen der pro Befehl verfügbaren Flags finden Sie in der [CLI-Referenz](/reference/cli-commands). Führen Sie `ai-i18n-tools <command> --help` für den integrierten Nutzungstext aus.

Globale Optionen: `-c <config>` (Standard: `ai-i18n-tools.config.json`), `-v` (ausführlich), `-P` / `--provider <name>` (überschreibt den aktiven LLM-Anbieter; muss unter `providers` konfiguriert werden), `-L` / `--ui-lang <code>` (Sprache für die Benutzeroberfläche/Protokolle des Tools), `-V` / `--version` und `-h` / `--help` – bei jedem Befehl akzeptiert. `-w` / `--write-logs [path]` leitet die Konsolenausgabe an eine Protokolldatei weiter (Standard: im Übersetzungscache-Verzeichnis), wirkt sich aber nur auf die Übersetzungs- und Synchronisierungsbefehle aus (`translate-docs`, `translate-json`, `translate-svg`, `translate-ui`, `sync-ui`, `sync`, `cleanup`). Mehrere Befehle akzeptieren `-l` / `--locale <codes>` (durch Kommas getrennte BCP-47), um die Zielsprachen zu begrenzen; `proofread-ui` verwendet eine einzelne Ausgangssprache. Eine Übersichtstabelle der Befehle finden Sie in der [CLI-Referenz](/reference/cli-commands).
...
Das angeforderte Gebietsschema wird exakt oder durch die nächstgelegene Variante mit den gelieferten UI-Sprachen abgeglichen (z. B. wird `pt-PT` zu `pt-BR` und `en-US` zu `en-GB` aufgelöst); wenn nichts übereinstimmt, wird auf das Ausgangsgebietsschema (`en-GB`) zurückgegriffen. Wenn eine UI-Sprache explizit angefordert wird (über das Flag, die Umgebungsvariable oder `uiLanguage`), aber kein geliefertes Bundle übereinstimmt, gibt die CLI eine einmalige Warnung aus, dass das Standardgebietsschema verwendet wird; ein Gebietsschema, das nur vom Host-Betriebssystem abgeleitet wird, warnt nie. Dies ist unabhängig von den `sourceLocale` / `targetLocales` Ihres Projekts. Gelieferte UI-Sprachen: `en-GB` (Quelle) plus `de`, `es`, `fr`, `hi-Latn`, `ja`, `ko`, `pt-BR`, `zh-Hans` und `zh-Hant`. Keine Konfiguration erforderlich – standardmäßig folgt das Tool dem Gebietsschema Ihres Betriebssystems. Weitere Details finden Sie unter [Tool-UI-Sprache](/reference/environment-variables#tool-ui-language).
...
- [Dokumentationsseite](https://wsj-br.github.io/ai-i18n-tools/) – vollständige VitePress-Anleitung (9 Gebietsschemas auf GitHub Pages).
- [Schnellstart](/guide/quick-start) – Einrichtung für UI-Strings, Dokumente und JSON (UI, Dokumente/`.astro`, JSON-Bundles, VitePress, Nextra, Fumadocs, Astro Starlight und einfaches Astro).
- [Anleitung für Gebietsschema-Assets](/guide/images-and-screenshots/) – Screenshots und illustrierte SVGs in übersetzten Dokumenten (flacher Link-Umschreiber, Screenshot-Skripte).
