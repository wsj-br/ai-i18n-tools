...
Nachdem das Modell zurückgegeben wurde, stellt `translate-docs` Karten wieder her und validiert das Segment: Nicht-Betonungs-Doppelklammer-Tokens müssen dieselbe geordnete Untersequenz wie die geschützte Quelle beibehalten (Betonungsmarker wie <code v-pre>{{SE}}</code> können sich mit der Wortreihenfolge bewegen, wenn die Pro-Typ-Zählungen übereinstimmen), wiederhergestellte HTML-Tag-Arten müssen mit der ungeschützten Quelle übereinstimmen, und jeder übrig gebliebene Doppelklammer-Bezeichner muss bereits in der Quelle existiert haben (so dass erfundene Tokens fehlschlagen). Die Dokumentaufforderung fordert die Modelle außerdem auf, jedes Token einmal zu kopieren, die Reihenfolge der nummerierten Tokens beizubehalten und keine neuen Doppelklammer-Wrapper zu erfinden; mechanische Prüfungen bleiben maßgebend.
