...
Es erfordert eine gültige Konfiguration mit **mindestens einem `docs[]`-Block**. Für jeden Block werden `.md`- / `.mdx`-Dateien unter `contentPaths` gesammelt, die `.translate-ignore`-Regeln des Projekts angewendet (gleiches Prinzip wie bei der Dokumentübersetzung) und optional auf einen Unterbaum mit `--path` / `--file` beschränkt. Jede Datei wird mit `applyHeadingAnchorsToMarkdown` transformiert: Für jede **flache ATX-Überschrift** (`# …` bis `###### …`) außerhalb von Codeblöcken wird entweder eine leere HTML-Zeile `<a id="slug"></a>` in der darüberliegenden Zeile eingefügt, wenn sie fehlt oder veraltet ist, oder – mit `--slug-style mdx-comment` – ein Docusaurus MDX-Suffix `{/* #slug */}` an die Überschriftenzeile angehängt. Slug-Algorithmen entsprechen gängigen Ökosystemen – `github` (Standard), `bitbucket`, `gitlab`, `pymdown` (optionale Unicode-Normalisierungs- / Prozentkodierungs-Flags), `azure-devops`, plus `mdx-comment` (GitHub-Slug + MDX-Kommentarausgabe) – sodass Anker-IDs mit bestehenden Tools (doctoc, PyMdown, Docusaurus usw.) konsistent bleiben. `--dry-run` meldet potenzielle Änderungen, ohne zu schreiben.
