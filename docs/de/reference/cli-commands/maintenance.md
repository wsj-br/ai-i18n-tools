...
Löscht die gesamte Tabelle `markdown_source_issues` und führt dann `sync --force-update` aus (Extrahieren, UI, SVG, Dokumente und `translate-json`, falls aktiviert), sodass Markdown-Probleme für aktuell konfigurierte Dokumente neu aufgefüllt werden; entfernt dann veraltete Segmentzeilen (null `last_hit_at` / leerer Dateipfad); löscht `file_tracking`-Zeilen, deren aufgelöster Quellpfad auf der Festplatte fehlt; entfernt Übersetzungszeilen, deren `filepath`-Metadaten auf eine fehlende Datei verweisen; bereinigt verwaiste `translation_failures`-Zeilen; und löscht Cache-Zeilen für nicht in der Konfiguration enthaltene Gebietsschemas (`sourceLocale`, Stamm-`targetLocales` und alle pro Block `docs[]` / `json[]` `targetLocales`). Nur Cache für eingestellte Gebietsschemas – generierte Dokumente, flache UI-Dateien und `strings.json`-Einträge bleiben erhalten (verwenden Sie [`purge-locale`](#purge-locale), um diese zu entfernen). Protokolliert die Bereinigungsanzahl nach der Synchronisierung (veraltete Segmente, verwaiste `file_tracking`, verwaiste Übersetzungen, verwaiste Fehler, nicht konfigurierte Gebietsschemas) sowie die anfängliche Anzahl der gelöschten Markdown-Probleme.
