...
1. Fügen Sie eine explizite ID in der Überschriftenzeile mit dem klassischen `{#…}`-Suffix von Docusaurus (CommonMark) oder dem MDX-Kommentar `{/* #… */}` (bevorzugt für `.mdx`) hinzu, z. B. `## TLS configuration {#tls-configuration}` oder `## TLS configuration {/* #tls-configuration */}`. Während der `translate-docs` wird nur der sichtbare Überschriftentext an das Modell gesendet – das ID-Suffix wird zuerst entfernt und am **Ende** der übersetzten Überschriftenzeile wieder angefügt (Docusaurus ignoriert ein `{/* #id */}`, das mitten im Titel landet).
2. Führen Sie `docusaurus write-heading-ids` aus dem Stammverzeichnis Ihres Docusaurus-Projekts aus (oft `pnpm run write-heading-ids`, wenn es in `package.json` verdrahtet ist), um IDs zu Überschriften hinzuzufügen oder zu aktualisieren, die keine haben – verwenden Sie `--syntax mdx-comment` für die `{/* #… */}`-Form. Alternativ führen Sie `ai-i18n-tools write-heading-ids --slug-style mdx-comment` auf demselben `docs[]` / `contentPaths` aus. Dieser Befehl positioniert auch dieselben englischen IDs in vorhandenen übersetzten Dateien neu (er schlägt den übersetzten Titel nicht). Führen Sie ihn nach dem Umbenennen von Überschriften erneut aus, damit veraltete IDs mit den aktuellen Titeln übereinstimmen.
...
1. Führen Sie `ai-i18n-tools write-heading-ids` für Ihre Quell-`.md` / `.mdx` vor `translate-docs` aus (gleiche `docs[]` / `contentPaths` wie üblich). Es fügt explizite HTML-Anker in der Zeile vor jeder Überschrift ein, sodass `id`-Werte von jeder übersetzten Kopie gemeinsam genutzt werden, und es kopiert dieselben englischen IDs in vorhandene übersetzte Dateien. Führen Sie es nach dem Umbenennen von Überschriften erneut aus, damit veraltete Anker-IDs aktualisiert werden, um dem aktuellen Titel zu entsprechen.
2. Richten Sie Ihre Markdown-**Ankerlinks** auf diese stabilen IDs aus, z. B. `[label](other.md#section-id)`, wobei `section-id` dem Anker entspricht, den das Tool geschrieben hat – nicht einer Vermutung, die nur auf englischen Wörtern basiert.
