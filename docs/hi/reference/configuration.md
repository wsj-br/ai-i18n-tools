...
**translate-docs**, **translate-svg**, और **translate-json** (और `sync` के अंदर के मिलान वाले चरण): प्रति फ़ाइल अधिकतम समानांतर LLM **बैच** अनुरोध (प्रत्येक बैच में कई सेगमेंट हो सकते हैं)। छोड़े जाने पर डिफ़ॉल्ट **4**। `translate-ui` पर लागू नहीं होता है — इसके बजाय `uiBatchConcurrency` का उपयोग करें। `-b` / `--batch-concurrency` के साथ ओवरराइड करें।

---

<a id="uibatchconcurrency-optional"></a>
### `uiBatchConcurrency` (वैकल्पिक)

**अनुवाद-यूआई**, **समन्वय-यूआई**, और `sync` के यूआई चरण: अधिकतम समानांतर LLM **बैच** अनुरोध **एकल स्थानीयता** के भीतर (50 के प्लेन-स्ट्रिंग टुकड़े, फिर बहुवचन समूह)। डिफ़ॉल्ट **2** जब छोड़ा गया। `concurrency` (समानांतर लक्षित स्थानीयताएँ) और `batchConcurrency` (दस्तावेज़/JSON/SVG) से स्वतंत्र। कोई CLI ध्वज नहीं; इसे कॉन्फ़िगरेशन में सेट करें या प्रोग्रामेटिक `runTranslateUI` के लिए `uiBatchConcurrency` पास करें।
...
डिफ़ॉल्ट लोकेल समवर्ती **4** के साथ, यह **8** इन-फ़्लाइट UI API कॉल तक है। एक बड़े लोकेल (`-l de`) का अनुवाद करते समय इसे बढ़ाएँ; यदि प्रदाता दर-सीमित करता है तो इसे कम रखें।
...
### `requestTimeout` / `requestTimeoutMs` (वैकल्पिक)

प्रत्येक LLM अनुरोध के लिए प्रतीक्षा करने का अधिकतम समय, प्रत्येक प्रदाता के लिए। `requestTimeout` पूरे सेकंड में है; `requestTimeoutMs` मिलीसेकंड में है। डिफ़ॉल्ट: जब दोनों को छोड़ दिया जाता है तो **45** सेकंड। दोनों में से केवल एक सेट करें। एक प्रदाता जो किसी भी फ़ील्ड को सेट करता है, वह उस प्रदाता के लिए उस मान का उपयोग करता है।
...
- `requestTimeout`
  इस प्रदाता को भेजे गए प्रत्येक अनुरोध के लिए प्रतीक्षा करने का अधिकतम समय सेकंड में। शीर्ष-स्तरीय `requestTimeout` / `requestTimeoutMs` को ओवरराइड करता है। जब न तो यह प्रदाता और न ही शीर्ष-स्तरीय कॉन्फ़िगरेशन टाइमआउट सेट करता है, तो डिफ़ॉल्ट `45` सेकंड होता है। एक ही ऑब्जेक्ट पर `requestTimeout` और `requestTimeoutMs` में से केवल एक ही सेट करें।
- `requestTimeoutMs`
  इस प्रदाता को भेजे गए प्रत्येक अनुरोध के लिए प्रतीक्षा करने का अधिकतम समय मिलीसेकंड में। शीर्ष-स्तरीय `requestTimeout` / `requestTimeoutMs` को ओवरराइड करता है। जब न तो यह प्रदाता और न ही शीर्ष-स्तरीय कॉन्फ़िगरेशन टाइमआउट सेट करता है, तो डिफ़ॉल्ट `45000` (45 सेकंड) होता है। एक ही ऑब्जेक्ट पर `requestTimeout` और `requestTimeoutMs` में से केवल एक ही सेट करें।
- `pricing` (वैकल्पिक)
  प्रदाता-व्यापी USD प्रति 1,000,000 टोकन: `{ "inputPerMTokens": 0.15, "outputPerMTokens": 0.6 }`। जब किसी बिल किए गए कॉल में प्रदाता द्वारा रिपोर्ट किया गया `usage.cost` नहीं होता है (OpenRouter के अलावा अधिकांश प्रदाता), तो यह दर उस कॉल के इनपुट और आउटपुट टोकन पर लागू होती है। राशि अनुवाद सारांश में शामिल की जाती है और `api_calls` पंक्ति पर संग्रहीत की जाती है। एक मेल खाती `modelPricing` प्रविष्टि इस डिफ़ॉल्ट को ओवरराइड करती है। प्रदाता द्वारा रिपोर्ट की गई लागत को कभी भी बदला नहीं जाता है। बिना लागत के संग्रहीत की गई पंक्तियों का अनुमान बाद में [`usage`](/hi/reference/cli-commands/workflows#usage) और [उपयोग और लागतें](/hi/guide/translation-dashboard/usage) द्वारा लगाया जा सकता है।
- `modelPricing` (वैकल्पिक)
  प्रति-मॉडल USD प्रति 1,000,000 टोकन: `{ "<model-id>": { "inputPerMTokens": 2.5, "outputPerMTokens": 10 } }`। उस मॉडल आईडी के लिए `pricing` को ओवरराइड करता है। कॉल के समय लागू होता है जब प्रदाता ने `usage.cost` को छोड़ दिया हो, और कॉल के साथ संग्रहीत किया जाता है।
...
एक लेगेसी शीर्ष-स्तरीय `openrouter` ब्लॉक (`baseUrl`, `translationModels`, `defaultModel`, `fallbackModel`, `maxTokens`, `temperature`, `requestTimeout`, `requestTimeoutMs` के साथ) अभी भी स्वीकार किया जाता है और लोड होने पर `providers.openrouter` (`provider: "openrouter"` के साथ) में स्वतः माइग्रेट हो जाता है; `defaultModel` / `fallbackModel` `translationModels` में समाहित हो जाते हैं।

एक चलाने योग्य उदाहरण के लिए जो एक कॉन्फ़िग में कई प्रदाताओं को कॉन्फ़िगर करता है और `-P` के साथ उनके बीच स्विच करता है, [`examples/multi-provider`](https://github.com/wsj-br/ai-i18n-tools/tree/main/examples/multi-provider/) (एक ही दस्तावेज़ पर `openai`, `anthropic`, `openrouter`, और `deepseek`) देखें।
...
अनुवाद के बाद सापेक्ष लिंक को पुनः लिखें (जब `docsOutput.style = "flat"` हो और कोई कस्टम `pathTemplate` न हो, तो स्वतः सक्षम हो जाता है)।
- `docsOutput.linkRewriteDocsRoot`
रेपो रूट का उपयोग फ्लैट-लिंक रिराइट प्रीफिक्स की गणना करते समय किया जाता है। आमतौर पर इसे `"."` के रूप में ही छोड़ दें, जब तक कि आपके अनुवादित डॉक्स किसी अन्य प्रोजेक्ट रूट के अंतर्गत न हों।
- `docsOutput.rewriteVitepressLinks`
जब `true` हो, तो अनुवाद के बाद VitePress लिंक नॉर्मलाइज़र चलाएँ। जब `docsOutput.style`, `"vitepress"` हो तो यह डिफ़ॉल्ट रूप से सक्षम होता है। किसी भी `doc-system` लेआउट के साथ उपयोग करें जहाँ लोकेल फ़ोल्डर `docsRoot` के अंतर्गत अंग्रेज़ी के साथ रखे गए हों। README-शैली के `docs/guide/…` पथों को साइट रूट (`/guide/…`) और लोकेल-सापेक्ष `../guide/…` लिंक में पुनः लिखता है। VitePress ट्री के बाहर रेपो फ़ाइलों के लिंक (`LICENSE`, `examples/`) के लिए, अंग्रेज़ी स्रोत में पूर्ण URL का उपयोग करें — [VitePress एकीकरण — डॉक्स होमपेज के रूप में README](/hi/guide/integrations/vitepress#readme-and-the-docs-homepage) देखें।
- `docsOutput.rewriteNextraLinks`
जब `true` हो, तो अनुवाद के बाद Nextra लिंक नॉर्मलाइज़र चलाएँ। जब `docsOutput.style`, `"nextra"` हो तो यह डिफ़ॉल्ट रूप से सक्षम होता है। Next.js `i18n` के लिए `content/en/…` और सापेक्ष `.mdx` पथों को लोकेल-तटस्थ साइट रूट (`/guide/…`) में पुनः लिखता है। [Nextra एकीकरण — लिंक कन्वेंशन](/hi/guide/integrations/nextra#link-conventions) देखें।
...
| `contentPaths` | प्रोजेक्ट रूट के तहत स्रोत `.json` फ़ाइलें, निर्देशिकाएँ, या ग्लोब। विशिष्ट i18next नेमस्पेस फ़ाइलें (`public/locales/en/*.json`) समर्थित हैं: नेस्टेड ऑब्जेक्ट, एरे, स्ट्रिंग मानों में <code v-pre>{{var}}</code> इंटरपोलेशन, और स्वतंत्र बहुवचन प्रत्यय कुंजियाँ (`key_one`, `key_other`)। |
...
रिपॉजिटरी से `contextFiles` का ड्राफ्ट तैयार करने के लिए, [AI एजेंट के साथ कॉन्टेक्स्ट फ़ाइल जनरेट करें](/hi/guide/glossary#generate-a-context-file-with-an-ai-agent) में कॉपी-पेस्ट एजेंट प्रॉम्प्ट का उपयोग करें। शब्द पंक्तियों और कॉन्टेक्स्ट फ़ाइलों को कैसे लागू किया जाता है, यह जानने के लिए [शब्दावली](/hi/guide/glossary) देखें।
