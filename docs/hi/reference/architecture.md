...
| | `src/glossary/` | प्रॉम्प्ट के लिए शब्दावली लोडिंग, शब्द संकेत और वैकल्पिक प्रोजेक्ट-संदर्भ फ़ाइलें |
...
- जब लक्ष्य लोकेल में अपेक्षित लेखन प्रणाली हो तो एक स्क्रिप्ट निर्देश को पहले लगाएं (स्पष्ट BCP-47 स्क्रिप्ट, या एक भाषा डिफ़ॉल्ट जैसे `hi` → देवनागरी, `ar` → अरबी, `ja` → जापानी काना/कांजी)।
...
`LlmClient.translateUIBatch` प्रत्येक मॉडल को क्रम से आज़माता है, पार्स, नेटवर्क, या गलत-स्क्रिप्ट त्रुटियों (देशी-स्क्रिप्ट लोकेल के लिए रोमनकृत/लैटिन फ़ॉलबैक सहित) पर वापस आता है। CLI `localeModels`, वैकल्पिक `uiModels`, और `translationModels` से प्रति लक्ष्य लोकेल के लिए उस सूची का निर्माण करता है ([प्रदाता और मॉडल](/hi/guide/providers-and-models#model-fallback-chain) देखें)।
...
यह एक वैध कॉन्फ़िग की आवश्यकता होती है जिसमें **कम से कम एक `docs[]` ब्लॉक** हो। प्रत्येक ब्लॉक के लिए यह `.md` / `.mdx` फ़ाइलों को `contentPaths` के तहत इकट्ठा करता है, परियोजना के `.translate-ignore` नियमों (डॉक ट्रांसलेशन के समान विचार) को लागू करता है, और वैकल्पिक रूप से `--path` / `--file` के साथ एक सबट्री तक सीमित करता है। प्रत्येक फ़ाइल को `applyHeadingAnchorsToMarkdown` के साथ परिवर्तित किया जाता है: बाड़े वाले कोड ब्लॉक के बाहर **फ्लैट एटीएक्स हेडिंग** (`# …` से `###### …`) के लिए, मौजूदा हेडिंग आईडी किसी भी रूप में चुने गए शैली से बदल दी जाती है। एचटीएमएल शैलियां एक सuffix-मुक्त हेडिंग के ऊपर की पंक्ति पर `<a id="slug"></a>` लिखती हैं; `--slug-style mdx-comment` हेडिंग पंक्ति पर `{/* #slug */}` (और एक पूर्ववर्ती एचटीएमएल एंकर को छोड़ देती है) लिखती है। स्लग हमेशा वर्तमान हेडिंग पाठ से आते हैं। `--remove` एचटीएमएल एंकर, क्लासिक `{#id}` सuffixes, और एमडीएक्स टिप्पणी आईडी को नए लिखे बिना हटा देता है। स्लग अल्गोरिदम सामान्य पारिस्थितिकी तंत्र — `github` (डिफ़ॉल्ट), `bitbucket`, `gitlab`, `pymdown` (वैकल्पिक यूनिकोड सामान्यीकरण / प्रतिशत-एन्कोडिंग फ़्लैग), `azure-devops`, प्लस `mdx-comment` (गिटहब स्लग + एमडीएक्स टिप्पणी आउटपुट) — से मेल खाते हैं ताकि एंकर आईडी मौजूदा टूलिंग (doctoc, PyMdown, Docusaurus, आदि) के साथ संगत रहें। `--dry-run` रिपोर्ट लिखे बिना संभावित संपादन करता है।

स्रोत पास के बाद, वही कमांड उन अंग्रेज़ी आईडी को प्रत्येक लोकेल के मौजूदा अनुवादित मार्कडाउन (`resolveTranslatedOutputPath`) पर फिर से स्थापित करता है। अनुवादित शीर्षकों को कभी भी फिर से स्लग नहीं किया जाता है; एक मध्य-शीर्षक `{#id}` / `{/* #id */}` को प्रत्यय (या HTML एंकर) पर वापस ले जाया जाता है जिसकी चयनित शैली अपेक्षा करती है। गुम लोकेल फ़ाइलों को छोड़ दिया जाता है।
...
1. **शीर्षक-आईडी प्रत्यय** (क्लासिक `{#id}` / MDX `{/* #id */}` एक ATX शीर्षक के अंत में) — पूरी तरह से लाइन से हटा दिए जाते हैं और मॉडल को **नहीं** भेजे जाते हैं। पुनर्स्थापित करने के बाद उन्हें मिलान शीर्षक के अंत में वापस पिन कर दिया जाता है ताकि Docusaurus अभी भी एक वैध आईडी देख सके। मध्य-शीर्षक टिप्पणियाँ पाठ में रहती हैं और बाद की MDX / `{#…}` परतों द्वारा संभाली जाती हैं।
2. **HTML टैग और टिप्पणियाँ** (`<strong>`, `<!-- ... -->`, आदि) - ज्ञात अनुमति सूची से छोटे अक्षरों वाले HTML टैग को ```{{HTM_N}}``` टोकन से बदल दिया जाता है। बड़े अक्षरों वाले JSX टैग (`<Highlight>`, `<Tabs>`, `</Tab>`) को MDX परत (चरण 5) द्वारा अलग से संभाला जाता है।
3. **एडमॉनिशियन मार्कर** (`:::note`, `:::`) - केवल शुरुआती लाइन पर निर्देश उपसर्ग को ```{{ADM_OPEN_N}}``` से बदल दिया जाता है; किसी भी समान-लाइन शीर्षक को मॉडल के अनुवाद के लिए छोड़ दिया जाता है। सटीक मूल पाठ के साथ पुनर्स्थापित किया गया।
4. **डॉक्टर एंकर** (HTML `<a id="…">`, बचा हुआ मध्य-शीर्षक Docusaurus `{#…}`) - शब्दशः संरक्षित।
5. **केवल MDX-निर्माण** (`src/processors/mdx-placeholders.ts`):
   - **MDX टिप्पणियाँ** (`{/* … */}`, जिसमें एक मध्य-शीर्षक `{/* #my-id */}` शामिल है जो लाइन-एंड प्रत्यय नहीं था) को ```{{MDX_N}}``` से बदल दिया गया।
   - **बड़े अक्षरों वाले JSX टैग** (`<Highlight>`, `<Tabs>`, `<TabItem>`, `<TOCInline />`, `</Highlight>`) - ```{{MDX_N}}``` के रूप में संरक्षित, जिसमें अनुवाद योग्य स्ट्रिंग विशेषताएँ (`label`, `tooltip`, `aria-label`) टैग के अंदर ```{{JXA_N}}``` में फिर से लिखी जाती हैं, जब तक कि विशेषता का नाम `docs[].protectAttributes` में दिखाई न दे; `<Tabs values={[ { label: '…' } ]}>` ऑब्जेक्ट लिटरल (`docs[].protectKeys` के माध्यम से छोड़े जा सकने वाले) के अंदर `label:` और `<TabItem value="…">` (जब कोई `label` विशेषता मौजूद न हो, छोटे अक्षरों वाले स्लग-जैसे मानों को छोड़ते हुए) भी निकाले जाते हैं। खंड में `||JXA_N: …||` लाइनों के रूप में जोड़े जाते हैं, `restoreMdx` द्वारा वापस मर्ज किए जाते हैं।
   - **MDX ब्रेस एक्सप्रेशन** (`{frontMatter.title}`, <code v-pre>style={{…}}</code>) - गहराई-जागरूक मिलान, ```{{MDX_N}}``` से बदला गया।
6. **मार्कडाउन URL** (`](url)`, `src="…"`) - अनुवाद के बाद एक मैप से पुनर्स्थापित किए गए।
7. **इनलाइन कोड स्पैन** (`` `code` ``) और **बोल्ड-रैप्ड इनलाइन कोड** (`**`code`**`) - संरक्षित।
8. **मार्कडाउन जोर** (वैकल्पिक, CJK/RTL लोकेल के लिए स्वतः सक्षम) - जोर देने वाले सीमांकक मास्क किए गए।
...
बिल किए गए मॉडल कॉल (स्वीकृत अनुवाद और रद्द किए गए पुनःप्रयास) `api_calls` में संग्रहीत किए जाते हैं। कॉल रिकॉर्ड करने वाले कमांड के बाद, सात UTC कैलेंडर दिनों से पुरानी विवरण पंक्तियाँ मासिक `api_totals` में स्थानांतरित हो जाती हैं। `usage` और डैशबोर्ड का उपयोग और लागत दृश्य दोनों तालिकाओं को संयोजित करते हैं। [उपयोग और लागत](/hi/guide/translation-dashboard/usage) देखें।

शब्दावली `Context` नोट्स और `glossary.contextFiles` का फ़िंगरप्रिंट (`prompt_context_hash`) तैयार किया जाता है और उन्हें UI, दस्तावेज़ों, JSON, SVG और प्रूफ़रीड प्रॉम्प्ट में इंजेक्ट किया जाता है। उस मार्गदर्शन को बदलने से अगले रन पर मेल खाने वाले कैश किए गए सेगमेंट और फ़ाइल-ट्रैकिंग पंक्तियाँ अमान्य हो जाती हैं। डैशबोर्ड `user-edited` कैश पंक्तियाँ सुरक्षित रखी जाती हैं। केवल पसंदीदा शब्दावली अनुवाद बदलने से मौजूदा कैश `--force` / `--force-update` तक यथावत बना रहता है।

`translate-docs` कमांड **फ़ाइल ट्रैकिंग** का भी उपयोग करता है ताकि मौजूदा, अप-टू-डेट आउटपुट वाले अपरिवर्तित स्रोत पूरी तरह से काम छोड़ सकें। `--check-cache` अपेक्षित लेखन प्रणाली के साथ लोकेल को फिर से खोलता है ताकि कैश किए गए सेगमेंट को फिर से मान्य किया जा सके; `--force-update` सेगमेंट कैश का उपयोग करते हुए प्रत्येक लोकेल के लिए फ़ाइल प्रोसेसिंग को फिर से चलाता है; `--force` फ़ाइल ट्रैकिंग को साफ़ करता है और API अनुवाद के लिए सेगमेंट कैश रीड को बायपास करता है। जब प्रत्येक कॉन्फ़िगर किया गया मॉडल एक मार्कडाउन सेगमेंट पर AST सत्यापन में विफल रहता है, तो `translate-docs` सेगमेंट को क्रमिक रूप से विभाजित कर सकता है और छोटे हिस्सों को फिर से प्रयास कर सकता है (`docs[].segmentSplitting.qualityRetrySplit`, डिफ़ॉल्ट रूप से चालू)। पूर्ण फ़्लैग तालिका के लिए [दस्तावेज़ — कैश व्यवहार और फ़्लैग](/hi/guide/documents/cli-options#cache-behaviour-and-translate-docs-flags) देखें।
...
- **मॉडल फ़ॉलबैक**: हल की गई सूची में प्रत्येक मॉडल को क्रम से आज़माता है; अनुरोध या पार्स विफलताओं पर फ़ॉलबैक करता है। प्रत्येक लक्ष्य लोकेल को अपनी हल की गई श्रृंखला मिलती है: कॉन्फ़िगर होने पर `localeModels(locale)` पहले, फिर `uiModels` (केवल UI पाइपलाइन), फिर `translationModels`। दस्तावेज़, JSON और SVG अनुवाद गैर-UI श्रृंखला के साथ प्रति-लोकेल क्लाइंट बनाते हैं। `bench-models` कमांड इसके बजाय प्रति कॉन्फ़िगर आईडी एक सिंगल-मॉडल क्लाइंट बनाता है (`translationModels`, `uiModels` और `localeModels` का संघ; `translationModels: [id]`, कोई फ़ॉलबैक नहीं) ताकि यह प्रत्येक मॉडल को स्वतंत्र रूप से समय और कीमत दे सके।
- **अनुरोध टाइमआउट**: सक्रिय प्रदाता पर `requestTimeout` (सेकंड) या `requestTimeoutMs`, अन्यथा कॉन्फ़िग के शीर्ष पर समान कुंजियाँ (डिफ़ॉल्ट 45 सेकंड), `AbortSignal.timeout` के माध्यम से प्रत्येक अनुरोध को रद्द करता है। जब CLI `check-models` (कोई भी प्रदाता) के लिए प्रदाता की मॉडल सूची लोड करता है तो `GET /models` पर समान मान लागू होता है। वैकल्पिक प्री-फ़्लाइट फ़िल्टर जो अज्ञात मॉडल आईडी को हटाता है, तभी चलता है जब सक्रिय प्रदाता OpenRouter हो।
- **OpenRouter अतिरिक्त** (केवल जब `openrouter` सक्रिय हो): `provider` अनुरोध फ़ील्ड, `HTTP-Referer` / `X-Title` हेडर और `usage.cost` से पढ़े गए सटीक USD लागत के माध्यम से थ्रूपुट रूटिंग। टोकन उपयोग प्रत्येक प्रदाता के लिए रिपोर्ट किया जाता है। जब प्रदाता `usage.cost` को छोड़ देता है, तो USD लागत `providers.<name>.modelPricing` या प्रदाता-व्यापी `pricing` डिफ़ॉल्ट से गणना की जाती है और `api_calls` पंक्ति पर संग्रहीत की जाती है। प्रदाता-रिपोर्ट की गई लागत को कभी भी बदला नहीं जाता है।
- **डीबग ट्रैफ़िक लॉग**: यदि `debugTrafficFilePath` सेट है, तो अनुरोध और प्रतिक्रिया JSON को एक फ़ाइल (प्रोग्रामेटिक) में जोड़ता है। CLI `--debug-failed` `cacheDir` के तहत `FAILED-TRANSLATION` फ़ाइलें लिखता है जिसमें सिस्टम/उपयोगकर्ता प्रॉम्प्ट, कच्ची सहायक प्रतिक्रिया और विफल UI, डॉक्स, JSON और SVG अनुवाद-जांच प्रयासों के लिए सत्यापन त्रुटियाँ होती हैं। प्रदाता API / खाली-बॉडी विफलताएँ केवल प्रॉम्प्ट-फ़ाइल को डंप करने के बजाय कंसोल पर प्रिंट होती हैं।
