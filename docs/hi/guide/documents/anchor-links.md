...
1. Docusaurus के क्लासिक `{#…}` प्रत्यय (CommonMark) या MDX टिप्पणी `{/* #… */}` (`.mdx` के लिए अनुशंसित) के साथ शीर्षक पंक्ति पर एक स्पष्ट आईडी जोड़ें, उदाहरण के लिए `## TLS configuration {#tls-configuration}` या `## TLS configuration {/* #tls-configuration */}`। `translate-docs` के दौरान, केवल दृश्यमान शीर्षक टेक्स्ट मॉडल को भेजा जाता है — आईडी प्रत्यय को पहले हटा दिया जाता है और अनुवादित शीर्षक पंक्ति के **अंत** में वापस जोड़ दिया जाता है (Docusaurus उस `{/* #id */}` को अनदेखा कर देता है जो शीर्षक के मध्य में आ जाता है)।
2. उन शीर्षकों पर आईडी जोड़ने या रीफ्रेश करने के लिए अपने Docusaurus प्रोजेक्ट रूट (अक्सर `package.json` में जुड़े होने पर `pnpm run write-heading-ids`) से `docusaurus write-heading-ids` चलाएँ जिनमें ये नहीं हैं — `{/* #… */}` रूप के लिए `--syntax mdx-comment` का उपयोग करें। वैकल्पिक रूप से, उसी `docs[]` / `contentPaths` पर `ai-i18n-tools write-heading-ids --slug-style mdx-comment` चलाएँ। यह कमांड मौजूदा अनुवादित फ़ाइलों में उसी अंग्रेज़ी आईडी को पुनः स्थानांतरित भी करती है (यह अनुवादित शीर्षक को स्लग नहीं करती है) और जब सेगमेंट संख्या मेल खाती है तो मेल खाते हुए कैश्ड सेगमेंट को अपडेट करती है, ताकि बाद का `sync --force-update` सुधारी गई आईडी को बनाए रखे। शीर्षकों का नाम बदलने के बाद इसे पुनः चलाएँ ताकि पुरानी आईडी वर्तमान शीर्षकों से मेल खा सकें।
...
1. `translate-docs` से पहले अपने स्रोत `.md` / `.mdx` पर `ai-i18n-tools write-heading-ids` चलाएँ (हमेशा की तरह समान `docs[]` / `contentPaths`)। यह प्रत्येक शीर्षक से पहले वाली पंक्ति पर स्पष्ट HTML एंकर्स डालता है ताकि `id` मान प्रत्येक अनुवादित प्रति द्वारा साझा किए जाएँ, और यह उन समान अंग्रेज़ी आईडी को मौजूदा अनुवादित फ़ाइलों में कॉपी करता है। जब सेगमेंट संख्या मेल खाती है, तो मेल खाता हुआ कैश्ड सेगमेंट भी अपडेट हो जाता है, ताकि बाद का `sync --force-update` सुधारी गई आईडी को बनाए रखे। शीर्षकों का नाम बदलने के बाद इसे पुनः चलाएँ ताकि पुरानी एंकर आईडी रीफ्रेश होकर वर्तमान शीर्षक से मेल खा सकें।
2. अपने मार्कडाउन **एंकर लिंक्स** को उन स्थिर आईडी की ओर इंगित करें, उदाहरण के लिए `[label](other.md#section-id)`, जहाँ `section-id` उस एंकर से मेल खाता है जो टूल ने लिखा है — न कि केवल अंग्रेज़ी शब्दों से लगाया गया अनुमान।
