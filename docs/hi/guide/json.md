...
वैकल्पिक फ़्लैग (`translate-docs` जैसे ही विचार): लक्ष्यों के सबसेट के लिए `-l` / `--locale`, फ़ाइलों को सीमित करने के लिए `-p` / `--path`, `--dry-run`, `--force` (मिलान की गई फ़ाइलों के लिए फ़ाइल ट्रैकिंग और सेगमेंट कैश साफ़ करें), `--force-update` (फ़ाइल हैश मिलान होने पर फिर से प्रोसेस करें; सेगमेंट कैश अभी भी लागू होता है), `--check-cache` (देशी स्क्रिप्ट लागू करने वाले लोकेल के लिए कैश्ड सेगमेंट को फिर से मान्य करें, भले ही फ़ाइल ट्रैकिंग मेल खाती हो), `-b` / `--batch-concurrency`, `--prompt-format` (`xml` \| `json-array` \| `json-object`)।
