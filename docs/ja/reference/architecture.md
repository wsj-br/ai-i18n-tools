...
モデルが応答を返した後、`translate-docs` はマップを復元してセグメントを検証します。強調以外の二重中括弧トークンは保護されたソースと同じ順序付きサブシーケンスを維持する必要があり（`{{SE}}` のような強調マーカーは、タイプごとの数が一致する場合に語順に合わせて移動できます）、復元されたHTMLタグの種類は保護されていないソースと一致する必要があり、残存する二重中括弧の識別子はソースに既に存在していなければなりません（そのため、新規に作成されたトークンは失敗します）。ドキュメントのプロンプトでは、各トークンを1回コピーし、番号付きトークンの順序を維持し、新しい二重中括弧のラッパーを作成しないこともモデルに求めており、機械的なチェックが最終的な判断基準となります。
