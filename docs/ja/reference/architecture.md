...
有効な設定には、**少なくとも1つの`docs[]`ブロック**が必要です。各ブロックについて、`contentPaths`の下にある`.md` / `.mdx`ファイルを収集し、プロジェクトの`.translate-ignore`ルール（ドキュメント翻訳と同じ考え方）を適用し、オプションで`--path` / `--file`を使用してサブツリーに制限します。各ファイルは`applyHeadingAnchorsToMarkdown`で変換されます。フェンスされたコードブロック外のすべての**フラットなATX見出し**（`# …`から`###### …`まで）について、空のHTML行`<a id="slug"></a>`が欠落しているか古い場合に上の行に挿入されるか、または — `--slug-style mdx-comment`を使用して — DocusaurusのMDXサフィックス`{/* #slug */}`が見出し行に追加されます。スラグアルゴリズムは一般的なエコシステムに一致します — `github`（デフォルト）、`bitbucket`、`gitlab`、`pymdown`（オプションのUnicode正規化 / パーセントエンコーディングフラグ）、`azure-devops`、および`mdx-comment`（github slug + MDXコメント出力） — これにより、アンカーIDが既存のツール（doctoc、PyMdown、Docusaurusなど）と一貫性を保ちます。`--dry-run`は書き込まずに編集予定の内容を報告します。
