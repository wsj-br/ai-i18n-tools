...
**ドキュメント（SQLiteキャッシュ）** — キャッシュ内の手動編集にはモデル `user-edited` のタグが付けられます。ロケールの書記体系チェックに失敗した編集（例: `hi` のローマ字表記のヒンディー語）は警告を表示しますが、確認するとテキストがそのまま保存されます。変更されていないソースに対して `translate-docs` または `sync` を再実行すると、キャッシュされた翻訳が再利用されます（LLM呼び出しは発生しません）。`sync --force-update` または `translate-docs --force-update` を実行して、キャッシュからディスク上のMarkdownを更新します。`--check-cache` を使用すると、すべてのファイルを書き換えることなく（間違った書記体系で確定された編集を含む）、ネイティブスクリプトのロケールを再検証できます。`--force` は、キャッシュをバイパスしてLLMから再翻訳したい場合（手動修正を上書きする場合）にのみ使用してください。
