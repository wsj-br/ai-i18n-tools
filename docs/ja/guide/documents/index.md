...
復元後、`translate-docs`は、HTMLタグのプレースホルダーが再利用または破棄されたセグメント（その結果、復元されたタグがソースマップと一致しなくなる）、またはモデルがソースに存在しない二重中括弧トークンを捏造したセグメント（例えば、架空のglossaryスタイルのトークン）も拒否します。復元前のチェックでは、<code v-pre>{{…}}</code>トークンの同じマルチセットと、番号付きトークンの同じ順序付きサブシーケンス（<code v-pre>{{HTM_N}}</code>, <code v-pre>{{URL_N}}</code>, …）が必要です。<code v-pre>**</code>のような強調マーカーは、タイプごとの数が一致している限り、自然な語順に合わせて移動できます。これらの失敗は、残存する公式内部トークンと同じモデルフォールバックパスを使用します。
