...
リストア後、`translate-docs`は、HTMLタグのプレースホルダーが再利用または削除されたセグメント（その結果、復元されたタグがソースマップと一致しなくなる場合）、またはモデルがソースに存在しない二重中括弧トークンをでっち上げたセグメント（例えば、架空のglossaryスタイルのトークン）も拒否します。リストア前のチェックでは、`{{…}}`トークンの同じ多重集合と、番号付きトークン（`{{HTM_N}}`, `{{URL_N}}`, …）の同じ順序の部分列が要求されます。`{{SE}}`のような強調マーカーは、タイプごとのカウントが一致する限り、自然な語順に合わせて移動できます。これらの失敗は、残存する公式内部トークンと同じモデルフォールバックパスを使用します。
