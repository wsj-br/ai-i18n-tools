...
| Fumadocs `meta.json` 标签和 UI 覆盖目录 | 文档 — `translate-docs`（当 `style: "fumadocs"` 时自动 `meta.json`，可选 `fumadocsUiCatalog`）；**不要** 使用 `json[]` — 参见 [Fumadocs 集成](/guide/fumadocs-integration) |
