...
恢复后，`translate-docs` 也会拒绝重用或丢弃 HTML 标签占位符的片段（因此恢复后的标签不再与源映射匹配），或者拒绝模型虚构了源中不存在的多余双花括号标记的片段（例如虚构的术语表样式标记）。恢复前的检查要求具有相同的 `{{…}}` 标记多重集以及相同的编号标记有序子序列（`{{HTM_N}}`、`{{URL_N}}`、…）；诸如 `{{SE}}` 之类的强调标记在其每种类型的计数仍然匹配时，可以随自然语序移动。这些失败情况使用与剩余官方内部标记相同的模型回退路径。
