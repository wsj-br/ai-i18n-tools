...
恢复后，`translate-docs` 还会拒绝以下片段：HTML 标签占位符被重用或丢弃（导致恢复后的标签不再与源映射匹配），或者模型凭空生成了源中不存在的剩余双花括号标记（例如虚构的词汇表样式标记）。恢复前检查要求具有相同的 <code v-pre>{{…}}</code> 标记多重集，以及相同的带编号标记有序子序列（<code v-pre>{{HTM_N}}</code>、<code v-pre>{{URL_N}}</code>、……）；当各类型的计数仍然匹配时，诸如 <code v-pre>**</code> 之类的强调标记可以随自然语序移动。这些失败情况使用与剩余官方内部标记相同的模型回退路径。
