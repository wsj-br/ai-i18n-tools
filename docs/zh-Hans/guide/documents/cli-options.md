...
CLI 在 SQLite 中维护**文件跟踪**（每个文件 × 区域设置的源哈希）和**片段**行（每个可翻译块的哈希 × 区域设置）。正常运行时，当跟踪的哈希与当前源匹配、输出文件已存在**且**输出的修改时间不早于源的修改时间时，会完全跳过该文件——但对于具有预期书写系统的区域设置（`hi`、`ja`、`zh-Hans`、`ar`、…），始终会重新检查片段缓存，以便重试罗马化或错误书写系统的行。否则，它会处理该文件并使用片段缓存，使未更改的文本不调用 API。当存储的文本未通过书写系统验证时，片段缓存命中也会被拒绝。

| 标志                          | 效果                                                                                                                                                                                                                                                              |
|-------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| *(默认)*                   | 当跟踪记录与磁盘输出匹配时跳过未更改的文件（具有预期书写系统的区域设置除外，这些区域设置会重新检查片段缓存）；对其余文件使用片段缓存。错误书写系统的缓存命中会被拒绝并重新翻译。                                    |
...
| `--debug-failed`              | 全局。在 `cacheDir` 下为每次失败的模型尝试（错误书写系统、解析或质量问题）写入详细的 `FAILED-TRANSLATION` 日志，包括诸如罗马化 `zh-Hans`/`hi` 之类的回退情况——而不仅仅是在所有模型都失败时。也适用于 `sync` / `sync-ui` / `cleanup`。 |
