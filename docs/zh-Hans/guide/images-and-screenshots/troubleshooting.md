...
**`regexAdjustments` 在围栏配置示例内部的重写**
`postProcessing` 作用于完整的已翻译 markdown 正文，包括围栏代码块。如果文档页面嵌入了包含匹配路径的配置片段（例如 `screenshots/en-GB/`），该片段在翻译输出中也会被重写。在可复用示例中优先使用通用的 `screenshots/[^/]+/` 形式，或者接受翻译后的文档会在图示中显示特定于区域设置的路径。
