...
| `--debug-failed`             | 翻译命令 | 为每次被丢弃的模型尝试（脚本错误、解析、质量或 API 错误）在根目录 `cacheDir` 下写入 `FAILED-TRANSLATION` 日志，包括回退情况——而不仅仅是在所有模型都失败时。适用于 `translate-ui`、`translate-docs`、`translate-json`、`translate-svg`、`sync-ui`、`sync` 和 `cleanup`。 |
