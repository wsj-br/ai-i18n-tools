...
模型返回后，`translate-docs` 恢复映射并验证片段：非强调双花括号标记必须保持与受保护源相同的有序子序列（当各类型计数匹配时，诸如 `{{SE}}` 之类的强调标记可随语序移动），恢复的 HTML 标签种类必须与未受保护源匹配，且任何剩余的双花括号标识符必须已存在于源中（因此虚构的标记会失败）。文档提示还要求模型复制每个标记一次，保持编号标记的顺序，并且不要虚构新的双花括号包装器；机械检查仍然具有权威性。
