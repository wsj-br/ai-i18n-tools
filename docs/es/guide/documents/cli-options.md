...
La CLI mantiene el **seguimiento de archivos** en SQLite (hash de origen por archivo × configuración regional) y filas de **segmentos** (hash × configuración regional por fragmento traducible). Una ejecución normal omite un archivo por completo cuando el hash rastreado coincide con el origen actual, el archivo de salida ya existe **y** la hora de modificación de la salida es al menos tan reciente como la del origen, excepto para las configuraciones regionales con un sistema de escritura esperado (`hi`, `ja`, `zh-Hans`, `ar`, …), que siempre vuelven a verificar la caché de segmentos para que las filas romanizadas o con escritura incorrecta puedan reintentarse. De lo contrario, procesa el archivo y usa la caché de segmentos para que el texto sin cambios no llame a la API. Los aciertos de la caché de segmentos también se rechazan cuando el texto almacenado falla la validación del script.

| Indicador | Efecto |
|---|---|
| *(predeterminado)* | Omite los archivos sin cambios cuando el seguimiento + la salida en disco coinciden (excepto las configuraciones regionales con un sistema de escritura esperado, que vuelven a verificar la caché de segmentos); usa la caché de segmentos para el resto. Los aciertos de la caché con escritura incorrecta se rechazan y se vuelven a traducir. |
...
| `--debug-failed` | Global. Escribe registros detallados de `FAILED-TRANSLATION` en `cacheDir` para cada intento de modelo fallido (script, análisis o calidad incorrectos), incluidas las alternativas como `zh-Hans`/`hi` romanizado, no solo cuando todos los modelos fallan. También se aplica a `sync` / `sync-ui` / `cleanup`. |
