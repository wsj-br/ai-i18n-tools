...
Guías específicas de cada framework para integrar ai-i18n-tools en sitios de documentación y proyectos Astro. Cada integración utiliza la canalización [Documentos](/guide/documents/) (`translate-docs` / `sync`) para el contenido de la página; las cadenas de shell (navegación, barra lateral, tema) se manejan dentro de esa misma canalización cuando se indica, no a través de la canalización [JSON](/guide/json) separada.
...
| --- | --- | --- |
| Astro Starlight o Astro simple | Cadenas de UI `ui-starlight` / híbridas | [Astro](/guide/integrations/astro) |
...
Todas las integraciones de frameworks de documentación comparten el mismo modelo de bloque `docs[]` descrito en [Documentos](/guide/documents/). Establezca `docsOutput.style` para que coincida con su framework (`"docusaurus"`, `"vitepress"`, `"nextra"`, `"fumadocs"` o `"astro-starlight"`). Para el diseño de la carpeta de salida y el comportamiento de reescritura de enlaces, consulte [Diseños de salida](/guide/documents/output-layouts) y [Reescritura de enlaces](/guide/documents/link-rewriting).
