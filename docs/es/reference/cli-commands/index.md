...
| [`help`](setup#help) | Mostrar ayuda para un subcomando. |

<a id="models--languagesmodels"></a>
### [Modelos y lenguajes](models)
...
| [`migrate-intlayer`](ui-strings#migrate-intlayer) | Importar diccionarios de Intlayer `.content.ts` y reescribir sitios simples de `useIntlayer` / `getIntlayer` a `t()`. |
...
### [Flujos de trabajo e informes](workflows)
...
| [`usage`](workflows#usage) | Imprimir los tokens y costos registrados de las llamadas a la API del modelo. |

<a id="cache-maintenancemaintenance"></a>
### [Mantenimiento de caché](maintenance)
...
### [Panel de control y glosario](tools)

| Comando | Resumen |
|---------|---------|
| [`dashboard`](tools#dashboard) (`dash`) | Inicia la interfaz de usuario web del Panel de traducción. |
...
| `-L` / `--ui-lang <code>` | Cada comando | Idioma de la interfaz de usuario de la herramienta (ayuda de la CLI, registros/resúmenes, panel); fuente de mayor prioridad. Consulte [Idioma de la interfaz de usuario de la herramienta](/es/guide/tool-ui-language). |
...
| `--debug-failed` | Comandos de traducción | Escribe registros `FAILED-TRANSLATION` bajo la raíz `cacheDir` para cada verificación de traducción descartada (script, análisis o calidad incorrectos), incluyendo las alternativas, no solo cuando todos los modelos fallan. Los errores de API del proveedor o de cuerpo vacío se imprimen en la consola (una vez por modelo y mensaje de error) y no escriben esos archivos. Se aplica a `translate-ui`, `translate-docs`, `translate-json`, `translate-svg`, `sync-ui`, `sync` y `cleanup`. |
