...
Después de que el modelo regresa, `translate-docs` restaura los mapas y valida el segmento: los tokens de doble llave sin énfasis deben mantener la misma subsecuencia ordenada que el origen protegido (los marcadores de énfasis como `{{SE}}` pueden moverse con el orden de las palabras cuando las cuentas por tipo coinciden), los tipos de etiquetas HTML restauradas deben coincidir con el origen desprotegido, y cualquier identificador de doble llave restante ya debe haber existido en el origen (por lo que los tokens inventados fallan). La instrucción del documento también pide a los modelos que copien cada token una vez, mantengan el orden de los tokens numerados y no inventen nuevos envoltorios de doble llave; las comprobaciones mecánicas siguen siendo autorizadas.
