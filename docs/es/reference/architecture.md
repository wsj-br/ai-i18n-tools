...
- **Respaldo de modelos**: intenta cada modelo en la lista resuelta en orden; recurre a otros en caso de fallos de solicitud o análisis. Cada configuración regional de destino obtiene su propia cadena resuelta: `localeModels(locale)` primero cuando está configurado, luego `uiModels` (solo canalizaciones de interfaz de usuario), luego `translationModels`. La traducción de documentos, JSON y SVG crea un cliente por configuración regional con la cadena que no es de interfaz de usuario. El comando `bench-models` en su lugar crea un único cliente de un solo modelo por ID configurado (unión de `translationModels`, `uiModels` y `localeModels`; `translationModels: [id]`, sin respaldo) para que pueda cronometrar y tasar cada modelo de forma independiente.
- **Tiempo de espera de solicitud**: el `requestTimeoutMs` del proveedor activo (30 segundos por defecto) aborta cada solicitud a través de `AbortSignal.timeout`. El mismo valor se aplica a `GET /models` cuando la CLI carga la lista de modelos de un proveedor para `check-models` (cualquier proveedor). El filtro opcional previo al vuelo que descarta ID de modelo desconocidos se ejecuta solo cuando el proveedor activo es OpenRouter.
...
- **Registro de tráfico de depuración**: si `debugTrafficFilePath` está configurado, añade JSON de solicitud y respuesta a un archivo (programático). La CLI `--debug-failed` escribe archivos `FAILED-TRANSLATION` en `cacheDir` con el mensaje del sistema/usuario, la respuesta bruta del asistente y los errores de validación para los intentos fallidos de verificación de traducción de UI, documentos, JSON y SVG. Los fallos de la API del proveedor / de cuerpo vacío se imprimen en la consola en lugar de volcar un archivo solo con el mensaje.
