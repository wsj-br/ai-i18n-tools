...
| | `src/glossary/` | Carga de glosario, sugerencias de términos y archivos de contexto de proyecto opcionales para las indicaciones |
...
Requiere una configuración válida con **al menos un bloque `docs[]`**. Para cada bloque, recopila archivos `.md` / `.mdx` en `contentPaths`, aplica las reglas `.translate-ignore` del proyecto (la misma idea que la traducción de documentos) y, opcionalmente, restringe a un subárbol con `--path` / `--file`. Cada archivo se transforma con `applyHeadingAnchorsToMarkdown`: para cada **encabezado ATX plano** (`# …` a `###### …`) fuera de los bloques de código cercados, los ID de encabezado existentes de cualquier forma se reemplazan con el estilo seleccionado. Los estilos HTML escriben `<a id="slug"></a>` en la línea anterior a un encabezado sin sufijo; `--slug-style mdx-comment` escribe `{/* #slug */}` en la línea del encabezado (y elimina un ancla HTML precedente). Los slugs siempre provienen del texto del encabezado actual. `--remove` elimina los anclajes HTML, los sufijos clásicos de `{#id}` y los ID de comentarios MDX sin escribir nuevos. Los algoritmos de slug coinciden con los ecosistemas comunes —`github` (predeterminado), `bitbucket`, `gitlab`, `pymdown` (normalización Unicode opcional / banderas de codificación de porcentaje), `azure-devops`, más `mdx-comment` (slug de github + salida de comentarios MDX)— para que los ID de anclaje se mantengan consistentes con las herramientas existentes (doctoc, PyMdown, Docusaurus, etc.). `--dry-run` informa las ediciones que se realizarían sin escribirlas.

Después del pase de origen, el mismo comando reposiciona esos identificadores en inglés en el markdown traducido existente de cada configuración regional (`resolveTranslatedOutputPath`). Los títulos traducidos nunca se vuelven a slugificar; un `{#id}` / `{/* #id */}` a mitad de encabezado se mueve de nuevo al sufijo (o ancla HTML) que espera el estilo seleccionado. Los archivos de configuración regional que faltan se omiten.
...
1. **Sufijos de ID de encabezado** (clásico `{#id}` / MDX `{/* #id */}` al final de un encabezado ATX) — se eliminan completamente de la línea y **no** se envían al modelo. Después de la restauración, se vuelven a fijar al final del encabezado coincidente para que Docusaurus siga viendo un ID válido. Los comentarios a mitad de encabezado permanecen en el texto y son manejados por las capas posteriores de MDX / `{#…}`.
2. **Etiquetas y comentarios HTML** (`<strong>`, `<!-- ... -->`, etc.) - las etiquetas HTML en minúsculas de una lista de permitidos conocida se reemplazan con tokens ```{{HTM_N}}```. Las etiquetas JSX en mayúsculas (`<Highlight>`, `<Tabs>`, `</Tab>`) son manejadas por separado por la capa MDX (paso 5).
3. **Marcadores de advertencia** (`:::note`, `:::`) - solo el prefijo de la directiva en la línea de apertura se reemplaza con ```{{ADM_OPEN_N}}```; cualquier título en la misma línea se deja para que el modelo lo traduzca. Se restaura con el texto original exacto.
4. **Anclas de documentos** (HTML `<a id="…">`, `{#…}` de Docusaurus sobrante a mitad de encabezado) - se conservan textualmente.
5. **Constructos solo de MDX** (`src/processors/mdx-placeholders.ts`):
   - **Comentarios de MDX** (`{/* … */}`, incluyendo un `{/* #my-id */}` a mitad de encabezado que no era un sufijo de fin de línea) reemplazados con ```{{MDX_N}}```.
   - **Etiquetas JSX en mayúsculas** (`<Highlight>`, `<Tabs>`, `<TabItem>`, `<TOCInline />`, `</Highlight>`) - se conservan como ```{{MDX_N}}``` con atributos de cadena traducibles (`label`, `tooltip`, `aria-label`) reescritos a ```{{JXA_N}}``` dentro de la etiqueta a menos que el nombre del atributo aparezca en `docs[].protectAttributes`; `label:` dentro de literales de objeto `<Tabs values={[ { label: '…' } ]}>` (omitibles a través de `docs[].protectKeys`) y `<TabItem value="…">` (cuando no existe el atributo `label`, omitiendo valores en minúsculas similares a slugs) también se extraen. Se añaden al segmento como líneas `||JXA_N: …||`, fusionadas de nuevo por `restoreMdx`.
   - **Expresiones de llave MDX** (`{frontMatter.title}`, <code v-pre>style={{…}}</code>) - coincidencia consciente de la profundidad, reemplazadas con ```{{MDX_N}}```.
6. **URL de Markdown** (`](url)`, `src="…"`) - restauradas de un mapa después de la traducción.
7. **Spans de código en línea** (`` `code` ``) y **código en línea envuelto en negrita** (`**`code`**`) - conservados.
8. **Énfasis de Markdown** (opcional, habilitado automáticamente para configuraciones regionales CJK/RTL) - delimitadores de énfasis enmascarados.
...
Las llamadas al modelo facturadas (traducciones aceptadas y reintentos descartados) se almacenan en `api_calls`. Después de un comando que haya registrado llamadas, las filas de detalle con más de siete días naturales UTC pasan a las `api_totals` mensuales. `usage` y la vista de Uso y costos del panel combinan ambas tablas. Consulte [Uso y costos](/es/guide/translation-dashboard/usage).

Las notas del glosario `Context` y `glossary.contextFiles` reciben una huella digital (`prompt_context_hash`) y se inyectan en la IU, la documentación, JSON, SVG y los prompts de revisión. Modificar esa directriz invalida los segmentos en caché coincidentes y las filas de seguimiento de archivos en la siguiente ejecución. Las filas de la caché de `user-edited` del panel se conservan. Cambiar solo una traducción preferida del glosario mantiene la caché existente intacta hasta `--force` / `--force-update`.

El comando `translate-docs` también utiliza el **seguimiento de archivos** para que las fuentes sin cambios con salidas existentes y actualizadas puedan omitir el trabajo por completo. `--check-cache` vuelve a abrir las configuraciones regionales con un sistema de escritura esperado para que los segmentos en caché se vuelvan a validar; `--force-update` vuelve a ejecutar el procesamiento de archivos para cada configuración regional mientras sigue utilizando la caché de segmentos; `--force` borra el seguimiento de archivos y omite las lecturas de la caché de segmentos para la traducción de la API. Cuando cada modelo configurado falla la validación AST en un segmento de markdown, `translate-docs` puede dividir progresivamente el segmento y reintentar partes más pequeñas (`docs[].segmentSplitting.qualityRetrySplit`, activado por defecto). Consulte [Documentos — comportamiento y banderas de la caché](/es/guide/documents/cli-options#cache-behaviour-and-translate-docs-flags) para ver la tabla completa de banderas.
...
- **Respaldo del modelo**: intenta cada modelo en la lista resuelta en orden; recurre a solicitudes o fallos de análisis. Cada configuración regional de destino obtiene su propia cadena resuelta: `localeModels(locale)` primero cuando está configurado, luego `uiModels` (solo canalizaciones de interfaz de usuario), luego `translationModels`. La traducción de documentos, JSON y SVG crea un cliente por configuración regional con la cadena que no es de interfaz de usuario. El comando `bench-models` en su lugar crea un único cliente de un solo modelo por ID configurado (unión de `translationModels`, `uiModels` y `localeModels`; `translationModels: [id]`, sin respaldo) para que pueda cronometrar y fijar el precio de cada modelo de forma independiente.
- **Tiempo de espera de la solicitud**: `requestTimeout` (segundos) o `requestTimeoutMs` en el proveedor activo, de lo contrario las mismas claves en la parte superior de la configuración (predeterminado 45 segundos), aborta cada solicitud a través de `AbortSignal.timeout`. El mismo valor se aplica a `GET /models` cuando la CLI carga una lista de modelos de un proveedor para `check-models` (cualquier proveedor). El filtro opcional previo al vuelo que elimina los ID de modelo desconocidos solo se ejecuta cuando el proveedor activo es OpenRouter.
- **Extras de OpenRouter** (solo cuando `openrouter` está activo): enrutamiento de rendimiento a través del campo de solicitud `provider`, encabezados `HTTP-Referer` / `X-Title`, y costo exacto en USD leído de `usage.cost`. El uso de tokens se informa para cada proveedor. Cuando el proveedor omite `usage.cost`, el costo en USD se calcula a partir de `providers.<name>.modelPricing` o el valor predeterminado `pricing` de todo el proveedor y se almacena en la fila `api_calls`. Un costo informado por el proveedor nunca se reemplaza.
- **Registro de tráfico de depuración**: si se establece `debugTrafficFilePath`, anexa el JSON de solicitud y respuesta a un archivo (programático). La CLI `--debug-failed` escribe archivos `FAILED-TRANSLATION` en `cacheDir` con la indicación del sistema/usuario, la respuesta bruta del asistente y los errores de validación para los intentos fallidos de verificación de traducción de UI, documentos, JSON y SVG. Los fallos de la API del proveedor / de cuerpo vacío se imprimen en la consola en lugar de volcar un archivo solo con la indicación.
