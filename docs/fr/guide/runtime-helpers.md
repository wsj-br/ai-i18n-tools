...
Créez `localeLoaders` avec `makeLocaleLoadersFromManifest(uiLanguages, sourceLocale, …)` afin que les clés restent alignées avec `targetLocales` après `generate-ui-languages`. Voir `docs/guide/ui-strings/i18next-runtime.md` (câblage d'exécution), `examples/nextjs-app/`, `examples/console-app/` et `examples/astro-website/` (`makeT` personnalisé sans i18next).
