...
Après la restauration, `translate-docs` rejette également les segments où les espaces réservés de balises HTML ont été réutilisés ou supprimés (de sorte que les balises restaurées ne correspondent plus à la carte source) ou où le modèle a inventé des jetons à double accolade restants qui n'étaient pas dans la source (par exemple, un jeton de style glossaire inventé). Les vérifications avant la restauration exigent le même multiensemble de jetons `{{…}}` et la même sous-séquence ordonnée de jetons numérotés (`{{HTM_N}}`, `{{URL_N}}`, …) ; les marqueurs d'emphase tels que `{{SE}}` peuvent se déplacer avec l'ordre naturel des mots lorsque leurs comptes par type correspondent toujours. Ces échecs utilisent le même chemin de repli du modèle que les jetons internes officiels restants.
