...
Après la restauration, `translate-docs` rejette également les segments où les espaces réservés des balises HTML ont été réutilisés ou supprimés (de sorte que les balises restaurées ne correspondent plus à la carte source) ou où le modèle a inventé des jetons à double accolade restants qui n'étaient pas dans la source (par exemple, un jeton de style glossaire inventé). Les vérifications avant la restauration exigent le même multiensemble de jetons <code v-pre>{{…}}</code> et la même sous-séquence ordonnée de jetons structurels (<code v-pre>{{HTM_N}}</code>, marqueurs d'avertissement) ; les jetons de contenu tels que <code v-pre>{{ILC_N}}</code>, <code v-pre>{{URL_N}}</code> et les marqueurs d'emphase comme <code v-pre>{{SE}}</code> peuvent se déplacer avec l'ordre naturel des mots lorsque chaque identifiant/type correspond toujours. Ces échecs utilisent le même chemin de secours du modèle que les jetons internes officiels restants.
