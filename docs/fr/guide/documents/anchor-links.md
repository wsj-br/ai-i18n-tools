...
1. Ajoutez un identifiant explicite sur la ligne d'en-tête avec le suffixe classique de Docusaurus `{#…}` (CommonMark) ou le commentaire MDX `{/* #… */}` (préféré pour `.mdx`), par exemple `## TLS configuration {#tls-configuration}` ou `## TLS configuration {/* #tls-configuration */}`. Pendant `translate-docs`, seul le texte visible de l'en-tête est envoyé au modèle — le suffixe d'identifiant est d'abord supprimé puis réinséré à la **fin** de la ligne d'en-tête traduite (Docusaurus ignore un `{/* #id */}` qui se trouve au milieu du titre).
2. Exécutez `docusaurus write-heading-ids` depuis la racine de votre projet Docusaurus (souvent `pnpm run write-heading-ids` lorsqu'il est configuré dans `package.json`) pour ajouter ou rafraîchir les identifiants sur les en-têtes qui n'en ont pas — utilisez `--syntax mdx-comment` pour la forme `{/* #… */}`. Vous pouvez également exécuter `ai-i18n-tools write-heading-ids --slug-style mdx-comment` sur le même `docs[]` / `contentPaths`. Cette commande repositionne également les mêmes identifiants anglais dans les fichiers traduits existants (elle ne slugifie pas le titre traduit). Réexécutez après avoir renommé les en-têtes afin que les identifiants obsolètes correspondent aux titres actuels.
...
1. Exécutez `ai-i18n-tools write-heading-ids` sur votre `.md` / `.mdx` source avant `translate-docs` (même `docs[]` / `contentPaths` que d'habitude). Il insère des ancres HTML explicites sur la ligne précédant chaque en-tête afin que les valeurs `id` soient partagées par chaque copie traduite, et il copie ces mêmes identifiants anglais dans les fichiers traduits existants. Réexécutez-le après avoir renommé les en-têtes afin que les identifiants d'ancrage obsolètes soient mis à jour pour correspondre au titre actuel.
2. Pointez vos **liens d'ancrage** markdown vers ces identifiants stables, par exemple `[label](other.md#section-id)`, où `section-id` correspond à l'ancre écrite par l'outil — et non à une supposition basée uniquement sur des mots anglais.
