...
Vide l'intégralité de la table `markdown_source_issues`, puis exécute `sync --force-update` (extraction, UI, SVG, documents et `translate-json` si activé) afin que les problèmes de markdown soient repeuplés pour les documents actuellement configurés ; puis supprime les lignes de segment obsolètes (`last_hit_at` nul/chemin de fichier vide) ; supprime les lignes `file_tracking` dont le chemin source résolu est manquant sur le disque ; supprime les lignes de traduction dont les métadonnées `filepath` pointent vers un fichier manquant ; élague les lignes `translation_failures` orphelines ; et supprime les lignes de cache pour les locales absentes de la configuration (`sourceLocale`, racine `targetLocales` et tout `docs[]`/`json[]` `targetLocales` par bloc). Cache uniquement pour les locales retirées : les documents générés, les fichiers d'interface utilisateur plats et les entrées `strings.json` sont laissés intacts (utilisez [`purge-locale`](#purge-locale) pour les supprimer). Enregistre les nombres d'élagage après la synchronisation (segments obsolètes, `file_tracking` orphelins, traductions orphelines, échecs orphelins, locales non configurées) ainsi que le nombre initial de problèmes de markdown effacés.
