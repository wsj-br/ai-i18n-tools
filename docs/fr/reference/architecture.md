...
Il nécessite une configuration valide avec **au moins un bloc `docs[]`**. Pour chaque bloc, il rassemble les fichiers `.md` / `.mdx` sous `contentPaths`, applique les règles `.translate-ignore` du projet (même idée que la traduction de documents), et se limite éventuellement à une sous-arborescence avec `--path` / `--file`. Chaque fichier est transformé avec `applyHeadingAnchorsToMarkdown` : pour chaque **titre ATX plat** (`# …` à `###### …`) en dehors des blocs de code clôturés, soit une ligne HTML vide `<a id="slug"></a>` est insérée sur la ligne supérieure lorsqu'elle est manquante ou obsolète, soit — avec `--slug-style mdx-comment` — un suffixe MDX Docusaurus `{/* #slug */}` est ajouté sur la ligne du titre. Les algorithmes de slug correspondent aux écosystèmes courants — `github` (par défaut), `bitbucket`, `gitlab`, `pymdown` (indicateurs optionnels de normalisation Unicode / encodage en pourcentage), `azure-devops`, plus `mdx-comment` (slug github + sortie de commentaire MDX) — afin que les ID d'ancrage restent cohérents avec les outils existants (doctoc, PyMdown, Docusaurus, etc.). `--dry-run` signale les modifications potentielles sans les écrire.
