...
- **Journal de trafic de débogage** : si `debugTrafficFilePath` est défini, ajoute le JSON de la requête et de la réponse à un fichier (programmatique). La CLI `--debug-failed` écrit des fichiers `FAILED-TRANSLATION` sous `cacheDir` avec l’invite système/utilisateur, la réponse brute de l’assistant et les erreurs de validation pour les tentatives échouées de vérification de traduction d’interface utilisateur, de documents, de JSON et de SVG. Les échecs d’API de fournisseur/corps vide s’affichent sur la console au lieu de vider un fichier d’invite uniquement.
