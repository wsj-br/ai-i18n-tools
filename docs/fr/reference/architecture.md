...
Une fois le modèle renvoyé, `translate-docs` restaure les mappages et valide le segment : les jetons à double accolade sans emphase doivent conserver la même sous-séquence ordonnée que la source protégée (les marqueurs d'emphase tels que <code v-pre>{{SE}}</code> peuvent se déplacer avec l'ordre des mots lorsque les nombres par type correspondent), les types de balises HTML restaurés doivent correspondre à la source non protégée, et tout identifiant à double accolade restant doit déjà avoir existé dans la source (ainsi, les jetons inventés échouent). L'invite du document demande également aux modèles de copier chaque jeton une fois, de conserver l'ordre des jetons numérotés et de ne pas inventer de nouveaux wrappers à double accolade ; les vérifications mécaniques restent fiables.
