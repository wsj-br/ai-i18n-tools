...
| | `src/glossary/` | Chargement du glossaire, suggestions de termes et fichiers de contexte de projet facultatifs pour les invites |
| | `src/utils/` | Logger, hachage, analyseur d'ignorance, tables de largeur d'affichage, chargeur `.env` |
| **Runtime de votre application** | `src/runtime/` | Assistants i18next et utilitaires d'affichage — exportés sous le nom de `'ai-i18n-tools/runtime'` ([Assistants d'exécution](/fr/guide/runtime-helpers)) |
...
Tout ce qui est destiné à un usage programmatique est réexporté depuis `src/index.ts` ([API programmatique](/fr/reference/programmatic-api)).
...
Les sites SSG Astro simples peuvent ignorer i18next : chargez des `{locale}.json` plats au moment de la construction et résolvez les `t('English')` par clé de texte source (voir `examples/astro-website/src/i18n/t.ts` et [Chaînes d’interface utilisateur — site web Astro](/fr/guide/ui-strings/astro-website#astro-website-plain-astro-not-starlight)).

Les applications HTML simples suivent le même modèle de catalogue avec des attributs de marqueur au lieu d’appels `t()` — voir [Marquage HTML pour la traduction](/fr/guide/ui-strings/plain-html#marking-html-for-translation).
...
- Identifier les langues source et cible (par le nom d'affichage de `localeDisplayNames` ou `ui-languages.json`).
- Préfixer une directive de script lorsque la locale cible a un système d'écriture attendu (script BCP-47 explicite, ou une langue par défaut telle que `hi` → Devanagari, `ar` → Arabe, `ja` → Kana/Kanji japonais).
- Envoyer un tableau JSON de chaînes et demander un tableau JSON de traductions en retour.
- Inclure des indications de glossaire si disponibles.

`LlmClient.translateUIBatch` essaie chaque modèle dans l'ordre, en se rabattant sur les erreurs d'analyse, de réseau ou de script incorrect (y compris le repli romanisé/latin pour les locales à script natif). La CLI construit cette liste par locale cible à partir de `localeModels`, de `uiModels` facultatif et de `translationModels` (voir [Fournisseurs et modèles](/fr/guide/providers-and-models#model-fallback-chain)).
...
Il nécessite une configuration valide avec **au moins un bloc `docs[]`**. Pour chaque bloc, il rassemble les fichiers `.md` / `.mdx` sous `contentPaths`, applique les règles `.translate-ignore` du projet (même idée que la traduction de documentation), et se restreint éventuellement à une sous-arborescence avec `--path` / `--file`. Chaque fichier est transformé avec `applyHeadingAnchorsToMarkdown` : pour chaque **titre ATX plat** (`# …` à `###### …`) en dehors des blocs de code clôturés, les identifiants de titre existants de toute forme sont remplacés par le style sélectionné. Les styles HTML écrivent `<a id="slug"></a>` sur la ligne au-dessus d'un titre sans suffixe ; `--slug-style mdx-comment` écrit `{/* #slug */}` sur la ligne du titre (et supprime une ancre HTML précédente). Les slugs proviennent toujours du texte du titre actuel. `--remove` supprime les ancres HTML, les suffixes classiques `{#id}` et les identifiants de commentaire MDX sans en écrire de nouveaux. Les algorithmes de slug correspondent aux écosystèmes courants — `github` (par défaut), `bitbucket`, `gitlab`, `pymdown` (normalisation Unicode facultative / drapeaux d'encodage en pourcentage), `azure-devops`, plus `mdx-comment` (slug github + sortie de commentaire MDX) — afin que les identifiants d'ancre restent cohérents avec les outils existants (doctoc, PyMdown, Docusaurus, etc.). `--dry-run` signale les modifications potentielles sans les écrire.

Après le passage source, la même commande repositionne ces identifiants anglais sur le markdown traduit existant de chaque locale (`resolveTranslatedOutputPath`). Les titres traduits ne sont jamais re-slugifiés ; un `{#id}` / `{/* #id */}` en milieu de titre est replacé à la fin (ou ancre HTML) que le style sélectionné attend. Les fichiers de locale manquants sont ignorés.
...
1. **Suffixes d'ID d'en-tête** (`{#id}` classique / `{/* #id */}` MDX à la fin d'un en-tête ATX) — entièrement retirés de la ligne et **non** envoyés au modèle. Après restauration, ils sont rattachés à la fin de l'en-tête correspondant afin que Docusaurus voie toujours un ID valide. Les commentaires en milieu d'en-tête restent dans le texte et sont gérés par les couches MDX / `{#…}` ultérieures.
2. **Balises et commentaires HTML** (`<strong>`, `<!-- ... -->`, etc.) - les balises HTML en minuscules d'une liste blanche connue sont remplacées par des jetons ```{{HTM_N}}```. Les balises JSX capitalisées (`<Highlight>`, `<Tabs>`, `</Tab>`) sont gérées séparément par la couche MDX (étape 5).
3. **Marqueurs d'admonition** (`:::note`, `:::`) - seul le préfixe de la directive sur la ligne d'ouverture est remplacé par ```{{ADM_OPEN_N}}``` ; tout titre sur la même ligne est laissé au modèle pour traduction. Restauré avec le texte original exact.
4. **Ancres de document** (`<a id="…">` HTML, `{#…}` Docusaurus résiduel en milieu d'en-tête) - conservées telles quelles.
5. **Constructions spécifiques à MDX** (`src/processors/mdx-placeholders.ts`) :
   - **Commentaires MDX** (`{/* … */}`, y compris un `{/* #my-id */}` en milieu d'en-tête qui n'était pas un suffixe de fin de ligne) remplacés par ```{{MDX_N}}```.
   - **Balises JSX capitalisées** (`<Highlight>`, `<Tabs>`, `<TabItem>`, `<TOCInline />`, `</Highlight>`) - conservées sous forme de ```{{MDX_N}}``` avec des attributs de chaîne traduisibles (`label`, `tooltip`, `aria-label`) réécrits en ```{{JXA_N}}``` à l'intérieur de la balise, sauf si le nom de l'attribut apparaît dans `docs[].protectAttributes` ; `label:` à l'intérieur des littéraux d'objet `<Tabs values={[ { label: '…' } ]}>` (sautables via `docs[].protectKeys`) et `<TabItem value="…">` (lorsqu'aucun attribut `label` n'existe, sautant les valeurs de type slug en minuscules) sont également extraits. Ajoutés au segment sous forme de lignes `||JXA_N: …||`, fusionnés par `restoreMdx`.
...
6. **URL Markdown** (`](url)`, `src="…"`) - restaurées à partir d'une carte après traduction.
7. **Portées de code en ligne** (`` `code` ``) et **code en ligne encadré en gras** (`**`code`**`) - conservées.
8. **Mise en évidence Markdown** (optionnel, activé automatiquement pour les locales CJK/RTL) - délimiteurs de mise en évidence masqués.

Une fois que le modèle est renvoyé, `translate-docs` restaure les mappages et valide le segment : le même multiensemble de jetons à double accolade doit être présent, les jetons structurels (<code v-pre>{{HTM_N}}</code>, les marqueurs d'avertissement) doivent conserver leur sous-séquence ordonnée (les jetons de contenu tels que <code v-pre>{{ILC_N}}</code> / <code v-pre>{{URL_N}}</code> / <code v-pre>{{SE}}</code> peuvent se déplacer avec l'ordre des mots), les types de balises HTML restaurés doivent correspondre à la source non protégée, et tout identifiant à double accolade restant doit déjà avoir existé dans la source (donc les jetons inventés échouent). L'invite du document demande également aux modèles de copier chaque jeton une fois, de conserver l'ordre des jetons structurels et de ne pas inventer de nouveaux wrappers à double accolade ; les vérifications mécaniques restent fiables.

La protection partagée des attributs/clés pour les modèles Astro et MDX JSX est implémentée dans `src/processors/expression-attribute-protection.ts` et pilotée par bloc par `docs[].protectAttributes` et `docs[].protectKeys` (voir [protectAttributes / protectKeys](/fr/reference/configuration#protectattributes-protectkeys)).
...
À chaque exécution, les segments sont recherchés par hachage × locale. Seules les erreurs de cache sont transmises au LLM. Après la traduction, `last_hit_at` est réinitialisé pour les lignes de segment dans la portée de traduction actuelle qui n'ont pas été atteintes. Les succès de cache lors de la traduction de documents effacent les lignes `translation_failures` obsolètes pour ce segment. `cleanup` exécute `sync --force-update` en premier, puis supprime les lignes de segment obsolètes (`last_hit_at` nul / chemin de fichier vide), élague les clés `file_tracking` lorsque le chemin source résolu est manquant sur le disque (`doc-block:…`, `json-block:…`, `svg-files:…`, etc.), supprime les lignes de traduction dont le chemin de fichier de métadonnées pointe vers un fichier manquant, élague les lignes `translation_failures` orphelines, élague les lignes `markdown_source_issues` orphelines dont le chemin source résolu est manquant sur le disque, et supprime les lignes de cache pour les locales absentes de la configuration (`sourceLocale`, racine `targetLocales`, et tout `docs[]` / `json[]` `targetLocales` par bloc ; SQLite uniquement — utilisez `purge-locale` pour supprimer les fichiers générés) ; il ne sauvegarde pas `cache.db` à moins que `--backup <path>` ne soit passé, ce qui écrit d'abord une sauvegarde à ce chemin.

Les appels au modèle facturés (traductions acceptées et tentatives ignorées) sont stockés dans `api_calls`. Après une commande ayant enregistré des appels, les lignes de détail datant de plus de sept jours calendaires UTC sont transférées vers les `api_totals`. `usage` et la vue Utilisation et coûts du tableau de bord combinent ces deux tables. Voir [Utilisation et coûts](/fr/guide/translation-dashboard/usage).

Les notes de glossaire `Context` et les `glossary.contextFiles` sont identifiés par une empreinte (`prompt_context_hash`) et injectés dans l'interface utilisateur, la documentation, les fichiers JSON et SVG, ainsi que dans les invites de relecture. La modification de ces directives invalide les segments mis en cache correspondants et les lignes de suivi des fichiers lors de l'exécution suivante. Les lignes de cache `user-edited` du tableau de bord sont conservées. La modification exclusive d'une traduction préférée du glossaire conserve le cache existant en l'état jusqu'à `--force` / `--force-update`.

La commande `translate-docs` utilise également le **suivi de fichiers** afin que les sources inchangées avec des sorties existantes et à jour puissent ignorer complètement le travail. `--check-cache` rouvre les paramètres régionaux avec un système d'écriture attendu afin que les segments mis en cache soient revalidés ; `--force-update` réexécute le traitement des fichiers pour chaque paramètre régional tout en utilisant le cache de segments ; `--force` efface le suivi des fichiers et contourne les lectures du cache de segments pour la traduction d'API. Lorsque chaque modèle configuré échoue à la validation AST sur un segment de démarque, `translate-docs` peut diviser progressivement le segment et réessayer des parties plus petites (`docs[].segmentSplitting.qualityRetrySplit`, activé par défaut). Consultez [Documents — comportement et indicateurs du cache](/fr/guide/documents/cli-options#cache-behaviour-and-translate-docs-flags) pour le tableau complet des indicateurs.

**Format d'invite de lot :** `translate-docs --prompt-format` sélectionne les formes XML (`<seg>` / `<t>`) ou tableau/objet JSON pour `LlmClient.translateDocumentBatch` uniquement ; l'extraction, les espaces réservés et la validation restent inchangés. Voir [Format d'invite de lot](/fr/guide/documents/cli-options#batch-prompt-format).
...
Lorsque `docsOutput.style === "flat"`, les fichiers markdown traduits sont placés à côté de la source avec des suffixes de paramètres régionaux. Les liens relatifs entre les pages sont réécrits de sorte que `[Guide](./guide.md)` dans `readme.de.md` pointe vers `guide.de.md`. Contrôlé par `rewriteRelativeLinks` (activé automatiquement pour le style plat sans `pathTemplate` personnalisé). Le même passage ajoute un préfixe de profondeur par fichier aux URL d’actifs non-markdown avant l’exécution de `postProcessing.regexAdjustments` — voir [Réécriture de liens plats](/fr/guide/images-and-screenshots/link-rewriting#the-flat-link-rewriter-and-two-step-flow).
...
- **Modèle de secours** : essaie chaque modèle de la liste résolue dans l'ordre ; revient en arrière en cas d'échec de requête ou d'analyse. Chaque locale cible obtient sa propre chaîne résolue : `localeModels(locale)` en premier lorsqu'il est configuré, puis `uiModels` (pipelines d'interface utilisateur uniquement), puis `translationModels`. La traduction de documents, JSON et SVG crée un client par locale avec la chaîne non-UI. La commande `bench-models` construit plutôt un client à modèle unique par ID configuré (union de `translationModels`, `uiModels` et `localeModels` ; `translationModels: [id]`, pas de secours) afin qu'il puisse chronométrer et évaluer chaque modèle indépendamment.
- **Délai d'expiration de la requête** : `requestTimeout` (secondes) ou `requestTimeoutMs` sur le fournisseur actif, sinon les mêmes clés en haut de la configuration (par défaut 45 secondes), annule chaque requête via `AbortSignal.timeout`. La même valeur s'applique à `GET /models` lorsque la CLI charge la liste de modèles d'un fournisseur pour `check-models` (tout fournisseur). Le filtre de pré-vol facultatif qui supprime les ID de modèle inconnus ne s'exécute que lorsque le fournisseur actif est OpenRouter.
- **Extras OpenRouter** (uniquement lorsque `openrouter` est actif) : routage du débit via le champ de requête `provider`, les en-têtes `HTTP-Referer` / `X-Title` et le coût USD exact lu à partir de `usage.cost`. L'utilisation des jetons est signalée pour chaque fournisseur. Lorsque le fournisseur omet `usage.cost`, le coût USD est calculé à partir de `providers.<name>.modelPricing` ou de la valeur par défaut `pricing` à l'échelle du fournisseur et stocké sur la ligne `api_calls`. Un coût signalé par le fournisseur n'est jamais remplacé.
- **Journal de trafic de débogage** : si `debugTrafficFilePath` est défini, ajoute le JSON de la requête et de la réponse à un fichier (programmatique). La CLI `--debug-failed` écrit `FAILED-TRANSLATION` fichiers sous `cacheDir` avec l'invite système/utilisateur, la réponse brute de l'assistant et les erreurs de validation pour les tentatives d'échec de traduction/vérification d'interface utilisateur, de documents, de JSON et de SVG. Les échecs d'API de fournisseur/corps vide s'affichent sur la console au lieu de vider un fichier d'invite uniquement.
