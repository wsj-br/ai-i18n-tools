...
For `flat` style, keep source subdirectories so files with the same basename do not collide.
- `markdownOutput.rewriteRelativeLinks`
Rewrite relative links after translation (auto-enabled for `flat` style).
...
Print documentation cache and `strings.json` statistics (same aggregates as Translation Cache Editor → **Statistics**). `--max-columns`: max locale columns per model × locale table (default matches the editor).
...
Launch a local web editor for the cache, `strings.json`, and glossary CSV. With `--no-open`, the default browser is not opened automatically.  
**Note:** If you edit an entry in the cache editor, you must run a `sync --force-update` to rewrite the output files with the updated cache entry. Also, if the source text changes later, the manual edit will be lost since a new cache key is generated.
