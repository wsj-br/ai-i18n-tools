...
모델이 응답을 반환한 후, `translate-docs`는 맵을 복원하고 세그먼트를 검증합니다: 강조가 아닌 이중 중괄호 토큰은 보호된 소스와 동일한 순서의 하위 시퀀스를 유지해야 하며(유형별 개수가 일치할 때 `{{SE}}`과 같은 강조 마커는 어순에 따라 이동할 수 있음), 복원된 HTML 태그 종류는 보호되지 않은 소스와 일치해야 하고, 남은 이중 중괄호 식별자는 소스에 이미 존재했어야 합니다(따라서 새로 만든 토큰은 실패합니다). 문서 프롬프트는 또한 모델에게 각 토큰을 한 번씩 복사하고, 번호가 매겨진 토큰 순서를 유지하며, 새로운 이중 중괄호 래퍼를 만들지 않도록 요청합니다; 기계적 검사가 여전히 권위를 갖습니다.
