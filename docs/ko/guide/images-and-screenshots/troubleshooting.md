...
**펜스로 감싼 구성 예시 내부의 `regexAdjustments` 재작성**
`postProcessing`은(는) 펜스로 감싼 코드 블록을 포함하여 번역된 전체 마크다운 본문에서 실행됩니다. 문서 페이지에 일치하는 경로(예: `screenshots/en-GB/`)가 포함된 구성 스니펫이 삽입되어 있으면, 해당 스니펫도 번역된 출력에서 재작성됩니다. 재사용 가능한 예시에서는 일반적인 `screenshots/[^/]+/` 형식을 선호하거나, 번역된 문서의 그림 내부에 로케일별 경로가 표시됨을 인정하세요.
