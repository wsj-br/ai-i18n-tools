...
CLI는 SQLite에 **파일 추적**(파일당 소스 해시 × 로케일)과 **세그먼트** 행(번역 가능 청크당 해시 × 로케일)을 유지합니다. 정상 실행 시 추적된 해시가 현재 소스와 일치하고, 출력 파일이 이미 존재하며, **그리고** 출력의 수정 시간이 소스의 수정 시간보다 최소한 같거나 더 새로운 경우 파일 전체를 건너뜁니다 — 단, 예상되는 문자 체계를 가진 로케일(`hi`, `ja`, `zh-Hans`, `ar`, …)은 제외하며, 이 로케일은 로마자화되거나 잘못된 문자 체계의 행을 재시도할 수 있도록 항상 세그먼트 캐시를 재확인합니다. 그렇지 않은 경우 파일을 처리하고 변경되지 않은 텍스트가 API를 호출하지 않도록 세그먼트 캐시를 사용합니다. 저장된 텍스트가 문자 체계 검증에 실패할 경우 세그먼트 캐시 적중도 거부됩니다.

| 플래그                          | 효과                                                                                                                                                                                                                                                              |
|-------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| *(기본값)*                   | 추적 + 디스크상 출력이 일치할 때 변경되지 않은 파일을 건너뜁니다(예상되는 문자 체계를 가진 로케일은 제외하며, 해당 로케일은 세그먼트 캐시를 재확인함); 나머지에는 세그먼트 캐시를 사용합니다. 잘못된 문자 체계의 캐시 적중은 거부되고 재번역됩니다.                                    |
...
| `--debug-failed`              | 전역 설정. 각 실패한 모델 시도(잘못된 문자 체계, 구문 분석 또는 품질)에 대해 `cacheDir` 아래에 상세 `FAILED-TRANSLATION` 로그를 작성하며, 로마자화된 `zh-Hans`/`hi`와 같은 폴백을 포함합니다 — 모든 모델이 실패한 경우에만 국한되지 않습니다. `sync` / `sync-ui` / `cleanup`에도 적용됩니다. |
