...
CLI는 SQLite에 **파일 추적** 정보(파일 × 로케일별 소스 해시)와 **세그먼트** 행(번역 가능 청크당 해시 × 로케일)을 유지합니다. 일반 실행 시 추적된 해시가 현재 소스와 일치하고, 출력 파일이 이미 존재하며, **그리고** 출력 파일의 수정 시간이 소스 파일보다 최소한 같거나 더 최근인 경우 파일 전체를 건너뜁니다. 그렇지 않으면 파일을 처리하고 세그먼트 캐시를 사용하여 변경되지 않은 텍스트는 API를 호출하지 않습니다. 저장된 텍스트가 스크립트 검증에 실패하면 세그먼트 캐시 적중도 거부됩니다. 예상되는 문자 체계가 있는 로케일(`hi`, `ja`, `zh-Hans`, `ar`, …)에 대해 `--check-cache`를 사용하여 파일 수준 건너뛰기를 우회하면, `--force-update` 없이 로마자화되거나 잘못된 문자 체계의 캐시 행을 재시도할 수 있습니다.

| 플래그                          | 효과                                                                                                                                                                                                                                                              |
|-------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| *(기본값)*                   | 추적 + 디스크상 출력이 일치할 때 변경되지 않은 파일을 건너뛰고, 나머지에는 세그먼트 캐시를 사용합니다. 잘못된 문자 체계의 캐시 적중은 처리되는 파일에 대해서만 거부되고 재번역됩니다. |
...
| `--debug-failed`              | 전역. 잘못된 스크립트, 구문 분석 또는 품질 등 각 번역 검사 실패에 대해 `cacheDir` 아래에 상세한 `FAILED-TRANSLATION` 로그를 작성합니다. 여기에는 모든 모델이 실패한 경우뿐만 아니라 로마자 표기된 `zh-Hans`/`hi`와 같은 폴백도 포함됩니다. 제공자 API 오류는 파일 대신 콘솔에 출력됩니다. `sync` / `sync-ui` / `cleanup`에도 적용됩니다. |
| `--check-cache`               | 파일 추적이 건너뛰기를 수행하더라도 네이티브 문자 체계가 강제되는 로케일에 대해 캐시된 세그먼트를 재검증합니다. 예상 문자 체계가 없는 로케일은 여전히 건너뜁니다. 세그먼트 캐시는 계속 적용됩니다.                                                                               |
...
일부 모델은 다른 형식보다 특정 형식을 더 안정적으로 따르므로, 모델이 잘못된 배치나 일치하지 않는 세그먼트 ID를 자주 반환하는 경우 다른 모드를 시도해 보십시오. `json-array`는 모델이 일반적으로 잘 처리하는 흔하고 단순한 형식이기 때문에 기본값입니다.
