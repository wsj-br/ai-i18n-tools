...
사람이 읽을 수 있는 레이블이 포함된 **SVG 일러스트레이션 및 다이어그램**을 번역하도록 설계되었습니다. `translate-svg` 명령은 소스 `.svg` 파일을 읽고, `<text>`, `<title>` 및 `<desc>` 요소에서 텍스트를 추출하고, 활성 LLM 제공자를 통해 해당 문자열을 번역한 다음, **대상 로케일당 하나의 출력 SVG**를 생성합니다.
