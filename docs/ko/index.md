...
미리 알아두면 좋은 몇 가지 사항: UI 문자열은 활성 LLM 공급자를 통해 로케일별 누락된 항목을 번역하고( [LLM 공급자](#llm-providers) 참조) 플랫 JSON 파일(`de.json`, `pt-BR.json`, …)을 작성하며, 영어 원본 텍스트를 런타임 조회 키로 사용합니다. `strings.json`는 런타임 번들이 아닌 추출 캐시입니다. 문서는 `docs[].docsOutput.style` 값 `"nested"`, `"flat"`, `"doc-system"` 및 별칭 `"docusaurus"` / `"astro-starlight"` / `"vitepress"`를 지원합니다( [출력 레이아웃](/guide/documents/output-layouts) 참조). 이 세 가지 모두 `ai-i18n-tools.config.json`을 공유하며 결합할 수 있습니다. `sync`은 `features` 플래그에 따라 추출, UI 번역, SVG 번역, `translate-docs` 및 `translate-json`을 순서대로 실행합니다.
...
`ai-i18n-tools.config.json`를 편집합니다: `docs[].contentPaths`을 마크다운, MDX 및/또는 `.astro` 소스로 설정하고, `docs[].outputDir` 및 `docs[].docsOutput.style` (`"docusaurus"`, `"astro-starlight"`, `"vitepress"`, `"flat"` 등)를 설정합니다. 전체 필드 참조: [문서](/guide/documents/).

<a id="vitepress"></a>
### VitePress

`init -t ui-vitepress`는 `docsOutput.style: "vitepress"`과 테마/탐색/사이드바 문자열을 위한 `json[]` 블록을 스캐폴드합니다. `sync`를 실행하여 페이지 마크다운과 `theme.{locale}.json`를 함께 번역합니다. [VitePress 통합](/guide/vitepress-integration) 및 [examples/vitepress-docs](https://github.com/wsj-br/ai-i18n-tools/tree/main/examples/vitepress-docs/)를 참조하세요.
...
- [문서 사이트](https://wsj-br.github.io/ai-i18n-tools/) — 전체 VitePress 가이드 (GitHub Pages에 9개 로케일).
- [빠른 시작](/guide/quick-start) — UI 문자열, 문서 및 JSON 설정 (UI, 문서/`.astro`, JSON 번들, Astro Starlight 및 일반 Astro).
