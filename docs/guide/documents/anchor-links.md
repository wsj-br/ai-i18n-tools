...
1. Run `ai-i18n-tools write-heading-ids` on your source `.md` / `.mdx` before `translate-docs` (same `docs[]` / `contentPaths` as usual). It inserts explicit HTML anchors on the line before each heading so `id` values are shared by every translated copy, and it copies those same English ids onto existing translated files. Re-run it after renaming headings so stale anchor ids are refreshed to match the current title.
