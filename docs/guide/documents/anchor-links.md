...
1. Add an explicit id on the heading line with Docusaurus's classic `{#…}` suffix (CommonMark) or MDX comment `{/* #… */}` (preferred for `.mdx`), e.g. `## TLS configuration {#tls-configuration}` or `## TLS configuration {/* #tls-configuration */}`. During `translate-docs`, only the visible heading text is sent to the model — the id suffix is stripped first and pinned back to the **end** of the translated heading line (Docusaurus ignores a `{/* #id */}` that lands in the middle of the title).
2. Run `docusaurus write-heading-ids` from your Docusaurus project root (often `pnpm run write-heading-ids` when wired in `package.json`) to add or refresh ids on headings that lack them — use `--syntax mdx-comment` for the `{/* #… */}` form. Alternatively run `ai-i18n-tools write-heading-ids --slug-style mdx-comment` on the same `docs[]` / `contentPaths`. That command also repositions the same English ids in existing translated files (it does not slug the translated title) and updates the matching cached segment when segment counts line up, so a later `sync --force-update` keeps the repaired id. Re-run after renaming headings so stale ids match the current titles.
...
1. Run `ai-i18n-tools write-heading-ids` on your source `.md` / `.mdx` before `translate-docs` (same `docs[]` / `contentPaths` as usual). It inserts explicit HTML anchors on the line before each heading so `id` values are shared by every translated copy, and it copies those same English ids onto existing translated files. When segment counts line up, the matching cached segment is updated too, so a later `sync --force-update` keeps the repaired id. Re-run it after renaming headings so stale anchor ids are refreshed to match the current title.
