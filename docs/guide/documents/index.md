...
After restore, `translate-docs` also rejects segments where HTML tag placeholders were reused or dropped (so restored tags no longer match the source map) or where the model invented leftover double-brace tokens that were not in the source (for example a made-up glossary-style token). Pre-restore checks require the same multiset of `{{…}}` tokens and the same ordered subsequence of numbered tokens (`{{HTM_N}}`, `{{URL_N}}`, …); emphasis markers such as `{{SE}}` may move with natural word order when their per-type counts still match. Those failures use the same model-fallback path as leftover official internal tokens.
