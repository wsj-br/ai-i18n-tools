...
ai-i18n-tools includes its own string extractor. If you previously used `i18next-scanner`, `babel-plugin-i18next-extract`, or similar, you can remove those dev dependencies after migrating. Existing i18next **namespace JSON** files can stay on the [JSON pipeline](/guide/json#i18next-namespace-files). Intlayer `.content.ts` dictionaries can be imported with `migrate-intlayer` — see [Migrating from Intlayer](/guide/migrating-from-intlayer).
...
- **Repository root** — pnpm does not link the root package's own `bin` into `node_modules/.bin`. This checkout's `.envrc` adds `bin/` to `PATH` so the committed `bin/ai-i18n-tools` wrapper (beside the published `bin/ai-i18n-tools.mjs` shim) provides the bare command after `direnv allow`. Without direnv, use `node bin/ai-i18n-tools.mjs …`, `./bin/ai-i18n-tools …`, or root `pnpm i18n:*` scripts (or a shell alias / `pnpm add -g .` — see [Development Guide](https://github.com/wsj-br/ai-i18n-tools/blob/main/dev/DEVEL.md#running-the-cli-during-development)).
