...
The manifest is generated by `generate-ui-languages` from `sourceLocale` + `targetLocales` and the bundled master catalog. It is written to `uiLanguagesPath` (required). If you change locales in config, run `generate-ui-languages` again to refresh the file.
