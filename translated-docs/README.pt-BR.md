...
O pacote publicado é **somente ESM** (`"type": "module"`). Use `import` no Node.js, em bundlers ou com `import()` — `require('ai-i18n-tools')` **não é suportado.**
...
Exportado de `'ai-i18n-tools/runtime'` - funciona em qualquer ambiente JS, sem necessidade de importar o i18next:
