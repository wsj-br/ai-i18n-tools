...
**विषय सूची**  *जेनरेटेड विथ [DocToc](https://github.com/thlorenz/doctoc)*
...
`t("…")` / `i18n.t("…")` **लिटरल्स** से मास्टर कैटलॉग (`strings.json` के साथ वैकल्पिक प्रति-स्थानीय **`models`** मेटाडेटा) बनाता है, वैकल्पिक रूप से **`package.json` `description`**, और विन्यास में सक्षम होने पर प्रत्येक **`englishName`** को `ui-languages.json` से। ओपनराउटर के माध्यम से प्रति स्थानीय अनुवाद अनुपलब्ध प्रविष्टियों का अनुवाद करता है और फ्लैट JSON फ़ाइलें (`de.json`, `pt-BR.json`, …) लिखता है जो i18next के लिए तैयार हैं।
...
प्रकाशित पैकेज केवल **ESM** (`"type": "module"`) है। नोड.जेएस, बंडलर्स या `import()` से `import` का उपयोग करें — `require('ai-i18n-tools')` का समर्थन **नहीं** किया जाता है।
...
| हेल्पर | विवरण |
|---|---|
| `defaultI18nInitOptions(sourceLocale)` | कुंजी-के-रूप-में-डिफ़ॉल्ट सेटअप के लिए मानक i18next आरंभ विकल्प। |
| `setupKeyAsDefaultT(i18n, { stringsJson, sourcePluralFlatBundle? })` | अनुशंसित वायरिंग: कुंजी-ट्रिम + बहुवचन **`wrapT`** को **`strings.json`** से, वैकल्पिक रूप से **`translate-ui`** `{sourceLocale}.json` बहुवचन कुंजी को मर्ज करता है। |
...
| `makeLocaleLoadersFromManifest(uiLanguages, sourceLocale, makeLoader)` | **`ui-languages.json`** से **`code`** के अलावा हर **`sourceLocale`** के लिए **`makeLoadLocale`** के लिए **`localeLoaders`** मैप बनाता है। |
...
| `getUILanguageLabelNative(lang)` | `t()` को कॉल किए बिना प्रदर्शन लेबल (हेडर-शैली)। |
