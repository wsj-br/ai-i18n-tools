...
`ai-i18n-tools` es una CLI y un conjunto de herramientas para internacionalizar aplicaciones y sitios de documentación JavaScript/TypeScript —incluidos Docusaurus, Astro, Starlight, VitePress y Markdown/MDX simple— utilizando grandes modelos de lenguaje.
...
- **Cadenas de interfaz de usuario** — Extrae llamadas `t("…")` de JS/TS (y opcionalmente archivos `.astro`) y genera JSON plano por localidad para i18next o búsqueda estática SSG.
- **Documentos** — Traduce páginas Markdown, MDX y `.astro` listadas en `docs[].contentPaths` usando `translate-docs`. Funciona con **VitePress**, **Starlight**, **Docusaurus**, sitios basados en Astro, o cualquier generador de sitios estáticos que lea desde archivos fuente Markdown/MDX/`.astro`.
- **JSON** — Traduce paquetes JSON anidados arbitrarios definidos en `json[]`. Use `translate-json` cuando el texto de la interfaz de usuario resida en archivos JSON por localidad en lugar de llamadas `t()` en el código fuente.
...
| Páginas localizadas o sitios de documentos (VitePress, Starlight, Docusaurus, Astro, etc.) | **Documentos** — `translate-docs`            |
...
- [Tipos de traducción](#translation-types)
...
- [Inicio rápido](#quick-start)
  ...
  - [Astro (Astro simple y Starlight)](#astro-plain-astro--starlight)
  - [Sincronización combinada](#combined-sync)
- [Ayudantes de tiempo de ejecución](#runtime-helpers)
- [Comandos de la CLI](#cli-commands)
...
Cada tipo de traducción tiene su propia guía con detalles completos de configuración: [Cadenas de interfaz de usuario](../docs/guide/ui-strings/), [Documentos](../docs/guide/documents/) y [JSON](../docs/guide/json.md). Consulte [¿Qué es ai-i18n-tools?](../docs/guide/what-is-ai-i18n-tools.md) para una comparación lado a lado.

Algunas cosas que vale la pena saber de antemano: las cadenas de interfaz de usuario traducen las entradas faltantes por localidad a través del proveedor de LLM activo (consulte [Proveedores de LLM](#llm-providers)) y escriben archivos JSON planos (`de.json`, `pt-BR.json`, …), con el texto fuente en inglés como clave de búsqueda en tiempo de ejecución — `strings.json` es la caché de extracción, no el paquete en tiempo de ejecución. Los documentos admiten los valores `docs[].docsOutput.style` `"nested"`, `"flat"`, `"doc-system"` y los alias `"docusaurus"` / `"astro-starlight"` / `"vitepress"` (consulte [Diseños de salida](../docs/guide/documents/output-layouts.md)). Los tres comparten `ai-i18n-tools.config.json` y se pueden combinar; `sync` ejecuta la extracción, la traducción de la interfaz de usuario, la traducción de SVG, `translate-docs` y `translate-json` en orden según sus banderas `features`.
...
Para escribir el comando `ai-i18n-tools` sin formato en bash/zsh, anteponga el directorio bin local a `PATH` (consulte [Uso de la CLI](../docs/guide/installation.md#using-the-cli) para notas de PowerShell, direnv y Windows):
...
Prefiera `sync` en lugar de encadenar manualmente `extract`, `translate-ui`, `translate-svg`, `translate-docs` y `translate-json` — el orden y las banderas de características son fáciles de equivocarse cuando se ejecutan manualmente. Consulte [Scripts `package.json` recomendados](../docs/guide/quick-start.md#recommended-packagejson-scripts) en la guía de inicio rápido.
...
Luego, conecte i18next en su aplicación usando los ayudantes de `'ai-i18n-tools/runtime'`. Consulte [Paso 4: Conectar i18next en tiempo de ejecución](../docs/guide/ui-strings/i18next-runtime.md) en la guía de cadenas de interfaz de usuario para la configuración completa.
...
Edite `ai-i18n-tools.config.json`: establezca `docs[].contentPaths` en fuentes markdown, MDX y/o `.astro`; `docs[].outputDir` y `docs[].docsOutput.style` (`"docusaurus"`, `"astro-starlight"`, `"vitepress"`, `"flat"`, etc.). Referencia completa del campo: [Documentos](../docs/guide/documents/).

<a id="vitepress"></a>
### VitePress

`init -t ui-vitepress` genera `docsOutput.style: "vitepress"` más un bloque `json[]` para cadenas de tema/navegación/barra lateral. Ejecute `sync` para traducir el markdown de la página y `theme.{locale}.json` juntos. Consulte [integración de VitePress](../docs/guide/vitepress-integration.md) y [ejemplos/vitepress-docs](https://github.com/wsj-br/ai-i18n-tools/tree/main/examples/vitepress-docs/).

<a id="astro-plain-astro--starlight"></a>
### Astro (Astro plano y Starlight)

**Astro Starlight** — `init -t ui-starlight`, luego `translate-docs`. Las anulaciones de la interfaz de usuario de Starlight pueden usar `src/content/i18n/en.json` con `jsonPathTemplate` en un bloque `docs[]` separado cuando sea necesario ([Documentos — inicializar para documentación](../docs/guide/documents/index.md#step-1-initialise-for-documentation)).
...
Andamiaje de la interfaz de usuario con `init -t ui-astro-website`. Para HTML codificado en páginas `.astro`, habilite `features.translateDocs` y agregue un bloque `docs[]` con `docsOutput.style: "astro-starlight"` (consulte [Páginas del sitio web de Astro (analizar y reemplazar)](../docs/guide/ui-strings/astro-website.md#astro-website-pages-parse-and-replace)). Mantenga `targetLocales`, `i18n.locales` en `astro.config.mjs` y `ui-languages.json` alineados (las rutas de Astro usan códigos en minúsculas como `pt-br`; los nombres de archivo de paquetes planos siguen el uso de mayúsculas y minúsculas de la configuración, por ejemplo, `pt-BR.json`).

Conecte `t()` en tiempo de compilación sin i18next a menos que agregue islas de cliente; consulte [Cadenas de interfaz de usuario del sitio web de Astro (SSG)](../docs/guide/ui-strings/astro-website.md#astro-website-ui-strings-ssg) y el `src/i18n/t.ts` del ejemplo.
...
- [Inicio rápido](../docs/guide/quick-start.md) — configuración para cadenas de interfaz de usuario, documentos y JSON (interfaz de usuario, documentos/`.astro`, paquetes JSON, Astro Starlight y Astro simple).
- [Guía de recursos locales](../docs/guide/images-and-screenshots/) - capturas de pantalla y SVG ilustrados en documentos traducidos (reescritor de enlaces planos, scripts de captura de pantalla).
...
- Componentes internos del mantenedor para **este** repositorio: `dev/package-context.md` (solo clonar; no en npm).
