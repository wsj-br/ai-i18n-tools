...
使用 `i18next-scanner` 的 `Parser.parseFuncFromString` 在任意 JS/TS 文件中查找 `t("literal")` 和 `i18n.t("literal")` 调用。函数名和文件扩展名可配置。`extract` **还会将非扫描器输入合并到同一目录中：** 当启用 `reactExtractor.includePackageDescription`（默认）时的项目 `package.json` `description`，以及当 `reactExtractor.includeUiLanguageEnglishNames` 为 `true` 且设置了 `uiLanguagesPath` 时来自 `ui-languages.json` 的每个 **`englishName`**（源码中已找到的字符串优先）。片段哈希是经过修剪的源字符串的 **MD5 前 8 位十六进制字符** —— 这些将成为 `strings.json` 中的键。
...
`extract` 添加新键，并为扫描中仍存在的键保留现有的 `translated` / `models` 数据（扫描器字面量、可选描述、可选清单 `englishName`）。`translate-ui` 填充缺失的 `translated` 条目，更新其翻译的语言环境的 `models`，并写入扁平化的语言环境文件。
...
每个目标语言环境都会获得一个扁平化的 JSON 文件（`de.json`），将源字符串映射为翻译（无 `models` 字段）：
...
i18next 将这些作为资源包加载，并通过源字符串（键即默认值模型）查找翻译。

### UI 翻译提示

`buildUIPromptMessages` 构造系统和用户消息，用于：
...
- `MarkdownExtractor` - 将 Markdown 拆分为带类型的片段：`frontmatter`、`heading`、`paragraph`、`code`、`admonition`。不可翻译的片段（代码块、原始 HTML）将逐字保留。
...
在翻译之前，敏感语法会被替换为不透明的标记，以防止 LLM 破坏：

1. **警告标记**（`:::note`、`:::`）- 使用原始文本精确恢复。
2. **文档锚点**（HTML `<a id="…">`，Docusaurus 标题 `{#…}`）- 逐字保留。
3. **Markdown 链接**（`](url)`、`src="../…"`）- 翻译后从映射中恢复。
...
每次运行时，会按哈希 × 区域设置查找片段。只有未命中缓存的条目才会发送到 LLM。翻译完成后，`last_hit_at` 会被重置，针对当前翻译范围内未命中的片段行。`cleanup` 首先运行 `sync --force-update`，然后移除陈旧的片段行（`last_hit_at` 为空 / 文件路径为空），当解析后的源路径在磁盘上不存在时（`doc-block:…`、`svg-assets:…` 等），修剪 `file_tracking` 键，并删除其元数据文件路径指向缺失文件的翻译行；除非传入 `--no-backup`，否则会先备份 `cache.db`。

`translate-docs` 命令还使用 **文件跟踪**，因此已有输出的未更改源可以完全跳过处理。`--force-update` 重新运行文件处理，但仍使用片段缓存；`--force` 清除文件跟踪并绕过片段缓存读取以进行 API 翻译。完整标志表请参见 [入门](GETTING_STARTED.zh-CN.md#cache-behaviour-and-translate-docs-flags)。

**批处理提示格式：** `translate-docs --prompt-format` 仅为 `OpenRouterClient.translateDocumentBatch` 选择 XML（`<seg>` / `<t>`）或 JSON 数组/对象结构；提取、占位符和验证保持不变。详见 [批处理提示格式](GETTING_STARTED.zh-CN.md#batch-prompt-format)。
...
当 `markdownOutput.style === "flat"` 时，翻译后的 Markdown 文件会与源文件并列存放，并带有区域设置后缀。页面之间的相对链接会被重写，使得 `[Guide](../guide.md)` 在 `readme.de.md` 中指向 `guide.de.md`。由 `rewriteRelativeLinks` 控制（在没有自定义 `pathTemplate` 的情况下，扁平样式会自动启用）。
...
- **模型回退**：按顺序尝试解析出的模型列表中的每个模型；在发生 HTTP 错误或解析失败时进行回退。UI 翻译优先解析 `ui.preferredModel`（如果存在），然后是 `openrouter` 模型。
...
使用 **`setupKeyAsDefaultT`** 作为常规应用入口点（键裁剪 + 复数 **`wrapT`** + 可选的 **`translate-ui`** `{sourceLocale}.json`）。单独调用 **`wrapI18nWithKeyTrim`** 进行应用连接已被 **弃用**。

使用 **`makeLocaleLoadersFromManifest(uiLanguages, sourceLocale, …)`** 构建 **`localeLoaders`**，以便在 **`generate-ui-languages`** 后键值仍与 **`targetLocales`** 保持一致。参见 **`docs/GETTING_STARTED.md`**（运行时连接）和 **`examples/nextjs-app/`** / **`examples/console-app/`**。
...
所有公共类型和类都从包根目录导出。示例：在不使用 CLI 的情况下，通过 Node.js 运行 translate-UI 步骤：
