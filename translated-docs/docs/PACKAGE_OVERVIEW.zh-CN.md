...
使用 `i18next-scanner` 的 `Parser.parseFuncFromString` 在任意 JS/TS 文件中查找 `t("literal")` 和 `i18n.t("literal")` 调用。函数名和文件扩展名可配置。`extract` **还会将非扫描器输入合并到同一目录中：** 当启用 `reactExtractor.includePackageDescription` 时（默认）的项目 `package.json` `description`，以及当 `reactExtractor.includeUiLanguageEnglishNames` 为 `true` 且设置了 `uiLanguagesPath` 时来自 `ui-languages.json` 的每个 **`englishName`**（源码中已找到的字符串优先保留）。片段哈希是经过修剪的源字符串的 **MD5 前 8 位十六进制字符** —— 这些将成为 `strings.json` 中的键。
...
`models`（可选）—— 每个语言环境，记录该语言环境在上一次成功的 `translate-ui` 运行后由哪个模型生成了翻译（如果文本是从 `editor` 网页 UI 保存的，则为 `user-edited`）。`locations`（可选）—— `extract` 发现该字符串的位置（扫描器 + 包描述行；仅清单的 `englishName` 字符串可省略 `locations`）。

`extract` 添加新键，并为扫描中仍存在的键保留现有的 `translated` / `models` 数据（扫描器字面量、可选描述、可选清单 `englishName`）。`translate-ui` 填充缺失的 `translated` 条目，更新其翻译的语言环境的 `models`，并写入扁平化的语言文件。

`ui-languages.json` **清单** —— 一个 JSON 数组，包含 `{ code, label, englishName, direction }`（BCP-47 `code`、UI `label`、参考 `englishName`、`"ltr"` 或 `"rtl"`）。使用 `generate-ui-languages` 从 `sourceLocale` + `targetLocales` 和打包的主 `data/ui-languages-complete.json` 构建项目文件。

### 扁平化语言文件

每个目标语言环境都会获得一个扁平化的 JSON 文件（`de.json`），将源字符串映射为翻译（不含 `models` 字段）：
...
i18next 将这些作为资源包加载，并通过源字符串查找翻译（键作为默认模型）。

### UI 翻译提示

`buildUIPromptMessages` 构建系统 + 用户消息，内容包括：

- 确定源语言和目标语言（通过 `localeDisplayNames` 或 `ui-languages.json` 的显示名称）。
- 发送字符串的 JSON 数组并请求返回 JSON 数组的翻译。
- 在可用时包含术语表提示。

`OpenRouterClient.translateUIBatch` 按顺序尝试每个模型，在解析或网络错误时回退。CLI 从 `openrouter.translationModels`（或遗留默认/回退）构建该列表；对于 `translate-ui`，可选的 `ui.preferredModel` 在设置时会被添加（与其余部分去重）。
...
所有提取器扩展 `BaseExtractor` 并实现 `extract(content, filepath): Segment[]`。

- `MarkdownExtractor` - 将 markdown 拆分为类型化段落：`frontmatter`、`heading`、`paragraph`、`code`、`admonition`。不可翻译的段落（代码块、原始 HTML）将逐字保留。
- `JsonExtractor` - 从 Docusaurus JSON 标签文件中提取字符串值。
- `SvgExtractor` - 从 SVG 中提取 `<text>`、`<title>` 和 `<desc>` 内容（由 `translate-svg` 用于 `config.svg` 下的资产，而不是 `translate-docs`）。
...
SQLite 数据库（通过 `node:sqlite`）存储以 `(source_hash, locale)` 为键的行，包含 `translated_text`、`model`、`filepath`、`last_hit_at` 和相关字段。哈希是规范化内容的 SHA-256 前 16 个十六进制字符（空格折叠）。

在每次运行中，按哈希 × 语言环境查找段落。只有缓存未命中的部分会发送到 LLM。翻译后，`last_hit_at` 会重置当前翻译范围内未命中的段落行。`cleanup` 首先运行 `sync --force-update`，然后删除过时的段落行（空 `last_hit_at` / 空文件路径），在磁盘上缺少解析源路径时修剪 `file_tracking` 键（`doc-block:…`、`svg-assets:…` 等），并删除其元数据文件路径指向缺失文件的翻译行；除非传递了 `--no-backup`，否则会先备份 `cache.db`。

`translate-docs` 命令还使用 **文件跟踪**，因此未更改的源文件及其现有输出可以完全跳过工作。`--force-update` 在仍使用段落缓存的同时重新运行文件处理；`--force` 清除文件跟踪并绕过 API 翻译的段落缓存读取。有关完整标志表，请参见 [入门](GETTING_STARTED.zh-CN.md#cache-behaviour-and-translate-docs-flags)。

**批处理提示格式：** `translate-docs --prompt-format` 仅选择 XML（`<seg>` / `<t>`）或 JSON 数组/对象形状用于 `OpenRouterClient.translateDocumentBatch`；提取、占位符和验证保持不变。请参见 [批处理提示格式](GETTING_STARTED.zh-CN.md#batch-prompt-format)。
...
- `docusaurus` 风格：在 `docsRoot` 下，输出使用 `{outputDir}/{locale}/docusaurus-plugin-content-docs/current/{relativeToDocsRoot}`；在 `docsRoot` 之外的路径回退到嵌套布局。
...
- **自定义** `jsonPathTemplate`：JSON 标签文件的单独自定义布局，使用相同的占位符。
- `linkRewriteDocsRoot` 帮助平面链接重写器在翻译输出根目录不在默认项目根目录时计算正确的前缀。

### 扁平链接重写

当 `markdownOutput.style === "flat"` 时，翻译后的 Markdown 文件会与源文件并列放置，并带有区域设置后缀。页面之间的相对链接会被重写，使得 `[Guide](../guide.md)` 在 `readme.de.md` 中指向 `guide.de.md`。由 `rewriteRelativeLinks` 控制（在没有自定义 `pathTemplate` 的情况下，扁平样式会自动启用）。
...
封装了 OpenRouter 聊天补全 API。主要行为：

- **模型回退**：按顺序尝试解析出的模型列表中的每一个；在发生 HTTP 错误或解析失败时进行回退。UI 翻译优先解析 `ui.preferredModel`（如果存在），然后是 `openrouter` 模型。
...
`Logger` 支持 `debug`、`info`、`warn`、`error` 级别，并带有 ANSI 彩色输出。启用详细模式（`-v`）后，会激活 `debug`。当设置了 `logFilePath` 时，日志行也会写入该文件。

---

## 运行时辅助 API

这些内容从 `'ai-i18n-tools/runtime'` 导出，可在任何 JavaScript 环境中使用（浏览器、Node.js、Deno、Edge）。它们 **不** 会从 `i18next` 或 `react-i18next` 导入。
...
使用 **`setupKeyAsDefaultT`** 作为常规应用入口点（键裁剪 + 复数 **`wrapT`** + 可选的 **`translate-ui`** `{sourceLocale}.json`）。单独调用 **`wrapI18nWithKeyTrim`** 进行应用配置已 **弃用**。
...
所有公共类型和类都从包根目录导出。示例：在不使用 CLI 的情况下，从 Node.js 运行 translate-UI 步骤：
...
通过以编程方式导入 `doc-translate.ts` 实用程序，将其传递给 doc-translate 管道。
