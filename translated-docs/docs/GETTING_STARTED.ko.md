...
게시된 패키지는 **ESM 전용**입니다. Node.js 또는 번들러에서는 `import`/`import()`을 사용하고, `require('ai-i18n-tools')`는 사용하지 마십시오. 패키지는 **`engines.node` `>=22.16.0`** 를 선언하므로, 이전 버전의 Node.js는 지원되지 않습니다.
...
**기존에 “run `translate-ui`, then `translate-svg`, then `translate-docs`”이라고 불렸던 모든 작업에 대해 `sync`** 를 사용하는 것이 좋습니다: `ai-i18n-tools sync`는 **extract**(활성화된 경우), **translate-ui**, 선택적 **translate-svg**, 그리고 **translate-docs**를 올바른 순서와 공유 플래그로 실행합니다—당신의 구성에 따라. 이 세 가지 번역 명령을 수동으로 연결하는 것은 잘못될 수 있습니다(순서, 추출, 로케일 플래그). `i18n:translate:ui`, `i18n:translate:svg`, 및 `i18n:translate:docs`는 고립된 **단일** 단계가 필요할 때만 사용하세요.
...
**최소한의 README 전용 예제(`examples/console-app`)**
