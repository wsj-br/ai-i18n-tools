...
主に**markdownおよびMDXドキュメント**用に設計されており、`contentPaths`（読者が関心を持つページ）で使用されます。Docusaurusサイトでは、`docusaurus write-translations`によって生成された**JSONラベルファイル**も翻訳できます — これらはテーマ、ナビゲーションバー、フッター、およびプラグインUI文字列（シェルi18n）を含み、`docs/`の本文とは異なります。markdownに埋め込まれたPNGやその他のラスター画像については、[翻訳されたドキュメント内の画像とラスター資産](#images-and-raster-assets-in-translated-docs)を参照してください。READMEや`markdownOutput.style = "flat"`を含むドキュメント内のオプションの**言語スイッチャー**ブロックについては、[言語スイッチャー（`languageListBlock`）](#language-list-block)を参照してください。SVGファイルは、`features.translateSVG`が有効で、最上位の`svg`ブロックが設定されている場合に[`translate-svg`](#cli-reference)を介して翻訳されます — `documentations[].contentPaths`を介してではありません。
...
| `sync-ui [-l <codes>] [--force] [--dry-run] [-j <n>]`                                    | `features.extractUIStrings` が有効な場合に抽出を実行し、次に `features.translateUIStrings` が有効な場合に UI 文字列を翻訳します。UI 専用 — ドキュメントや SVG は対象外です。`translate-ui` と同じ `-l`、`--force`、`--dry-run`、`-j` オプションを使用します。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `lint-source [-l <code>] [--chunk <n>] [--dry-run] [--json] [-j <n>]`                    | 最初に `extract` **を実行**（`features.extractUIStrings` が必要）して `strings.json` をソースと一致させた後、**ソース-ロケール**の UI 文字列に対して LLM によるレビュー（スペル、文法）を実施します。**用語のヒント**は `glossary.userGlossary` CSV からのみ取得されます（`translate-ui` と同じ範囲 — `strings.json` / `uiGlossary` ではないため、誤ったコピーが用語集として強化されることはありません）。OpenRouter（`OPENROUTER_API_KEY`）を使用します。アドバイスのみ（実行終了時は **0** を返します）。`cacheDir` 配下に **人間が読みやすい形式**のレポート（要約、問題点、および文字列ごとの **OK** 行）として `lint-source-results_<timestamp>.log` を出力します。端末には要約カウントと問題点のみを表示（文字列ごとに `[ok]` 行は出力しない）。最後の行にログファイル名を出力します。`--json`：完全な機械判読可能な JSON レポートを標準出力にのみ出力（ログファイルは人間が読みやすい形式のまま）。`--dry-run`：依然として `extract` を実行し、バッチ計画のみを出力（API 呼び出しは行わない）。`--chunk`：API バッチごとの文字列数（デフォルト **50**）。`-j`：並列実行する最大バッチ数（デフォルト `concurrency`）。`--json` を使用すると、人間向けの出力は stderr に出力されます。リンクには `path:line` を使用します。これは `dashboard` UI 文字列の「リンク」ボタンと同様です。 |
