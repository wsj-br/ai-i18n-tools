...
जब हर कॉन्फ़िगर किया गया मॉडल एक ही सेगमेंट पर `AST mismatch` के साथ विफल हो जाता है, तो `translate-docs` उस सेगमेंट को छोटे भागों में स्वचालित रूप से विभाजित कर सकता है (पहले सूची के मध्य बिंदु, फिर एकल सूची आइटम या छोटे पैराग्राफ टुकड़े), प्रत्येक भाग को पहले मॉडल से पुनः प्रयास कर सकता है, और परिणाम को मूल सेगमेंट कैश कुंजी के तहत फिर से जोड़ सकता है। यह डिफ़ॉल्ट रूप से चालू होता है (`segmentSplitting.qualityRetrySplit`); मॉडल समाप्त होने के बाद रोकने के लिए इसे `false` पर सेट करें। जब यह फॉलबैक चलता है तो रन सारांश में `Quality split retries` दिखाया जाता है।
...
डिफ़ॉल्ट सुनने वाला पोर्ट **8675** है। यदि वह पोर्ट उपलब्ध नहीं है, तो सर्वर अगले पोर्ट की कोशिश करता है (अधिकतम 1000 प्रयास तक) और उस पोर्ट को लॉग करता है जिसे यह चुनता है। पुराना उपनाम `editor` अभी भी काम करता है लेकिन एक चेतावनी प्रिंट करता है — `dashboard` को प्राथमिकता दें।
...
**Markdown मुद्दे** टैब `markdown_source_issues` SQLite तालिका से पंक्तियाँ सूचीबद्ध करता है। प्रत्येक पंक्ति एक **पूर्व-अनुवाद** खोज है: उदाहरण के लिए, डिलिमिटर रन जो कभी भी एक ही CommonMark-शैली के नियमों के तहत जोर/स्ट्राइकथ्रू के रूप में जोड़े नहीं जाते `translate-docs` जो मास्किंग के लिए उपयोग करता है, एक इनलाइन कोड स्पैन जो बैकटिक्स के साथ खोला गया है लेकिन कभी बंद नहीं हुआ, या `STRONG_OUTSIDE_LINK` जब `**` / `__` एक `[text](../../docs/url)` लिंक को लपेटते हैं (लिंक टेक्स्ट के अंदर केवल बोल्ड डालें)। यह **Failures** के समान **नहीं** है, जो प्रति-स्थान मॉडल आउटपुट और पोस्ट-अनुवाद सत्यापन समस्याओं को रिकॉर्ड करता है (`AST mismatch`, प्लेसहोल्डर लीक, और इसी तरह)।
...
`docsOutput` के समान स्तर पर (प्रति `docs[]` ब्लॉक)। डिफ़ॉल्ट `true`: स्टारलाइट/डॉक्यूसॉरस के लिए उपयोगकर्ता-अभिमुख YAML गद्य का अनुवाद करें (`title`, `description`, `sidebar.label`, `sidebar_label`, `keywords`, `hero.title`, `hero.tagline`, `hero.image.alt`, `hero.actions[].text`, `pagination_label`, `prev`/`next` लेबल)। पूरे फ्रंट मैटर ब्लॉक को अपरिवर्तित रखने के लिए `false` सेट करें; विशिष्ट डॉट-पथों तक सीमित करने के लिए एक स्ट्रिंग सरणी पास करें।
- `segmentSplitting`
`docsOutput` के समान स्तर पर (प्रति `docs[]` ब्लॉक)। `translate-docs` निष्कर्षण के लिए वैकल्पिक सूक्ष्म सेगमेंट: `{ "enabled", "maxCharsPerSegment"?, "splitPipeTables"?, "splitDenseParagraphs"?, "maxLinesPerParagraphChunk"?, "splitLongLists"?, "maxListItemsPerChunk"?, "qualityRetrySplit"?, "maxQualityRetrySplitDepth"? }`। जब `enabled` `true` है (जब `segmentSplitting` उपलब्ध नहीं है तो डिफ़ॉल्ट), घने पैराग्राफ, GFM पाइप तालिकाएं (पहला टुकड़ा हेडर, अलग करने वाला और पहली डेटा पंक्ति शामिल करता है), और लंबी सूचियां विभाजित की जाती हैं; उप-भाग एकल नए पंक्ति के साथ फिर से जुड़ते हैं (`tightJoinPrevious`)। केवल एक रिक्त पंक्ति द्वारा अलग किए गए बॉडी ब्लॉक प्रति सेगमेंट के लिए `"enabled": false` का उपयोग करें। जब `qualityRetrySplit` `true` है (डिफ़ॉल्ट), तो सभी मॉडल समाप्त होने के बाद AST सत्यापन में विफल मार्कडाउन सेगमेंट को क्रमिक रूप से विभाजित किया जाता है और पहले मॉडल से पुनः प्रयास किया जाता है; `maxQualityRetrySplitDepth` (डिफ़ॉल्ट `3`) पुनरावर्ती विभाजन को सीमित करता है।
- `warnMarkdownSourceIssues`
जब `true` (उपलब्ध नहीं होने पर डिफ़ॉल्ट), प्रत्येक `translate-docs` रन मार्कडाउन सेगमेंट में जोखिम भरे डिलीमीटर / अनबंद इनलाइन कोड के लिए पुनः स्कैन करता है, टर्मिनल चेतावनियां प्रिंट करता है, और उस फ़ाइल के कैश फ़ाइलपाथ के लिए `markdown_source_issues` पंक्तियों को प्रतिस्थापित करता है। इस ब्लॉक के लिए चेतावनियों और SQLite अपडेट को छोड़ने के लिए `false` सेट करें।
- `addFrontmatter`
जब `true` (उपलब्ध नहीं होने पर डिफ़ॉल्ट), अनुवादित मार्कडाउन फ़ाइलों में YAML कुंजियां शामिल होती हैं: `translation_last_updated`, `source_file_mtime`, `source_file_hash`, `translation_language`, `source_file_path`, और जब कम से कम एक सेगमेंट में मॉडल मेटाडेटा होता है, तो `translation_models` (उपयोग किए गए OpenRouter मॉडल आईडी की क्रमबद्ध सूची)। छोड़ने के लिए `false` पर सेट करें।
...
| `check-markdown …`                                                                                         | प्रत्येक `docs[]` ब्लॉक के `contentPaths` के तहत मार्कडाउन/MDX को स्कैन करता है (`translate-docs` के समान खोज, `.translate-ignore` का पालन करता है): डिलीमीटर जोड़ीकरण, अनबंद इनलाइन कोड, और जब `**`/`__` एक `[text](../../docs/url)` लिंक को लपेटते हैं तो `STRONG_OUTSIDE_LINK`। `-p` / `--path` या `-f` / `--file`: वैकल्पिक क्षेत्र। **stderr** पर `relativePath:line: [ISSUE_CODE] message` पंक्तियां प्रिंट करता है; कोई भी समस्या होने पर एग्जिट कोड **1**। `--json`: **stdout** पर JSON रिपोर्ट। `cacheDir` में `markdown_source_issues` लिखता है जब तक `--no-cache` न हो। `-v` स्रोत हैश को stderr पंक्तियों में जोड़ता है।                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `cleanup [--dry-run] [--no-backup] [--backup <path>]`                                                      | सबसे पहले `sync --force-update` चलाता है (निकालें, UI, SVG, दस्तावेज़), फिर बेकार सेगमेंट पंक्तियाँ हटा देता है (शून्य `last_hit_at` / खाली फ़ाइलपथ); उन `file_tracking` पंक्तियों को हटा देता है जिनका संकल्पित स्रोत पथ डिस्क पर अनुपलब्ध है; उन अनुवाद पंक्तियों को हटा देता है जिनका `filepath` मेटाडेटा लापता फ़ाइल की ओर इशारा करता है; अनाथ `translation_failures` पंक्तियों को हटा देता है। चार गिनतियों को लॉग करता है (बेकार सेगमेंट, अनाथ `file_tracking`, अनाथ अनुवाद, अनाथ विफलताएँ)। कैश डायरेक्टरी के तहत समयस्टैम्प युक्त SQLite बैकअप बनाता है, जब तक कि `--no-backup` हो।                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `dashboard [-p <port>] [--no-open]`                                                                        | अनुवाद डैशबोर्ड लॉन्च करें (कैश सेगमेंट्स, `strings.json`, शब्दावली, विफलताओं और आँकड़ों के लिए स्थानीय वेब UI)। डिफ़ॉल्ट पोर्ट **8675** (अगर अनुपलब्ध हो तो अगले पोर्ट पर पुनः प्रयास करता है)। `--no-open` के साथ, डिफ़ॉल्ट ब्राउज़र स्वचालित रूप से नहीं खुलता है। अप्रचलित उपनाम `editor` अभी भी काम करता है लेकिन चेतावनी प्रिंट करता है।                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
