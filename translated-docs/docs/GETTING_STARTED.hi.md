...
| `check-markdown …`                                                            | **कोई API नहीं।** प्रत्येक `documentations[]` ब्लॉक के तहत `contentPaths` में मार्कडाउन/MDX को स्कैन करता है (`translate-docs` के समान खोज, `.translate-ignore` का पालन करता है): डिलीमीटर जोड़ी बनाना, अपूर्ण इनलाइन कोड, और जब `**`/`__` एक `` `...` `` स्पैन या एक `[text](../url)` लिंक को लपेटते हैं तो `STRONG_OUTSIDE_INLINE_CODE` / `STRONG_OUTSIDE_LINK`। `-p` / `--path` या `-f` / `--file`: वैकल्पिक क्षेत्र। **stderr** पर `relativePath:line: [ISSUE_CODE] message` पंक्तियाँ प्रिंट करता है; कोई भी समस्या होने पर निकास कोड **1**। `--json`: **stdout** पर JSON रिपोर्ट। `cacheDir` में `markdown_source_issues` लिखता है जब तक `--no-cache` न हो। `-v` stderr पंक्तियों में स्रोत हैश जोड़ता है।                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `clean-temp [-r|--root <path>] [-f|--force] [--dry-run]`                     | **कोई कॉन्फ़िगरेशन नहीं।** डिफ़ॉल्ट रूप से cwd के लिए निर्देशिका ट्री को `*.log` और `cache.db.backup*.sqlite` के लिए पार करता है, `find -print` की तरह `./…` पथ प्रिंट करता है। मिलान होने पर: जब तक `-f` / `--force` न हो, तब तक `Delete these files? (y/n)` के लिए प्रॉम्प्ट करता है (बिना प्रॉम्प्ट के हटाता है)। कोई मिलान नहीं होने पर: प्रॉम्प्ट किए बिना बाहर निकलता है। `--dry-run`: केवल सूची, कोई प्रॉम्प्ट या हटाना नहीं (`--force` को ओवरराइड करता है)।                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
