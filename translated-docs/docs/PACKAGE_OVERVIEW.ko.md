...
`i18next-scanner`의 `Parser.parseFuncFromString`을 사용하여 JS/TS 파일에서 `t("literal")` 및 `i18n.t("literal")` 호출을 찾습니다. 함수 이름과 파일 확장자는 구성 가능합니다. `extract` **또한 스캐너가 아닌 입력을 동일한 카탈로그에 병합합니다.** `reactExtractor.includePackageDescription`이(가) 활성화된 경우(기본값) 프로젝트 `package.json` `description`과(와) `reactExtractor.includeUiLanguageEnglishNames`이(가) `true`이고 `uiLanguagesPath`이(가) 설정된 경우 `ui-languages.json`의 각 **`englishName`** 을(를) 병합합니다(소스에서 이미 발견된 문자열이 우선 적용됨). 세그먼트 해시는 잘린 소스 문자열의 **MD5 앞 8자리 16진수**이며, 이는 `strings.json`의 키가 됩니다.
...
`ui-languages.json` **매니페스트** — `{ code, label, englishName, direction }`(BCP-47 `code`, UI `label`, 참조 `englishName`, `"ltr"` 또는 `"rtl"`)의 JSON 배열. `sourceLocale` + `targetLocales` 및 번들된 마스터 `data/ui-languages-complete.json`에서 프로젝트 파일을 생성하려면 `generate-ui-languages`을(를) 사용합니다.
...
`buildUIPromptMessages`은 다음을 수행하는 시스템 및 사용자 메시지를 생성합니다.

- 소스 및 대상 언어를 식별합니다(`localeDisplayNames` 또는 `ui-languages.json`의 표시 이름 기준).
- 문자열의 JSON 배열을 전송하고 번역된 문자열의 JSON 배열을 반환하도록 요청합니다.
- 용어집 힌트가 있는 경우 포함합니다.
