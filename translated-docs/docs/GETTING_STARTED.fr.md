...
| `cleanup [--dry-run] [--no-backup] [--backup <path>]`                                                      | Exécute d'abord `sync --force-update` (extraction, interface utilisateur, SVG, documentation), puis supprime les lignes de segments obsolètes (`last_hit_at` nul / chemin de fichier vide) ; supprime les lignes de `file_tracking` dont le chemin source résolu est manquant sur le disque ; supprime les lignes de traduction dont les métadonnées `filepath` font référence à un fichier manquant ; élimine les lignes `translation_failures` orphelines. Journalise quatre compteurs (segments obsolètes, `file_tracking` orphelines, traductions orphelines, échecs orphelins). Crée une sauvegarde SQLite horodatée dans le répertoire de cache, sauf si `--no-backup`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
