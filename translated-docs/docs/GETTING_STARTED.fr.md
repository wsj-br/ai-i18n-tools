...
Conçu pour la documentation en markdown, les sites Docusaurus et les fichiers d'étiquettes JSON. Pour les images PNG et autres images matricielles intégrées au markdown, voir [Images et ressources matricielles dans les documents traduits](#images-and-raster-assets-in-translated-docs). Les ressources SVG autonomes sont traduites via [`translate-svg`](#cli-reference) lorsque `features.translateSVG` est activé et que le bloc `svg` de niveau supérieur est défini — et non via `documentations[].contentPaths`.
...
- Les lignes de segment sont indexées globalement par `(source_hash, locale)` (hachage = contenu normalisé). Un texte identique dans deux fichiers partage une seule ligne ; `translations.filepath` est des métadonnées (dernier auteur), pas une deuxième entrée de cache par fichier.
- `file_tracking.filepath` utilise des clés avec espace de noms : `doc-block:{index}:{relPath}` par bloc `documentations` (`relPath` est un chemin posix relatif à la racine du projet : chemins markdown collectés ; **les fichiers d’étiquettes JSON utilisent le chemin relatif au répertoire courant du fichier source**, par exemple `docs-site/i18n/en/code.json`, afin que le nettoyage puisse résoudre le fichier réel), et `svg-assets:{relPath}` pour les ressources SVG autonomes situées sous `translate-svg`.
...
- Après une exécution, `last_hit_at` est effacé uniquement pour les lignes de segment **dans la même portée de traduction** (en respectant `--path` et les types activés) qui n’ont pas été atteintes, ainsi une exécution filtrée ou uniquement pour la documentation n’indique pas comme obsolètes des fichiers non concernés.
...
**SVG** destiné à être utilisé comme ressource illustrée utilise le bloc `svg` et `translate-svg` — voir [`svg` (facultatif)](#svg-optional). Les chemins répertoriés dans `documentations[].contentPaths` concernent le markdown/MDX (et les étiquettes JSON facultatives), pas la traduction de fichiers SVG autonomes.
...
| `requestTimeoutMs` | Durée maximale, en millisecondes, d'attente pour chaque requête HTTP vers OpenRouter (génération de conversations et appels internes `GET /models`). Par défaut : `30000` (30 secondes). |
...
| `warnMarkdownSourceIssues`                     | Lorsque `true` (valeur par défaut si omis), chaque exécution de `translate-docs` analyse à nouveau les segments Markdown à la recherche de délimiteurs risqués ou de code en ligne non fermé, affiche des avertissements dans le terminal, et remplace les lignes `markdown_source_issues` pour le chemin de cache de ce fichier. Définissez `false` pour ignorer les avertissements et les mises à jour SQLite pour ce bloc.                                                                                                                                                                                                                                                                                                                                                           |
...
| `check-models` | Valide chaque identifiant de modèle OpenRouter configuré par rapport à `GET /models` (appartenance au catalogue, `expiration_date`, coût en dollars américains par million de jetons pour les invites et les complétions). Nécessite `OPENROUTER_API_KEY`. Se termine avec un statut non nul si un identifiant configuré est manquant ou expiré. Respecte `openrouter.requestTimeoutMs` pour la requête de catalogue. |
...
| `strip-md-bold-inline …`                                                    | **Pas d'API.** Nécessite au moins un bloc `documentations[]`. Supprime les `**` entourant le code en ligne dans `.md` / `.mdx` sous la `contentPaths` de chaque bloc (respecte `.translate-ignore`). `-p` / `--path` ou `-f` / `--file`, `--dry-run`, `--no-backup` (ignore les `.backup.*` horodatés avant écrasement).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `check-markdown …`                                                            | **Pas d'API.** Analyse le Markdown/MDX dans chaque `documentations[]` du bloc `contentPaths` (même découverte que `translate-docs`, respecte `.translate-ignore`) : appariement des délimiteurs, code en ligne non fermé, et `STRONG_OUTSIDE_INLINE_CODE` / `STRONG_OUTSIDE_LINK` lorsque `**`/`__` encadrent une portion `` `...` `` ou un lien `[text](../url)`. `-p` / `--path` ou `-f` / `--file` : portée facultative. Affiche les lignes `relativePath:line: [ISSUE_CODE] message` sur **stderr** ; code de sortie **1** s'il y a un problème. `--json` : rapport JSON sur **stdout**. Écrit `markdown_source_issues` dans `cacheDir` sauf si `--no-cache`. `-v` ajoute les hachages des sources aux lignes stderr.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `statistics [--max-columns <n>]`                                             | Affiche le cache de documentation et les statistiques `strings.json` (mêmes agrégats que l'éditeur de cache de traduction → **Statistiques**). `--max-columns` : nombre maximal de colonnes par langue par modèle × tableau de langues (valeur par défaut identique à l'éditeur).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
