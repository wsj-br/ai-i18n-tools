...
- [Flux de travail 2 - Traduction de documents](#workflow-2---document-translation)
  ...
  - [Dispositions de sortie](#output-layouts)
    - [Liens d'ancre dans la disposition plate](#anchor-links-in-flat-layout)
...
- [Flux de travail combiné (interface utilisateur + documentation)](#combined-workflow-ui--docs)
  - [Flux de documentation mixte (Docusaurus + plat)](#mixed-documentation-workflow-docusaurus--flat)
- [Éditeur du cache de traduction](#translation-cache-editor)
  - [Échecs (traduction de document)](#failures-document-translation)
...
Gardez `localeLoaders` **alignés avec la configuration** en les dérivant de `ui-languages.json` à l’aide de `makeLocaleLoadersFromManifest` (ceci filtre `SOURCE_LOCALE` en utilisant la même normalisation que `makeLoadLocale`). Lorsque vous ajoutez une langue à `targetLocales` et exécutez `generate-ui-languages`, le manifeste est mis à jour et vos chargeurs suivent automatiquement le changement — il n’est pas nécessaire de maintenir une carte codée en dur séparée.
...
`setupKeyAsDefaultT(i18n, { stringsJson, sourcePluralFlatBundle? })` est le câblage **recommandé** pour les projets ai-i18n-tools : il applique le nettoyage des clés + la substitution de langue source avec <code>"{{var}}"</code> (comportement identique à celui de `wrapI18nWithKeyTrim` de niveau inférieur), fusionne éventuellement les clés suffixées au pluriel depuis `translate-ui` via `{sourceLocale}.json` grâce à `addResourceBundle`, puis installe la gestion plurielle `wrapT` à partir de votre `strings.json`. Ce fichier agrégé doit être la version plate des pluriels pour votre langue source **configurée** — la même que `sourceLocale` dans `ai-i18n-tools.config.json` et `SOURCE_LOCALE` lors de l'initialisation i18n (voir Étape 4 ci-dessus). Omettez `sourcePluralFlatBundle` uniquement pendant l'initialisation (fusionnez-le une fois que `translate-ui` a généré `{sourceLocale}.json`). L'utilisation seule de `wrapI18nWithKeyTrim` est **déconseillée** dans le code applicatif — utilisez plutôt `setupKeyAsDefaultT`.
...
1. Exécutez `ai-i18n-tools write-heading-ids` sur votre source `.md` / `.mdx` avant `translate-docs` (même `documentations[]` / `contentPaths` que d'habitude). Cet outil insère des ancres HTML explicites sur la ligne précédant chaque en-tête, afin que les valeurs `id` soient partagées par chaque version traduite.
2. Faites pointer vos **liens d'ancre** en markdown vers ces identifiants stables, par exemple `[label](../other.md#section-id)`, où `section-id` correspond à l'ancre insérée par l'outil — et non à une déduction basée uniquement sur les mots anglais.
...
Avec `markdownOutput.style` `flat` et la réécriture de liens relatifs par défaut, les liens entre les pages traduites sont réécrits selon la langue. Les liens vers des fichiers non markdown reçoivent un préfixe de profondeur afin de rester relatifs à chaque fichier de sortie (par exemple, `figure.png` à côté de la source peut devenir `../figure.png` dans le fichier traduit). Cette URL ne résout généralement que **dans** le répertoire de sortie. L'interface en ligne de commande (CLI) n'émet pas le fichier binaire à cet emplacement, donc les lecteurs rencontrent une erreur 404 sauf si vous copiez les ressources, les servez ailleurs ou réécrivez le lien. Appliquez vos règles après la traduction : `postProcessing` s'exécute après le réassemblage des segments et la réécriture des liens plats (voir la ligne `markdownOutput.postProcessing` dans [Référence de configuration](#configuration-reference)).

**Modèle 1 — Ressource dans le même dépôt, à côté de la source anglaise (ce package)**

Ce dépôt traduit `docs/GETTING_STARTED.md` en `translated-docs/docs/GETTING_STARTED.<locale>.md`. La source utilise une image sœur, `translation-cache-editor.png`. La réécriture plate ciblerait `translated-docs/translation-cache-editor.png`, qui n'est jamais écrite. Le `ai-i18n-tools.config.json` racine ajoute une règle qui correspond à la partie stable finale de l'image markdown (le segment d'URL `](…)`, pas le texte alternatif traduit) et redirige vers `docs/` :
...
**Modèle 2 — Dossiers de captures d'écran par langue** (`examples/nextjs-app`)

L'exemple Next.js utilise deux blocs `documentations[]` dans `examples/nextjs-app/ai-i18n-tools.config.json`.

- **Documentation Docusaurus** (`markdownOutput.style` `docusaurus`) : les pages anglaises sous `docs-site/docs/` font référence à des captures d'écran avec un segment de langue fixe dans l'URL, par exemple `/img/screenshots/en-GB/screenshot.png` dans `feature-showcase.md`. Le post-traitement remplace ce segment afin que chaque page traduite sous `docs-site/i18n/<locale>/…/current/` accède à son propre dossier :
...
Fournissez les fichiers PNG correspondants dans l'arborescence statique de votre site (par exemple `docs-site/static/img/screenshots/<locale>/` pour les URL commençant par `/img/screenshots/`).

- **README racine, sortie plate** (deuxième bloc `documentations[]` dans le même fichier) : seul `README.md` est traduit, avec `markdownOutput.style` `flat` et `outputDir` `translated-docs`, ce qui donne `translated-docs/README.<locale>.md`. Les images en anglais utilisent souvent un segment de dossier stable au milieu du chemin (par exemple `images/screenshots/en-GB/overview.png`). Un post-traitement remplace le segment de chemin unique situé entre `images/screenshots/` et le reste de l'URL par le `${translatedLocale}` actif, de sorte que chaque README traduit pointe vers `images/screenshots/de/…`, `images/screenshots/fr/…`, etc. Ce modèle diffère de la règle Docusaurus : ici, `search` correspond à **n'importe quel** nom de dossier (`[^/]+/`), et pas seulement à `en-GB/`.
...
Exécutez `translate-svg` (ou `sync`) pour que `images/*.svg` devienne des sorties par langue sous `public/assets/`. Les références markdown pointent vers ces URL indépendamment de `translate-docs`.

**Exemple minimal avec seulement README** (`examples/console-app`)

`examples/console-app/ai-i18n-tools.config.json` traduit `README.md` en `translated-docs/` avec uniquement `postProcessing.languageListBlock`. Il ne définit aucune règle d'image — ce qui convient lorsque le README n'a pas de fichiers matriciels associés ou utilise uniquement des URL absolues que votre hébergeur sert déjà.
...
| Champ         | Description                                               |
|---------------|-----------------------------------------------------------|
| `sourceRoots` | Répertoires ou motifs glob (relatifs au répertoire courant) analysés pour les appels `t("…")`. |
...
- `description`
Note facultative lisible par un humain pour ce bloc (non utilisée pour la traduction). Préfixe le titre dans l'en-tête `translate-docs` `🌐` lorsqu'elle est définie ; également affichée dans les en-têtes de section `status`.
- `contentPaths`
Corps de pages Markdown/MDX à traduire (`translate-docs` analyse ceux-ci pour `.md` / `.mdx`). Prend en charge les **chemins de répertoires ou motifs glob** (par exemple `"docs/**/*.md"`, `"guides/*.mdx"`). C'est là que provient la documentation localisée.
- `outputDir`
...
| Champ                         | Description                                                                                                                                                                                                                                                                        |
|-------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `sourcePath`                  | Un ou plusieurs répertoires **ou motifs génériques** (par exemple `"images/*.svg"`, `"**/icons/*.svg"`). Les motifs sont résolus par rapport à la racine du projet et analysés récursivement pour les fichiers `.svg`.                                                                                       |
...
- `extract`
Mettre à jour `strings.json` à partir des littéraux `t("…")` / `i18n.t("…")`, une description facultative `package.json` et des entrées facultatives `englishName` du manifeste (voir `ui.reactExtractor`). Nécessite `features.extractUIStrings`.

- `generate-ui-languages [--master <path>] [--dry-run]`
Écrire `ui-languages.json` dans `ui.flatOutputDir` (ou `uiLanguagesPath` si défini) en utilisant `sourceLocale` + `targetLocales` et le fichier `data/ui-languages-complete.json` intégré (ou `--master`). Affiche un avertissement et émet des espaces réservés `TODO` pour les langues manquantes dans le fichier maître. Si vous avez un manifeste existant avec des valeurs personnalisées pour `label` ou `englishName`, elles seront remplacées par les valeurs par défaut du catalogue maître — vérifiez et ajustez le fichier généré par la suite.

- `translate-docs …`
Traduire les fichiers markdown/MDX et JSON pour chaque bloc `documentations` (`contentPaths`, `jsonSource` facultatif). `-j` : nombre maximal de langues en parallèle ; `-b` : nombre maximal d'appels d'API par lot par fichier. `--prompt-format` : format de transmission du lot (`xml` \| `json-array` \| `json-object`). Voir [Comportement du cache et indicateurs `translate-docs`](#cache-behaviour-and-translate-docs-flags) et [Format du prompt par lot](#batch-prompt-format).

- `write-heading-ids …`
**Pas d'API.** Nécessite au moins un bloc `documentations[]`. Collecte les éléments `.md` / `.mdx` sous le `contentPaths` de chaque bloc (respecte `.translate-ignore`). Insère une ligne d'ancre HTML `<a id="slug"></a>` immédiatement **avant** chaque en-tête ATX plat `#` (ignore les en-têtes à l'intérieur des blocs de code délimités). `-p` / `--path` ou `-f` / `--file` : limite à un fichier ou un répertoire relatif au projet. `--slug-style` : `github` (par défaut ; doctoc / anchor-markdown-header), `bitbucket`, `gitlab`, `pymdown`, `azure-devops`. Avec `pymdown`, `--pymdown-case`, `--pymdown-normalize`, `--pymdown-percent-encode` / `--no-pymdown-percent-encode` sont facultatifs. `--dry-run` : affiche uniquement les modifications.

- `strip-md-bold-inline …`
**Pas d'API.** Nécessite au moins un bloc `documentations[]`. Supprime les éléments `**` entourant le code en ligne dans `.md` / `.mdx` sous le `contentPaths` de chaque bloc (respecte `.translate-ignore`). `-p` / `--path` ou `-f` / `--file`, `--dry-run`, `--no-backup` (ignore les `.backup.*` horodatés avant écrasement).

- `check-markdown …`
**Pas d'API.** Analyse les fichiers Markdown/MDX situés sous le `contentPaths` de chaque bloc `documentations[]` (découverte identique à `translate-docs`, respecte `.translate-ignore`) : appariement des délimiteurs, code en ligne non fermé, et éléments `STRONG_OUTSIDE_INLINE_CODE` / `STRONG_OUTSIDE_LINK` lorsque `**`/`__` encadrent un élément `` `...` `` ou un lien `[text](../url)`. `-p` / `--path` ou `-f` / `--file` : portée facultative. Affiche les lignes `relativePath:line: [ISSUE_CODE] message` sur **stderr** ; code de sortie **1** s'il y a un problème. `--json` : rapport JSON sur **stdout**. Écrit `markdown_source_issues` dans `cacheDir` sauf si `--no-cache` est présent. `-v` ajoute les hachages des sources aux lignes stderr.

- `translate-svg …`
Traduit les fichiers SVG configurés dans `config.svg` (séparément de la documentation). Nécessite `features.translateSVG`. Mêmes principes de cache que pour la documentation ; prend en charge `--no-cache` pour ignorer les lectures/écritures SQLite lors de cette exécution. `-j`, `-b`, `--force`, `--force-update`, `-p` / `--path`, `--dry-run`.

- `translate-ui [--locale <code>] [--force] [--dry-run] [-j <n>]`
Traduit uniquement les chaînes d'interface. `--force` : retraduit toutes les entrées par langue (ignore les traductions existantes). `--dry-run` : aucune écriture, aucun appel API. `-j` : nombre maximal de langues en parallèle. Nécessite `features.translateUIStrings`.

- `lint-source [-l <code>] [--chunk <n>] [--dry-run] [--json] [-j <n>]`
Exécute `extract` **en premier** (nécessite `features.extractUIStrings`) afin que `strings.json` corresponde à la source, puis analyse par LLM des chaînes d'interface **source-langue** (orthographe, grammaire). Les **indices de terminologie** proviennent uniquement du fichier CSV `glossary.userGlossary` (même portée que `translate-ui` — pas `strings.json` / `uiGlossary`, afin de ne pas renforcer une mauvaise formulation comme entrée de glossaire). Utilise OpenRouter (`OPENROUTER_API_KEY`). À titre indicatif uniquement (code de sortie **0** à la fin de l'exécution). Écrit `lint-source-results_<timestamp>.log` sous `cacheDir` sous forme de rapport **lisible par un humain** (résumé, problèmes, et lignes **OK** par chaîne) ; le terminal affiche uniquement les comptages récapitulatifs et les problèmes (pas de lignes `[ok]` par chaîne). Affiche le nom du fichier journal sur la dernière ligne. `--json` : rapport JSON entièrement lisible par machine sur stdout uniquement (le fichier journal reste lisible par un humain). `--dry-run` : exécute tout de même `extract`, puis affiche uniquement le plan par lot (pas d'appels API). `--chunk` : nombre de chaînes par lot API (par défaut **50**). `-j` : nombre maximal de lots en parallèle (par défaut `concurrency`). Avec `--json`, la sortie au format humain est dirigée vers stderr. Les liens utilisent `path:line` comme le bouton « lien » des chaînes d'interface `editor`.

- `export-ui-xliff [-l <codes>] [-o <dir>] [--untranslated-only] [--dry-run]`
Exporte `strings.json` vers XLIFF 2.0 (un `.xliff` par langue cible). `-o` / `--output-dir` : répertoire de sortie (par défaut : même dossier que le catalogue). `--untranslated-only` : uniquement les unités manquantes d'une traduction pour cette langue. Lecture seule ; pas d'API.

- `sync …`
Extrait (si activé), puis traduction de l'interface, puis `translate-svg` lorsque `features.translateSVG` et `config.svg` sont définis, puis traduction de la documentation — sauf si sauté avec `--no-ui`, `--no-svg` ou `--no-docs`. Options partagées : `-l`, `-p` / `-f`, `--dry-run`, `-j`, `-b` (uniquement pour le regroupement de la documentation), `--force` / `--force-update` (documentation uniquement ; mutuellement exclusives lorsque la documentation est exécutée). La phase de documentation transmet également `--emphasis-placeholders` et `--debug-failed` (même signification que `translate-docs`). `--prompt-format` n'est pas un indicateur `sync` ; l'étape de documentation utilise la valeur par défaut intégrée (`json-array`).

- `status [--max-columns <n>]`
Lorsque `features.translateUIStrings` est activé, affiche la couverture de l'interface par langue (`Translated` / `Missing` / `Total`). Puis affiche l'état de traduction du Markdown par fichier × langue (pas de filtre `--locale` ; les langues proviennent de la configuration). Les listes importantes de langues sont divisées en plusieurs tableaux répétés d'au plus `n` colonnes de langues (par défaut **9**) afin que les lignes restent étroites dans le terminal.

- `statistics [--max-columns <n>]`
Affiche le cache de la documentation et les statistiques de `strings.json` (agrégats identiques à Éditeur du cache de traduction → **Statistiques**). `--max-columns` : nombre maximal de colonnes de langue par modèle × tableau de langue (par défaut, correspond à l'éditeur).

- `cleanup [--dry-run] [--no-backup] [--backup <path>]`
Exécute d'abord `sync --force-update` (extraction, interface utilisateur, SVG, documents), puis supprime les lignes de segments obsolètes (`last_hit_at` nul / chemin de fichier vide) ; supprime les lignes de `file_tracking` dont le chemin source résolu est manquant sur le disque ; supprime les lignes de traduction dont les métadonnées `filepath` pointent vers un fichier manquant. Affiche trois compteurs (obsolètes, `file_tracking` orphelins, traductions orphelines). Crée une sauvegarde SQLite horodatée dans le répertoire du cache, sauf si `--no-backup`.

- `clean-temp [-r|--root <path>] [-f|--force] [--dry-run]`
**Pas de configuration.** Parcourt l'arborescence d'un répertoire (par défaut : répertoire courant) à la recherche de `*.log` et de `cache.db.backup*.sqlite`, affiche les chemins `./…` comme `find -print`. S'il y a des correspondances : demande confirmation `Delete these files? (y/n)` sauf si `-f` / `--force` (suppression sans confirmation). S'il n'y a aucune correspondance : quitte sans demander confirmation. `--dry-run` : liste uniquement, pas de confirmation ni de suppression (remplace `--force`).

- `editor [-p <port>] [--no-open]`
Lance un éditeur web local pour le cache, `strings.json` et le fichier CSV du glossaire. Avec `--no-open`, le navigateur par défaut n'est pas ouvert automatiquement.  
...
Écrit un modèle `glossary-user.csv` vide. `-o` : remplace le chemin de sortie (par défaut : `glossary.userGlossary` depuis la configuration, ou `glossary-user.csv`).

Toutes les commandes acceptent `-c <path>` pour spécifier un fichier de configuration non par défaut, `-v` pour une sortie détaillée, et `-w` / `--write-logs [path]` pour rediriger la sortie console vers un fichier journal (chemin par défaut : sous `cacheDir` racine).

Le programme principal prend également en charge `-V` / `--version` et `-h` / `--help` ; `ai-i18n-tools help [command]` affiche la même aide par commande que `ai-i18n-tools <command> --help`.
