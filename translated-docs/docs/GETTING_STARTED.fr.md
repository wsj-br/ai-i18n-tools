...
- [Référence de configuration](#configuration-reference)
  ...
  - [`provider` et `providers`](#openrouter)
  - [`features`](#features)
  - [`ui`](#ui)
  - [`cacheDir`](#cachedir)
...
Définissez votre clé API fournisseur (OpenRouter affiché ; utilisez la variable d'environnement qui correspond à votre fournisseur actif — voir le [tableau des préréglages](#openrouter)) :
...
### Marquage du HTML pour la traduction
...
#### Exemple concret : localisation d'une application HTML simple (le tableau de bord intégré)
...
La partie de ce snippet qui parcourt les marqueurs est exactement `applyStaticI18n` dans [`src/dashboard-app/app.js`](../../docs/../src/dashboard-app/app.js). Comme le texte source anglais est la clé du catalogue, les chaînes non traduites reviennent automatiquement à l'anglais.

Différence avec le tableau de bord groupé : comme il dispose d'un serveur Node, il ne récupère pas un fichier statique `/locales/{locale}.json`. Le client appelle `GET /api/ui-i18n`, et le serveur résout la locale active (`--ui-lang` > `AI_I18N_LANG` > configuration `uiLanguage` > système d'exploitation hôte) et renvoie `{ locale, dir, bundle }`. Le client définit ensuite `document.documentElement` `lang`/`dir` à partir de cette réponse (plutôt que de lire `lang` pour choisir la locale) avant d'appeler `applyStaticI18n`. Les bundles eux-mêmes ne sont pas le contenu de l'outil à traduire — ce sont les chaînes d'interface utilisateur du tableau de bord, expédiées dans `src/i18n/locales/{locale}.json` (copiées dans `dist/i18n/locales` lors de la compilation) et lues côté serveur par `loadUiBundle` dans [`src/i18n/index.ts`](../../docs/../src/i18n/index.ts). Le `t()` du tableau de bord prend également en charge l'interpolation `{{name}}`, contrairement au `t` minimal ci-dessus.
...
`translate-docs` envoie des segments traduisibles au fournisseur LLM actif en **lots** (groupés par `batchSize` / `maxBatchChars`). Le drapeau `--prompt-format` ne modifie que le **format de transmission** de ce lot ; les jetons `PlaceholderHandler`, les vérifications de l'AST markdown, les clés de cache SQLite et le repli par segment en cas d'échec de l'analyse du lot restent inchangés.
...
Pour un exemple exécutable qui configure plusieurs fournisseurs dans une même configuration et bascule entre eux avec `-P`, consultez [`examples/multi-provider`](../../docs/../examples/multi-provider/) (`openai`, `anthropic`, `nvidia` et `deepseek` dans le même document).
...
Niveau identique à `docsOutput` (par bloc `docs[]`). Valeur par défaut `true` : traduire le texte YAML destiné à l'utilisateur pour Starlight/Docusaurus (`title`, `description`, `sidebar.label`, `sidebar_label`, `keywords`, `hero.title`, `hero.tagline`, `hero.image.alt`, `hero.actions[].text`, `pagination_label`, `prev`/`next` libellés). Définissez `false` pour conserver le bloc entier de front matter inchangé ; passez un tableau de chaînes pour limiter à des chemins pointés spécifiques.
- `segmentSplitting`
Niveau identique à `docsOutput` (par bloc `docs[]`). Segments facultatifs plus fins pour l'extraction `translate-docs` : `{ "enabled", "maxCharsPerSegment"?, "splitPipeTables"?, "splitDenseParagraphs"?, "maxLinesPerParagraphChunk"?, "splitLongLists"?, "maxListItemsPerChunk"?, "qualityRetrySplit"?, "maxQualityRetrySplitDepth"? }`. Lorsque `enabled` vaut `true` (par défaut quand `segmentSplitting` est omis), les paragraphes denses, les tableaux GFM avec délimiteurs (le premier fragment inclut l'en-tête, le séparateur et la première ligne de données) et les longues listes sont divisés ; les sous-parties sont réassemblées avec des sauts de ligne simples (`tightJoinPrevious`). Définissez `"enabled": false` pour utiliser un segment par bloc de texte séparé par une ligne vide uniquement. Lorsque `qualityRetrySplit` vaut `true` (par défaut), les segments Markdown qui échouent à la validation AST après épuisement de tous les modèles sont progressivement divisés et relancés à partir du premier modèle ; `maxQualityRetrySplitDepth` (par défaut `3`) limite les divisions récursives.
- `warnMarkdownSourceIssues`
Lorsque `true` (par défaut si omis), chaque exécution `translate-docs` analyse à nouveau les segments Markdown à la recherche de délimiteurs risqués ou de code en ligne non fermé, affiche des avertissements dans le terminal, et remplace les lignes `markdown_source_issues` pour le chemin de cache du fichier. Définissez `false` pour ignorer les avertissements et les mises à jour SQLite pour ce bloc.
- `addFrontmatter`
Lorsque `true` (par défaut si omis), les fichiers Markdown traduits incluent les clés YAML : `translation_last_updated`, `source_file_mtime`, `source_file_hash`, `translation_language`, `source_file_path`, et, lorsqu'au moins un segment possède des métadonnées de modèle, `translation_models` (liste triée des identifiants de modèles OpenRouter utilisés). Définissez sur `false` pour ignorer.
...
| `check-markdown …`                                                                                         | Analyse les fichiers Markdown/MDX présents dans le répertoire `contentPaths` de chaque bloc `docs[]` (découverte identique à `translate-docs`, respecte `.translate-ignore`) : appariement des délimiteurs, code en ligne non fermé, et `STRONG_OUTSIDE_LINK` lorsque `**`/`__` encadrent un lien `[text](../../docs/url)`. `-p` / `--path` ou `-f` / `--file` : portée facultative. Affiche des lignes `relativePath:line: [ISSUE_CODE] message` sur **stderr** ; code de sortie **1** s'il y a des problèmes. `--json` : rapport JSON sur **stdout**. Écrit `markdown_source_issues` dans `cacheDir` sauf si `--no-cache`. `-v` ajoute les hachages source aux lignes stderr.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `lint-source [-l <code>] [--chunk <n>] [--dry-run] [--json] [-j <n>]`                                      | Exécute `extract` **en premier** (nécessite `features.translateUIStrings`) pour que `strings.json` corresponde à la source, puis l'examen par LLM des chaînes d'interface utilisateur **dans la langue source** (orthographe, grammaire). Les **indices terminologiques** proviennent uniquement du CSV `glossary.userGlossary` (même portée que `translate-ui` — pas `strings.json` / `uiGlossary`, donc une mauvaise copie n'est pas renforcée en tant que glossaire). Utilise le fournisseur LLM actif (sa variable d'environnement API-key). Avis uniquement (sortie **0** lorsque l'exécution est terminée). Écrit `lint-source-results_<timestamp>.log` sous `cacheDir` sous forme de rapport **lisible par l'homme** (résumé, problèmes et lignes **OK** par chaîne) ; le terminal n'affiche que les décomptes de résumé et les problèmes (pas de lignes `[ok]` par chaîne). Affiche le nom du fichier journal sur la dernière ligne. `--json` : rapport JSON complet lisible par machine uniquement sur stdout (le fichier journal reste lisible par l'homme). `--dry-run` : exécute toujours `extract`, puis affiche uniquement le plan de lot (pas d'appels API). `--chunk` : chaînes par lot API (par défaut **50**). `-j` : lots parallèles maximum (par défaut `concurrency`). Avec `--json`, la sortie de style humain va sur stderr. Les liens utilisent `path:line` comme les chaînes d'interface utilisateur `dashboard` « lien ». |
...
| `cleanup [--dry-run] [--backup <path>]`                                                      | Exécute d'abord `sync --force-update` (extraction, UI, SVG, docs), puis supprime les lignes de segment obsolètes (`last_hit_at` nul / chemin de fichier vide) ; supprime les lignes `file_tracking` dont le chemin source résolu est manquant sur le disque ; supprime les lignes de traduction dont les métadonnées `filepath` pointent vers un fichier manquant ; élague les lignes `translation_failures` orphelines ; élague les lignes `markdown_source_issues` orphelines dont le chemin source résolu est manquant sur le disque. Enregistre cinq décomptes (segments obsolètes, `file_tracking` orphelins, traductions orphelines, échecs orphelins, problèmes de markdown orphelins). Aucune sauvegarde SQLite n'est effectuée, sauf si `--backup <path>` est passé, ce qui écrit une sauvegarde à ce chemin avant les modifications.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
