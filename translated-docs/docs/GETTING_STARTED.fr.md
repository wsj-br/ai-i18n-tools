...
- `write-heading-ids …`
Nécessite au moins un bloc `documentations[]`. Rassemble `.md` / `.mdx` sous `contentPaths` de chaque bloc (respecte `.translate-ignore`). Insère une ligne d’ancre HTML `<a id="slug"></a>` immédiatement **avant** chaque en-tête ATX plat (`#`) (saute les en-têtes à l’intérieur des blocs de code délimités) ; lorsqu’une ligne d’ancre est déjà présente, met à jour `id` si elle ne correspond plus au slug dérivé du texte actuel de l’en-tête. `-p` / `--path` ou `-f` / `--file` : limiter à un fichier ou répertoire relatif au projet. `--slug-style` : `github` (par défaut ; doctoc / anchor-markdown-header), `bitbucket`, `gitlab`, `pymdown`, `azure-devops`. Avec `pymdown`, facultatif `--pymdown-case`, `--pymdown-normalize`, `--pymdown-percent-encode` / `--no-pymdown-percent-encode`. `--dry-run` : afficher uniquement les modifications.

- `strip-md-bold-inline …`
Nécessite au moins un bloc `documentations[]`. Supprime les `**` autour du code en ligne dans `.md` / `.mdx` sous `contentPaths` de chaque bloc (respecte `.translate-ignore`). `-p` / `--path` ou `-f` / `--file`, `--dry-run`, `--no-backup` (ignorer les `.backup.*` horodatés avant écrasement).

- `check-markdown …`
Analyse les fichiers Markdown/MDX sous `contentPaths` de chaque bloc `documentations[]` (même découverte que `translate-docs`, respecte `.translate-ignore`) : appariement des délimiteurs, code en ligne non fermé, et `STRONG_OUTSIDE_INLINE_CODE` / `STRONG_OUTSIDE_LINK` lorsque `**`/`__` encadrent un élément `` `...` `` ou un lien `[text](../../docs/url)`. `-p` / `--path` ou `-f` / `--file` : portée facultative. Affiche les lignes `relativePath:line: [ISSUE_CODE] message` sur **stderr** ; code de sortie **1** s’il y a un problème. `--json` : rapport JSON sur **stdout**. Écrit `markdown_source_issues` dans `cacheDir` sauf si `--no-cache`. `-v` ajoute les hachages sources aux lignes stderr.
...
Affiche le cache de documentation et les statistiques `strings.json` (mêmes agrégats que Translation Dashboard → **Statistics**). `--max-columns` : nombre maximal de colonnes par locale dans chaque tableau modèle × locale (par défaut, correspond au tableau de bord).
...
Lance le Translation Dashboard (interface web locale pour les segments du cache, `strings.json`, le glossaire, les échecs et les statistiques). Avec `--no-open`, le navigateur par défaut n'est pas ouvert automatiquement. L'alias obsolète `editor` fonctionne encore mais affiche un avertissement.
