...
Las aplicaciones HTML sencillas siguen el mismo modelo de catálogo con atributos de marcador en lugar de llamadas a `t()`; consulta [GETTING_STARTED — Marcado de HTML para traducción](GETTING_STARTED.es.md#marking-html-for-translation).
...
- **Resolución de locale** (`resolveUiLocale` en `src/core/ui-locale.ts`): selecciona el locale de la interfaz de usuario desde `-L` / `--ui-lang` > `AI_I18N_LANG` > configuración `uiLanguage` > locale del sistema operativo anfitrión (`Intl.DateTimeFormat().resolvedOptions().locale`). El candidato se normaliza y se compara con el conjunto de bundles enviado exactamente o por variación más cercana (por ejemplo, `pt-PT` → `pt-BR`, `en-US` → `en-GB`), y se vuelve al locale de origen (`en-GB`). La CLI se resuelve una vez antes de que se construya la ayuda (análisis de argv pre-parse) y nuevamente después de cargar la configuración para que se aplique `uiLanguage` (la bandera y la variable de entorno todavía ganan).
- **Tiempo de ejecución** (`src/i18n/index.ts`): un `t(source, vars)` mínimo con interpolación `{{name}}`, claveado por la cadena de origen en inglés contra bundles planos por locale en `src/i18n/locales/<code>.json` (copiados a `dist/i18n/locales` en el momento de la compilación). Las claves o bundles que faltan devuelven el texto de origen. Este es el mismo modelo de clave como valor predeterminado que el flujo de trabajo 1 — no hay búsqueda de hash.
- **Panel**: el servidor expone `GET /api/ui-i18n` que devuelve `{ locale, dir, bundle }` para el locale de la interfaz de usuario resuelto; el frontend establece `<html lang>` / `dir` y localiza la marca estática a través de atributos `data-i18n*`.
- **Dogfooding**: los bundles se producen ejecutando el pipeline de extracción → `translate-ui` del propio paquete contra `ai-i18n-self.config.json` (`pnpm i18n:self`). Las claves del catálogo provienen de llamadas `t()` en `src/cli/` y `src/i18n/` más los marcadores `data-i18n*` del panel en `src/dashboard-app/index.html`.
