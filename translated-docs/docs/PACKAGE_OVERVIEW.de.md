...
- **Locale-Auflösung** (`resolveUiLocale` in `src/core/ui-locale.ts`): Wählt die UI-Locale aus `-L` / `--ui-lang` > `AI_I18N_LANG` > Konfiguration `uiLanguage` > Host-Betriebssystem-Locale (`Intl.DateTimeFormat().resolvedOptions().locale`). Der Kandidat wird normalisiert und exakt oder mit der nächstgelegenen Variante mit dem ausgelieferten Bundle-Set abgeglichen (z. B. `pt-PT` → `pt-BR`, `en-US` → `en-GB`), wobei auf die Quell-Locale (`en-GB`) zurückgegriffen wird. Die CLI wird einmal vor dem Erstellen der Hilfe (Scan vor dem Parsen von argv) und erneut nach dem Laden der Konfiguration aufgelöst, sodass `uiLanguage` gilt (das Flag und die Umgebungsvariable haben immer Vorrang).
- **Laufzeit** (`src/i18n/index.ts`): Ein minimales `t(source, vars)` mit `{{name}}`-Interpolation, das über den englischen Quelltext mit flachen pro-Locale-Bundles in `src/i18n/locales/<code>.json` (zur Build-Zeit nach `dist/i18n/locales` kopiert) indiziert wird. Fehlende Schlüssel oder Bundles geben den Quelltext zurück. Dies ist dasselbe Key-as-Default-Modell wie Workflow 1 – es gibt keine Hash-Suche.
- **Dashboard**: Der Server stellt `GET /api/ui-i18n` bereit, das `{ locale, dir, bundle }` für die aufgelöste UI-Locale zurückgibt; das Frontend setzt `<html lang>` / `dir` und lokalisiert statisches Markup über `data-i18n*`-Attribute.
- **Dogfooding**: Die Bundles werden durch Ausführen der eigenen Pipeline des Pakets extract → `translate-ui` gegen `ai-i18n-self.config.json` (`pnpm i18n:self`) erzeugt. Katalogschlüssel stammen aus `t()`-Aufrufen über `src/cli/` und `src/i18n/` sowie aus den `data-i18n*`-Markern des Dashboards in `src/dashboard-app/index.html`.
