...
ダッシュボードUIが変更されたときに、[`scripts/screenshot-translation-dashboard.sh`](../../docs/../scripts/screenshot-translation-dashboard.sh) を使用してPNGを更新してください。この画像はロケールごとには変わりません。
...
つまり、`flatPreserveRelativeDir: true` を使用すると、サブディレクトリ内のソースファイルは自動的に正しいプレフィックスを取得します。たとえば、`docs/GETTING_STARTED.md` は `translated-docs/docs/GETTING_STARTED.<locale>.md` に出力されます。ファイルごとのプレフィックスは `../../docs/` であるため、アセット `translation-dashboard.png`（ソースからの相対パス）は `../../docs/translation-dashboard.png` になります。これは `translated-docs/docs/` から `docs/translation-dashboard.png` へ正しく解決されます。
