...
Das veröffentlichte Paket ist **nur ESM**. Verwenden Sie `import`/`import()` in Node.js oder Ihrem Bundler; verwenden Sie nicht `require('ai-i18n-tools')`. Das Paket deklariert `engines.node` `>=22.16.0`; ältere Node.js-Versionen werden nicht unterstützt.
...
Entwickelt für Markdown-Dokumentation, Docusaurus-Websites und JSON-Label-Dateien. Für in Markdown eingebettete PNG- und andere Rasterbilder siehe [Bilder und Raster-Assets in übersetzten Dokumenten](#images-and-raster-assets-in-translated-docs). Eigenständige SVG-Assets werden über [`translate-svg`](#cli-reference) übersetzt, wenn `features.translateSVG` aktiviert ist und der `svg`-Block auf oberster Ebene gesetzt ist – nicht über `documentations[].contentPaths`.
...
- Segmentzeilen sind global nach `(source_hash, locale)` gekennzeichnet (Hash = normalisierter Inhalt). Identischer Text in zwei Dateien teilt sich eine Zeile; `translations.filepath` ist Metadaten (letzter Schreibender), keine zweite Cache-Einträge pro Datei.
- `file_tracking.filepath` verwendet namensräumlich getrennte Schlüssel: `doc-block:{index}:{relPath}` pro `documentations`-Block (`relPath` ist posix-Pfad relativ zum Projektstamm: gesammelte Markdown-Pfade; **JSON-Bezeichnungsdateien verwenden den zum aktuellen Arbeitsverzeichnis relativen Pfad der Quelldatei**, z. B. `docs-site/i18n/en/code.json`, sodass die Bereinigung die echte Datei auflösen kann) und `svg-assets:{relPath}` für eigenständige SVG-Ressourcen unter `translate-svg`.
- `translations.filepath` speichert zum aktuellen Arbeitsverzeichnis relative posix-Pfade für Markdown-, JSON- und SVG-Segmente (SVG verwendet dieselbe Pfadstruktur wie andere Ressourcen; das Präfix `svg-assets:…` steht **nur** bei `file_tracking`).
- Nach einem Durchlauf wird `last_hit_at` nur für Segmentzeilen gelöscht, die **im gleichen Übersetzungsbereich** liegen (unter Berücksichtigung von `--path` und aktivierten Typen) und nicht erreicht wurden, sodass ein gefilterter oder nur für Dokumentation durchgeführter Durchlauf keine nicht betroffenen Dateien als veraltet markiert.
...
**SVG**, die als illustrierte Assets gedacht sind, verwenden den `svg`-Block und `translate-svg` – siehe [`svg` (optional)](#svg-optional). In `documentations[].contentPaths` aufgeführte Pfade betreffen Markdown/MDX (und optionale JSON-Labels), nicht die Übersetzung eigenständiger SVG-Dateien.
...
**Muster 3 – Eigenständige SVG** (`examples/nextjs-app`)
...
| `requestTimeoutMs` | Maximale Wartezeit in Millisekunden für jede HTTP-Anfrage an OpenRouter (Chat-Vervollständigungen und interne `GET /models`-Aufrufe). Standard: `30000` (30 Sekunden). |
...
| `warnMarkdownSourceIssues`                     | Wenn `true` (Standard, wenn nicht angegeben), scannt jeder `translate-docs`-Lauf Markdown-Abschnitte erneut nach riskanten Trennzeichen / nicht geschlossenen Inline-Codes, gibt Terminalwarnungen aus und ersetzt `markdown_source_issues`-Zeilen für den Cache-Dateipfad dieser Datei. Setzen Sie `false`, um Warnungen und SQLite-Aktualisierungen für diesen Block zu überspringen.                                                                                                                                                                                                                                                                                                                                                           |
...
| `check-models` | Überprüft jede konfigurierte OpenRouter-Modell-ID gegenüber `GET /models` (Katalogzugehörigkeit, `expiration_date`, USD pro 1 Million Tokens für Prompt/Completion). Erfordert `OPENROUTER_API_KEY`. Beendet mit einem Nicht-Null-Status, wenn eine konfigurierte ID fehlt oder abgelaufen ist. Berücksichtigt `openrouter.requestTimeoutMs` für die Kataloganfrage. |
...
| `strip-md-bold-inline …`                                                    | **Keine API.** Erfordert mindestens einen `documentations[]`-Block. Entfernt `**` um Inline-Code in `.md` / `.mdx` unter jedem `contentPaths` des Blocks (beachtet `.translate-ignore`). `-p` / `--path` oder `-f` / `--file`, `--dry-run`, `--no-backup` (überspringt zeitgestempelte `.backup.*` vor dem Überschreiben).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `check-markdown …`                                                            | **Keine API.** Durchsucht Markdown/MDX unterhalb des `documentations[]`-Blocks `contentPaths` (gleiche Auffindung wie bei `translate-docs`, berücksichtigt `.translate-ignore`): Paarung von Delimitern, nicht geschlossener Inline-Code und `STRONG_OUTSIDE_INLINE_CODE` / `STRONG_OUTSIDE_LINK`, wenn `**`/`__` einen `` `...` ``-Abschnitt oder einen `[text](../url)`-Link umschließen. `-p` / `--path` oder `-f` / `--file`: optionaler Bereich. Gibt `relativePath:line: [ISSUE_CODE] message` Zeilen auf **stderr** aus; Exit-Code **1**, falls ein Problem vorliegt. `--json`: JSON-Bericht auf **stdout**. Schreibt `markdown_source_issues` in `cacheDir`, es sei denn `--no-cache`. `-v` fügt Quell-Hashes zu stderr-Zeilen hinzu.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `statistics [--max-columns <n>]`                                             | Gibt Cache-Dokumentation und `strings.json`-Statistiken aus (gleiche Aggregate wie Übersetzungs-Cache-Editor → **Statistiken**). `--max-columns`: maximale Locale-Spalten pro Modell × Locale-Tabelle (Standard entspricht dem Editor).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `clean-temp [-r|--root <path>] [-f|--force] [--dry-run]`                     | **Keine Konfiguration.** Durchläuft eine Verzeichnisstruktur (Standard: cwd) nach `*.log` und `cache.db.backup*.sqlite`, gibt `./…` Pfade wie `find -print` aus. Bei Funden: fragt `Delete these files? (y/n)` ab, sofern nicht `-f` / `--force` angegeben ist (löscht ohne Nachfrage). Bei keiner Übereinstimmung: Beendigung ohne Nachfrage. `--dry-run`: nur Auflisten, keine Nachfrage oder Löschungen (übersteuert `--force`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `editor [-p <port>] [--no-open]`                                            | Startet einen lokalen Web-Editor für den Cache, `strings.json` und die Glossar-CSV-Datei. Mit `--no-open` wird der Standardbrowser nicht automatisch geöffnet.<br><br>**Hinweis:** Wenn Sie einen Eintrag im Cache-Editor bearbeiten, müssen Sie einen `sync --force-update` ausführen, um die Ausgabedateien mit dem aktualisierten Cache-Eintrag neu zu schreiben. Außerdem geht die manuelle Bearbeitung verloren, wenn sich der Quelltext später ändert, da ein neuer Cache-Schlüssel generiert wird.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
