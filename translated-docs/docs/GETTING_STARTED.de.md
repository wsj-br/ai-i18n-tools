...
- [Konfigurationsreferenz](#configuration-reference)
  ...
  - [`provider` und `providers`](#openrouter)
  - [`features`](#features)
  - [`ui`](#ui)
  - [`cacheDir`](#cachedir)
...
Legen Sie Ihren Provider-API-Schlüssel fest (OpenRouter angezeigt; verwenden Sie die Umgebungsvariable, die Ihrem aktiven Provider entspricht — siehe [Preset-Tabelle](#openrouter)):
...
### HTML für die Übersetzung markieren
...
#### Ausführungsbeispiel: Lokalisierung einer reinen HTML-App (das gebündelte Dashboard)
...
Die Marker-durchlaufende Hälfte dieses Snippets ist genau `applyStaticI18n` in [`src/dashboard-app/app.js`](../../docs/../src/dashboard-app/app.js). Da der englische Quelltext der Katalogschlüssel ist, fallen nicht übersetzte Zeichenketten automatisch auf Englisch zurück.

Unterschiede im gebündelten Dashboard: Da es einen Node-Server hat, ruft es keine statische `/locales/{locale}.json` ab. Der Client ruft `GET /api/ui-i18n` auf, und der Server ermittelt das aktive Gebietsschema (`--ui-lang` > `AI_I18N_LANG` > Konfiguration `uiLanguage` > Host-Betriebssystem) und gibt `{ locale, dir, bundle }` zurück. Der Client setzt dann `document.documentElement` `lang`/`dir` aus dieser Antwort (anstatt `lang` zu lesen, um das Gebietsschema auszuwählen), bevor er `applyStaticI18n` aufruft. Die Bundles selbst sind nicht der Inhalt des Tools, der übersetzt wird — sie sind die UI-Zeichenketten des Dashboards selbst, die in `src/i18n/locales/{locale}.json` ausgeliefert (beim Build nach `dist/i18n/locales` kopiert) und serverseitig von `loadUiBundle` in [`src/i18n/index.ts`](../../docs/../src/i18n/index.ts) gelesen werden. Das `t()` des Dashboards unterstützt auch `{{name}}`-Interpolation, im Gegensatz zum minimalen `t` oben.
...
`translate-docs` sendet übersetzbare Segmente in **Batches** (gruppiert nach `batchSize` / `maxBatchChars`) an den aktiven LLM-Provider. Das Flag `--prompt-format` ändert nur das **Drahtformat** dieses Batches; `PlaceholderHandler`-Token, Markdown-AST-Prüfungen, SQLite-Cache-Schlüssel und pro-Segment-Fallback, wenn die Batch-Analyse fehlschlägt, bleiben unverändert.
...
BCP-47-Code für die Benutzeroberflächensprache des Tools (CLI-Hilfe, Log-/Zusammenfassungsnachrichten und Übersetzungs-Dashboard). Er ist unabhängig von `sourceLocale` / `targetLocales` und wird durch die `-L` / `--ui-lang`-Flag und die `AI_I18N_LANG`-Umgebungsvariable überschrieben. Unbekannte Werte degradieren sanft zum Quellsprachlocale (`en-GB`) – es gibt keine strenge Validierung. Siehe [Benutzeroberflächensprache des Tools](#tool-ui-language).
...
Für ein lauffähiges Beispiel, das mehrere Anbieter in einer Konfiguration einrichtet und mit `-P` zwischen ihnen wechselt, siehe [`examples/multi-provider`](../../docs/../examples/multi-provider/) (`openai`, `anthropic`, `nvidia` und `deepseek` im selben Dokument).
...
| `lint-source [-l <code>] [--chunk <n>] [--dry-run] [--json] [-j <n>]`                                      | Führt `extract` **zuerst** aus (erfordert `features.translateUIStrings`), damit `strings.json` mit der Quelle übereinstimmt, gefolgt von einer LLM-Überprüfung der UI-Strings in der **Quellsprache** (Rechtschreibung, Grammatik). **Terminhinweise** stammen ausschließlich aus der `glossary.userGlossary`-CSV (gleicher Umfang wie `translate-ui` – nicht `strings.json` / `uiGlossary`, sodass schlechte Kopien nicht als Glossar verstärkt werden). Verwendet den aktiven LLM-Anbieter (dessen API-Schlüssel-Umgebungsvariable). Nur beratend (Beendet mit **0**, wenn der Lauf abgeschlossen ist). Schreibt `lint-source-results_<timestamp>.log` unter `cacheDir` als **menschenlesbaren** Bericht (Zusammenfassung, Probleme und Zeilen mit **OK** pro String); das Terminal gibt nur Zusammenfassungsanzahlen und Probleme aus (keine `[ok]`-Zeilen pro String). Gibt den Log-Dateinamen in der letzten Zeile aus. `--json`: vollständiger maschinenlesbarer JSON-Bericht nur auf stdout (die Log-Datei bleibt menschenlesbar). `--dry-run`: führt immer noch `extract` aus, gibt dann nur den Batch-Plan aus (keine API-Aufrufe). `--chunk`: Strings pro API-Batch (Standard **50**). `-j`: maximale parallele Batches (Standard `concurrency`). Mit `--json` geht die menschenähnliche Ausgabe an stderr. Links verwenden `path:line` wie die UI-Strings „Link“-Schaltfläche von `dashboard`. |
...
| `cleanup [--dry-run] [--backup <path>]`                                                      | Führt zuerst `sync --force-update` aus (Extraktion, UI, SVG, Dokumente), entfernt dann veraltete Segmentzeilen (Null-`last_hit_at` / leerer Dateipfad); löscht `file_tracking`-Zeilen, deren aufgelöster Quellpfad auf der Festplatte fehlt; entfernt Übersetzungszeilen, deren `filepath`-Metadaten auf eine fehlende Datei verweisen; bereinigt verwaiste `translation_failures`-Zeilen; bereinigt verwaiste `markdown_source_issues`-Zeilen, deren aufgelöster Quellpfad auf der Festplatte fehlt. Protokolliert fünf Zählungen (veraltete Segmente, verwaiste `file_tracking`, verwaiste Übersetzungen, verwaiste Fehler, verwaiste Markdown-Probleme). Es wird kein SQLite-Backup erstellt, es sei denn, `--backup <path>` wird übergeben, wodurch vor den Änderungen ein Backup in diesen Pfad geschrieben wird.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `-L` / `--ui-lang <code>`    | Jeder Befehl | Sprache für die Benutzeroberfläche des Tools (CLI-Hilfe, Log-/Zusammenfassungsnachrichten, Dashboard); höchste Priorität als Quelle. Siehe [Benutzeroberflächensprache des Tools](#tool-ui-language). |
