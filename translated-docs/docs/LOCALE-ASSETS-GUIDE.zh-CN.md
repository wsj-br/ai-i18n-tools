...
| 配置路径 | 值 | 使用场景 | 说明 |
|-------------|-------|----------|-------|
| `docs[].docsOutput.style` | `"flat"` | 带区域设置后缀的 README / USER-GUIDE 文件 | 启用扁平链接重写器；当源文件位于子目录时，需与 `flatPreserveRelativeDir` 配合使用 |
| `docs[].docsOutput.style` | `"nested"`（默认） | 在 `outputDir` 下的简单区域设置子文件夹 | 不启用扁平链接重写器 |
| `docs[].docsOutput.style` | `"doc-system"` | 区域设置前缀的文档树（自定义生成器） | 设置 `docsRoot` 和 `localeSubpath`；不运行扁平链接重写器 |
| `docs[].docsOutput.style` | `"docusaurus"` / `"astro-starlight"` | 预设 `doc-system` 布局 | 与生成器特定的默认值别名对应 `localeSubpath` |
...
本指南使用配置中的确切 JSON 字符串——而不仅仅是英文单词——以确保翻译版本无歧义。加载时仍支持旧版键（`documentations`、`markdownOutput`）；但在新配置中应优先使用 `docs` 和 `docsOutput`。
...
- [ai-i18n-tools 对资源的操作（及不操作的内容）](#what-ai-i18n-tools-does-and-does-not-do-with-assets)
- [从一开始就为国际化设计](#design-for-i18n-from-the-start)
  ...
  - [文档系统网站（`docsOutput.style = "doc-system"`）](#doc-system-sites-docsoutputstyle--doc-system)
...
- [模式 D - 使用 `svg.style = "flat"` 翻译 SVG](#pattern-d---translated-svg-with-svgstyle--flat)
  - [配置](#config-1)
  - [应用参考](#app-reference)
...
如果初始路径省略了区域设置子目录（`images/screenshots/translate.png`），则在使用模式 B 之前需要重构整个目录树。

<a id="doc-system-sites-docsoutputstyle--doc-system"></a>
### 文档系统网站（`docsOutput.style = "doc-system"`）
...
将 `docs[].docsOutput.docsRoot` 设置为英文源文件根目录（例如 `"docs"` 或 `"src/content/docs"`）。当直接设置 `style: "doc-system"` 时，还必须将 `localeSubpath` 设置为网站在 `{locale}/` 和翻译文件之间使用的路径段。别名 `"docusaurus"` 和 `"astro-starlight"` 是预设的 `doc-system` 布局，带有默认的 `localeSubpath` 值（参见 [输出布局](GETTING_STARTED.zh-CN.md#output-layouts)）。
...
**模式 B** 适用于截图位于共享静态 URL 树的情况：从第一天起就使用带区域设置编码的文件夹，并使用一条通用的 `screenshots/[^/]+/` → `screenshots/${translatedLocale}/` 规则（参见 [配置 — 文档系统](#config---docsoutputstyle--doc-system)）。
...
当你后续添加国际化支持时，截图脚本会采用 `getScreenshotDir` 分割方式（参见 [模式 C](#pattern-c---colocated-raster-doc-system)），而 `translate-svg` 使用 `pathTemplate`。无需调整正则表达式。
...
| 模式 | 资源类型                  | 网站类型                                                                 | 工具机制                                               |
|---------|-----------------------------|---------------------------------------------------------------------------|--------------------------------------------------------------|
| A       | 光栅图（共享）             | `docsOutput.style = "flat"` 文档                                      | 每文件链接重写器；通常无需正则表达式                     |
...
当单个图像在所有语言区域（locale）之间共享时使用（无特定于语言区域的变体）。当启用 `docsOutput.style = "flat"` 时，扁平链接重写器会为每个输出文件计算深度前缀，因此与源文件位于同一目录的资源（例如 `docs/figure.png`，在 `docs/page.md` 中引用为 `figure.png`）在每种翻译输出中都能正确解析——无需配置 `postProcessing.regexAdjustments` 规则。
...
本仓库对翻译仪表板截图使用了模式 A：[GETTING_STARTED.md](GETTING_STARTED.zh-CN.md#translation-dashboard) 引用了同目录下的图像 [translation-dashboard.png](../../docs/../docs/translation-dashboard.png)。[ai-i18n-tools.config.json](../../docs/../ai-i18n-tools.config.json) 中设置了 `docsOutput.style = "flat"` 和 `flatPreserveRelativeDir: true`；通过每个文件的深度前缀，图像路径被正确解析，无需针对截图配置 `regexAdjustments`。

---

<a id="pattern-b---per-locale-folder-url-rewriting"></a>
## 模式 B - 按语言区域分文件夹（URL 重写）

适用于包含 `docsOutput.style = "flat"` 的 README/用户指南，以及从共享静态 URL 树提供截图的文档系统网站（`docsOutput.style = "doc-system"` 或别名 `"docusaurus"` / `"astro-starlight"`）。
...
参见 [examples/nextjs-app] 中的[截图脚本](../../docs/../examples/nextjs-app/scripts/screenshot-locales.sh) 中一个简单的 `bash` 示例，或 [Transrewrt 项目](https://github.com/wsj-br/transrewrt) 仓库中的 [take-screenshots.js](https://github.com/wsj-br/duplistatus/blob/master/scripts/take-screenshots.ts) 中更复杂的示例。

> **注意：** 以下四个子章节共享相同的 `regexAdjustments` 语言区域段替换（`screenshots/[^/]+/` → `screenshots/${translatedLocale}/`）。仅输出布局以及是否先运行扁平链接重写器有所不同——请跳转到与您的 `docsOutput.style` 匹配的子章节。

<a id="config---docsoutputstyle--flat"></a>
### 配置 - `docsOutput.style = "flat"`

当启用 `docsOutput.style = "flat"` 时，扁平链接重写器优先运行，并为非 Markdown 的 URL 添加深度前缀。对于仓库根目录下的 `README.md`，若配置为 `outputDir: "translated-docs/"`，则会添加 `../`：
...
实现示例：[examples/nextjs-app/docs-site/docs/feature-showcase.md](../../docs/../examples/nextjs-app/docs-site/docs/feature-showcase.md)（`/img/screenshots/en-GB/screenshot.png`），配合 [ai-i18n-tools.config.json](../../docs/../examples/nextjs-app/ai-i18n-tools.config.json) 中的第一个 `docs[]` 块。
...
## 模式 C - 共置光栅图像（`doc-system`）

当 `doc-system` 站点将特定于语言区域的资源与翻译后的 Markdown 文件共置时使用——无需 URL 重写。Docusaurus 预设（`docsOutput.style = "docusaurus"`）是参考实现；其他使用 `"doc-system"` 并自定义 `localeSubpath` 的生成器也遵循相同思路：英文资源位于源语言区域路径，翻译后的资源位于 `{outputDir}/{locale}/[localeSubpath/]assets/` 下。
...
参见 [duplistatus](https://github.com/wsj-br/duplistatus) 仓库中 [take-screenshots.ts](https://github.com/wsj-br/duplistatus/blob/master/scripts/take-screenshots.ts) 的生产实现（本地参考副本：[references/duplistatus/scripts/take-screenshots.ts](../../docs/../references/duplistatus/scripts/take-screenshots.ts)）。
...
[duplistatus](https://github.com/wsj-br/duplistatus) — [take-screenshots.ts](https://github.com/wsj-br/duplistatus/blob/master/scripts/take-screenshots.ts) 中的 `getScreenshotDir(locale)`；英文文档引用共置的 PNG 文件（例如 [dashboard.md](../../docs/../references/duplistatus/documentation/docs/user-guide/dashboard.md) 中包含 `../assets/screen-dashboard-summary.png`）；[ai-i18n-tools.config.json](../../docs/../references/duplistatus/ai-i18n-tools.config.json) 中无 PNG `regexAdjustments`。该项目中相同的 Pattern E SVG 文件也位于相同的 `current/assets/` 目录中（见下文）。

---

<a id="pattern-d---translated-svg-with-svgstyle--flat"></a>
## 模式 D - 带有 `svg.style = "flat"` 的翻译 SVG
...
对于 `docsOutput.style = "flat"`（除非设置了 `rewriteRelativeLinks: false` 或自定义的 `pathTemplate`），在 `postProcessing` 之前会运行一个内置的重写器。它会处理文档间链接（添加区域设置后缀）并对非 Markdown 资源 URL 添加深度前缀。

<a id="two-step-flow-when-docsoutputstyle--flat"></a>
### 当 `docsOutput.style = "flat"` 时的两步流程
...
对于 `docsOutput.style = "doc-system"`（包括 `"docusaurus"`、`"astro-starlight"` 和 `"nested"`），扁平链接重写器不会运行。`postProcessing` 将看到来自已翻译 Markdown 的原始 URL（通常为类似 `/img/screenshots/en-GB/foo.png` 的绝对路径）。
...
| 选项                                   | 效果                                                                                                           |
|------------------------------------------|------------------------------------------------------------------------------------------------------------------|
| `docsOutput.rewriteRelativeLinks`    | 显式启用或禁用扁平链接重写器（当 `docsOutput.style = "flat"` 时覆盖默认值） |
| `docsOutput.linkRewriteDocsRoot`     | 计算 `depthPrefix` 的根路径（默认为 `"."`）                                                        |
