...
用于工具自身 UI 语言（CLI 帮助、日志/摘要和翻译仪表板）的 BCP-47 代码。它独立于 `sourceLocale` / `targetLocales`，并被 `-L` / `--ui-lang` 标志和 `AI_I18N_LANG` 环境变量覆盖。未知值会优雅地降级为源区域设置（`en-GB`）——没有严格的验证。请参阅 [工具 UI 语言](#tool-ui-language)。
...
| `-L` / `--ui-lang <code>`    | 所有命令 | 工具自身 UI 的语言（CLI 帮助、日志/摘要、仪表板）；优先级最高的源。请参阅 [工具 UI 语言](#tool-ui-language)。 |
