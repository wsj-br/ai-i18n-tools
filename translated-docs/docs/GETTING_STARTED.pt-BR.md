...
Projetado para documentação em markdown, sites Docusaurus e arquivos de rótulos JSON. Para imagens PNG e outros ativos raster incorporados em markdown, consulte [Imagens e ativos raster em documentos traduzidos](#images-and-raster-assets-in-translated-docs). Ativos SVG autônomos são traduzidos por meio de [`translate-svg`](#cli-reference) quando `features.translateSVG` está habilitado e o bloco `svg` no nível superior está definido — não por meio de `documentations[].contentPaths`.
...
- `translations.filepath` armazena caminhos posix relativos ao diretório atual para segmentos markdown, JSON e SVG (SVG usa o mesmo formato de caminho que outros ativos; o prefixo `svg-assets:…` está presente **apenas** em `file_tracking`).
- Após uma execução, `last_hit_at` é limpo apenas para linhas de segmento **no mesmo escopo de tradução** (respeitando `--path` e tipos habilitados) que não foram atingidos, portanto, uma execução filtrada ou apenas de documentação não marca arquivos não relacionados como obsoletos.
...
**SVG** destinado como ativos ilustrados usa o bloco `svg` e `translate-svg` — veja [`svg` (opcional)](#svg-optional). Os caminhos listados em `documentations[].contentPaths` são para markdown/MDX (e rótulos JSON opcionais), não para tradução de SVG autônomo.
...
| `requestTimeoutMs` | Tempo máximo em milissegundos para aguardar cada requisição HTTP ao OpenRouter (conclusões de chat e chamadas internas `GET /models`). Padrão: `30000` (30 segundos).                                                      |
...
| `warnMarkdownSourceIssues`                     | Quando `true` (padrão quando omitido), cada execução `translate-docs` reanalisa segmentos de markdown em busca de delimitadores arriscados / códigos embutidos não fechados, exibe avisos no terminal e substitui linhas `markdown_source_issues` para o caminho de cache desse arquivo. Defina `false` para ignorar avisos e atualizações do SQLite para este bloco.                                                                                                                                                                                                                                                                                                                                                           |
...
| `check-models`                                                              | Valida cada ID de modelo OpenRouter configurado contra `GET /models` (adesão ao catálogo, `expiration_date`, USD por 1 milhão de tokens para prompt/conclusão). Requer `OPENROUTER_API_KEY`. Encerra com status não zero se algum ID configurado estiver ausente ou expirado. Respeita `openrouter.requestTimeoutMs` para a requisição do catálogo.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `strip-md-bold-inline …`                                                    | **Sem API.** Exige pelo menos um bloco `documentations[]`. Remove `**` ao redor de código embutido em `.md` / `.mdx` sob o `contentPaths` de cada bloco (respeita `.translate-ignore`). `-p` / `--path` ou `-f` / `--file`, `--dry-run`, `--no-backup` (ignora `.backup.*` com carimbo de data antes da substituição).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `check-markdown …`                                                            | **Sem API.** Analisa markdown/MDX em cada pasta `documentations[]` do bloco `contentPaths` (mesma descoberta que `translate-docs`, respeita `.translate-ignore`): pares de delimitadores, código embutido não fechado e `STRONG_OUTSIDE_INLINE_CODE` / `STRONG_OUTSIDE_LINK` quando `**`/`__` envolvem um trecho `` `...` `` ou um link `[text](../url)`. `-p` / `--path` ou `-f` / `--file`: escopo opcional. Imprime linhas `relativePath:line: [ISSUE_CODE] message` no **stderr**; código de saída **1** se houver algum problema. `--json`: relatório JSON no **stdout**. Grava `markdown_source_issues` em `cacheDir` exceto se `--no-cache`. `-v` adiciona hashes de origem às linhas no stderr.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `statistics [--max-columns <n>]`                                             | Exibe cache de documentação e estatísticas de `strings.json` (agregações iguais às do Editor de Cache de Tradução → **Estatísticas**). `--max-columns`: número máximo de colunas de localidade por tabela modelo × localidade (padrão igual ao do editor).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `clean-temp [-r|--root <path>] [-f|--force] [--dry-run]`                     | **Sem configuração.** Percorre a árvore de diretórios (padrão: diretório atual) em busca de `*.log` e `cache.db.backup*.sqlite`, exibe caminhos `./…` como em `find -print`. Com correspondências: exibe prompt `Delete these files? (y/n)` exceto se `-f` / `--force` (exclui sem confirmação). Sem correspondências: encerra sem exibir prompt. `--dry-run`: lista apenas, sem prompt ou exclusões (substitui `--force`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `editor [-p <port>] [--no-open]`                                            | Inicia um editor web local para o cache, `strings.json` e o CSV de glossário. Com `--no-open`, o navegador padrão não é aberto automaticamente.<br><br>**Observação:** Se você editar uma entrada no editor de cache, deve executar um `sync --force-update` para reescrever os arquivos de saída com a entrada de cache atualizada. Além disso, se o texto de origem mudar posteriormente, a edição manual será perdida, pois uma nova chave de cache será gerada.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
