...
O pacote publicado é **apenas ESM**. Use `import`/`import()` no Node.js ou no seu empacotador; não use `require('ai-i18n-tools')`. O pacote declara **`engines.node` `>=22.16.0`**; versões antigas do Node.js não são suportadas.
...
**Exemplo mínimo somente README (`examples/console-app`)**
