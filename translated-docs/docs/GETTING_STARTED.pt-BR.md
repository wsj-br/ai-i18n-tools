...
- [Editor de Cache de Tradução](#translation-cache-editor)
  ...
  - [Problemas de Markdown (verificações estáticas)](#markdown-issues-static-checks)
...
O pacote publicado é **somente ESM**. Use `import`/`import()` no Node.js ou no seu empacotador; não use `require('ai-i18n-tools')`. O pacote declara `engines.node` `>=22.16.0`; versões antigas do Node.js não são suportadas.
...
O modelo padrão `init` (`ui-markdown`) habilita apenas a extração e tradução da **UI**. O modelo `ui-docusaurus` habilita a tradução de **documentos** (`translate-docs`). Use `sync` quando você quiser um único comando que execute extração, tradução da interface, tradução opcional de arquivos SVG e tradução de documentação de acordo com sua configuração.
...
Mantenha `localeLoaders` **alinhado com a configuração** derivando-os de `ui-languages.json` usando `makeLocaleLoadersFromManifest` (isso filtra `SOURCE_LOCALE` usando a mesma normalização que `makeLoadLocale`). Quando você adiciona um idioma a `targetLocales` e executa `generate-ui-languages`, o manifesto é atualizado e seus carregadores acompanham automaticamente a alteração — não é necessário manter um mapa codificado separadamente.
...
`setupKeyAsDefaultT(i18n, { stringsJson, sourcePluralFlatBundle? })` é a **recomendada** configuração para projetos ai-i18n-tools: ela aplica key-trim + fallback de interpolação <code>"{{var}}"</code> do idioma de origem (mesmo comportamento que o `wrapI18nWithKeyTrim` de nível inferior), opcionalmente mescla chaves com sufixo plural `translate-ui` `{sourceLocale}.json` via `addResourceBundle`, e então instala `wrapT` com conhecimento de plural a partir do seu `strings.json`. Esse arquivo agrupado deve ser o arquivo plano de plural para o seu idioma de origem **configurado** — o mesmo `sourceLocale` que em `ai-i18n-tools.config.json` e `SOURCE_LOCALE` no seu bootstrap i18n (veja Passo 4 acima). Omita `sourcePluralFlatBundle` apenas durante o bootstrap (mescle-o assim que `translate-ui` tiver emitido `{sourceLocale}.json`). `wrapI18nWithKeyTrim` sozinho está **obsoleto** para código de aplicação — use `setupKeyAsDefaultT` em vez disso.
...
Projetado principalmente para **documentação em markdown e MDX** sob `contentPaths` (as páginas que importam para os leitores). Em sites Docusaurus, você também pode traduzir **arquivos JSON de rótulos** gerados por `docusaurus write-translations` — eles contêm strings de tema, barra de navegação, rodapé e plugins (i18n do shell), distintos do conteúdo principal em `docs/`. Para imagens PNG e outros recursos raster inseridos em markdown, consulte [Imagens e recursos raster na documentação traduzida](#images-and-raster-assets-in-translated-docs). Arquivos SVG são traduzidos por meio de [`translate-svg`](#cli-reference) quando `features.translateSVG` está habilitado e o bloco `svg` de nível superior está definido — não por meio de `documentations[].contentPaths`.
...
Páginas de documentação (primárias):
...
`"flat"` — coloca os arquivos traduzidos ao lado do original com sufixo de idioma, ou em um subdiretório. Links relativos entre páginas são reescritos automaticamente.
...
1. Execute `ai-i18n-tools write-heading-ids` no seu `.md` de origem / `.mdx` antes de `translate-docs` (mesmo `documentations[]` / `contentPaths` de costume). Ele insere âncoras HTML explícitas na linha anterior a cada título, de modo que os valores `id` sejam compartilhados por todas as cópias traduzidas.
2. Aponte seus **links de âncora** em markdown para esses IDs estáveis, por exemplo, `[label](../other.md#section-id)`, onde `section-id` corresponde à âncora escrita pela ferramenta — e não apenas a um palpite baseado em palavras em inglês.
...
`translate-docs` traduz segmentos de markdown (incluindo o texto alternativo da imagem). Ele **não** copia arquivos raster (PNG, JPEG, WebP, GIF) para o seu diretório `outputDir` da documentação. Coloque os arquivos nos locais para onde as URLs reescritas apontam ou ajuste as URLs após a tradução (geralmente com `markdownOutput.postProcessing.regexAdjustments`).

**SVG** destinado a ativos ilustrados usa o bloco `svg` e `translate-svg` — veja [`svg`](#svg). Os caminhos listados em `documentations[].contentPaths` são para markdown/MDX (e rótulos JSON opcionais), não para tradução de arquivos SVG.

**Por que o layout plano geralmente precisa de correção**

Com `markdownOutput.style` `flat` e reescrita de link relativo padrão, os links entre páginas traduzidas são reescritos por localidade. Links para arquivos não markdown recebem um prefixo de profundidade para que permaneçam relativos a cada arquivo de saída (por exemplo, `figure.png` ao lado do arquivo original pode se tornar `../figure.png` no arquivo traduzido). Essa URL geralmente resolve apenas **dentro** do diretório de saída. A CLI não emite o arquivo binário ali, então os leitores encontram um arquivo ausente, a menos que você copie os ativos, os sirva em outro local ou reescreva o link. Aplique suas regras após a tradução: `postProcessing` é executado após a reassociação de segmentos e a reescrita de links planos (veja a linha `markdownOutput.postProcessing` em [Referência de configuração](#configuration-reference)).

**Padrão 1 — Ativo no mesmo repositório ao lado do código-fonte em inglês (este pacote)**

Este repositório traduz `docs/GETTING_STARTED.md` para `translated-docs/docs/GETTING_STARTED.<locale>.md`. O código-fonte usa uma imagem irmã, `translation-cache-editor.png`. A reescrita plana apontaria para `translated-docs/translation-cache-editor.png`, que nunca é escrita. O `ai-i18n-tools.config.json` raiz adiciona uma regra que corresponde à parte final estável da imagem em markdown (o segmento de URL `](…)`, não o texto alternativo traduzido) e redireciona de volta para `docs/`:
...
**Padrão 2 — Pastas de capturas de tela por localidade** (`examples/nextjs-app`)

O exemplo do Next.js usa dois blocos `documentations[]` em `examples/nextjs-app/ai-i18n-tools.config.json`.

- **Documentação Docusaurus** (`markdownOutput.style` `docusaurus`): páginas em inglês sob `docs-site/docs/` referenciam capturas de tela com um segmento de localidade fixo na URL, por exemplo `/img/screenshots/en-GB/screenshot.png` em `feature-showcase.md`. O pós-processamento substitui esse segmento para que cada página traduzida sob `docs-site/i18n/<locale>/…/current/` resolva para sua própria pasta:
...
Inclua PNGs correspondentes na árvore estática do seu site (por exemplo, `docs-site/static/img/screenshots/<locale>/` para URLs que começam com `/img/screenshots/`).

- **README raiz, saída plana** (segundo bloco `documentations[]` no mesmo arquivo): apenas `README.md` é traduzido, com `markdownOutput.style` `flat` e `outputDir` `translated-docs`, então você obtém `translated-docs/README.<locale>.md`. As imagens em inglês geralmente usam um segmento de pasta estável no meio do caminho (por exemplo, `images/screenshots/en-GB/overview.png`). O pós-processamento substitui qualquer segmento único do caminho localizado entre `images/screenshots/` e o restante da URL pelo `${translatedLocale}` ativo, de modo que cada README traduzido aponte para `images/screenshots/de/…`, `images/screenshots/fr/…` etc. Esse padrão difere da regra do Docusaurus: aqui `search` corresponde a **qualquer** nome de pasta (`[^/]+/`), não apenas `en-GB/`.
...
Mantenha os arquivos PNG no disco sob `images/screenshots/<locale>/` (mesma estrutura que as URLs usam após a reescrita).

**Padrão 3 — arquivo SVG** (`examples/nextjs-app`)

O mesmo exemplo habilita `features.translateSVG` e mapeia SVGs de origem para a pasta pública do aplicativo web:
...
Execute `translate-svg` (ou `sync`) para que `images/*.svg` se torne saídas por localidade sob `public/assets/`. Referências em markdown apontam para essas URLs separadamente de `translate-docs`.

**Exemplo mínimo somente com README** (`examples/console-app`)

`examples/console-app/ai-i18n-tools.config.json` traduz `README.md` para `translated-docs/` usando apenas `postProcessing.languageListBlock`. Ele não define regras de imagem — apropriado quando o README não tem arquivos raster irmãos ou usa apenas URLs absolutas que seu host já disponibiliza.
...
- O primeiro bloco de docs traduz **markdown** de `docs-site/docs/` para `docs-site/i18n/<locale>/docusaurus-plugin-content-docs/current/` (páginas de documentação localizadas).
- Com `features.translateJSON` e `jsonSource`, esse mesmo bloco também traduz o **JSON de shell do Docusaurus** em `docs-site/i18n/en/` para cada pasta de localidade de destino — barra de navegação, rodapé e catálogos de tema/plugin, não o conteúdo do corpo MDX.
- O segundo bloco de docs traduz `README.md` em arquivos planos com sufixo de localidade em `translated-docs/`.
...
| Campo         | Descrição                                               |
|---------------|-----------------------------------------------------------|
| `sourceRoots` | Diretórios ou padrões glob (relativos ao cwd) verificados em busca de chamadas `t("…")`. |
...
- `description`
Nota legível opcional para este bloco (não usada para tradução). Prefixada no título `translate-docs` `🌐` quando definida; também exibida nos cabeçalhos de seção `status`.
- `contentPaths`
Corpos de página Markdown/MDX para traduzir (`translate-docs` os verifica em busca de `.md` / `.mdx`). Suporta **caminhos de diretório ou padrões glob** (por exemplo, `"docs/**/*.md"`, `"guides/*.mdx"`). É daí que vem a prosa da documentação localizada.
- `outputDir`
...
- `jsonSource`
Opcional. Diretório de origem para catálogos JSON de rótulos do Docusaurus para este bloco (por exemplo, `"i18n/en"` de `docusaurus write-translations`). Os corpos das páginas sempre vêm de `contentPaths`; `jsonSource` fornece apenas JSON da interface/casca, não MDX.
- `markdownOutput.style`
`"nested"` (padrão), `"docusaurus"` ou `"flat"`.
- `markdownOutput.docsRoot`
Diretório raiz de docs de origem para o layout do Docusaurus (por exemplo, `"docs"`).
- `markdownOutput.pathTemplate`
Caminho de saída markdown personalizado. Placeholders: <code>"{outputDir}"</code>, <code>"{locale}"</code>, <code>"{LOCALE}"</code>, <code>"{relPath}"</code>, <code>"{stem}"</code>, <code>"{basename}"</code>, <code>"{extension}"</code>, <code>"{docsRoot}"</code>, <code>"{relativeToDocsRoot}"</code>.
- `markdownOutput.jsonPathTemplate`
Caminho de saída JSON personalizado para arquivos de rótulo. Suporta os mesmos placeholders que `pathTemplate`.
- `markdownOutput.flatPreserveRelativeDir`
Para o estilo `flat`, manter subdiretórios de origem para que arquivos com o mesmo nome base não colidam.
- `markdownOutput.rewriteRelativeLinks`
Reescrever links relativos após a tradução (ativado automaticamente para o estilo `flat`).
- `markdownOutput.linkRewriteDocsRoot`
Diretório raiz do repositório usado ao calcular prefixos de reescrita de links planos. Normalmente, mantenha como `"."`, a menos que sua documentação traduzida esteja em uma raiz de projeto diferente.
- `markdownOutput.postProcessing`
Transformações opcionais no **corpo do markdown** (a YAML front matter é preservada). Executado após a remontagem dos segmentos e a reescrita de links planos, e antes de `addFrontmatter`.
- `segmentSplitting`
Mesmo nível que `markdownOutput` (por bloco `documentations[]`). Segmentos opcionais mais refinados para extração de `translate-docs`: `{ "enabled", "maxCharsPerSegment"?, "splitPipeTables"?, "splitDenseParagraphs"?, "maxLinesPerParagraphChunk"?, "splitLongLists"?, "maxListItemsPerChunk"? }`. Quando `enabled` é `true` (padrão quando `segmentSplitting` é omitido), parágrafos densos, tabelas GFM com pipes (o primeiro bloco inclui cabeçalho, separador e primeira linha de dados) e listas longas são divididos; as partes são reunidas com quebras simples (`tightJoinPrevious`). Defina `"enabled": false` para usar um segmento por bloco do corpo delimitado por linha em branco apenas.
- `warnMarkdownSourceIssues`
Quando `true` (padrão quando omitido), cada execução de `translate-docs` reanalisa os segmentos markdown em busca de delimitadores arriscados ou código embutido não fechado, exibe avisos no terminal e substitui as linhas `markdown_source_issues` no caminho do arquivo de cache. Defina `false` para pular avisos e atualizações do SQLite para este bloco.
- `markdownOutput.postProcessing.regexAdjustments`
Lista ordenada de `{ "description"?, "search", "replace" }`. `search` é um padrão regex (string simples usa a flag `g`, ou `/pattern/flags`). `replace` suporta marcadores como `${translatedLocale}`, `${sourceLocale}`, `${sourceFullPath}`, `${translatedFullPath}`, `${sourceFilename}`, `${translatedFilename}`, `${sourceBasedir}`, `${translatedBasedir}`.
- `markdownOutput.postProcessing.languageListBlock`
`{ "start", "end", "separator", "label" }` — o tradutor localiza a primeira linha contendo `start` e a linha correspondente `end`, substituindo esse trecho por um seletor de idioma canônico. `label` controla a origem do rótulo do manifesto: `"local"` (padrão, usa `ui-languages.json` `label`) ou `"english"` (usa `englishName`). Os links são montados com caminhos relativos ao arquivo traduzido; quando nenhum manifesto é configurado, os rótulos vêm de `localeDisplayNames` e códigos de localidade.
- `addFrontmatter`
...
**Exemplo (pipeline de README plano — caminhos de imagens + opcional wrapper de lista de idiomas):**
...
| Campo                         | Descrição                                                                                                                                                                                                                                                                        |
|-------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `sourcePath`                  | Um ou mais diretórios **ou padrões glob** (por exemplo, `"images/*.svg"`, `"**/icons/*.svg"`). Os padrões são resolvidos em relação à raiz do projeto e verificados recursivamente por arquivos `.svg`.                                                                                       |
...
Cria um arquivo de configuração inicial (inclui `concurrency`, `batchConcurrency`, `batchSize`, `maxBatchChars` e `documentations[].addFrontmatter`). `--with-translate-ignore` cria um `.translate-ignore` inicial.
...
- `write-heading-ids …`
**Sem API.** Exige pelo menos um bloco `documentations[]`. Coleta `.md` / `.mdx` sob o `contentPaths` de cada bloco (respeita `.translate-ignore`). Insere uma linha de âncora HTML `<a id="slug"></a>` imediatamente **antes** de cada título ATX plano `#` (ignora títulos dentro de blocos de código com delimitadores). `-p` / `--path` ou `-f` / `--file`: limita a um arquivo ou diretório relativo ao projeto. `--slug-style`: `github` (padrão; doctoc / anchor-markdown-header), `bitbucket`, `gitlab`, `pymdown`, `azure-devops`. Com `pymdown`, opcional `--pymdown-case`, `--pymdown-normalize`, `--pymdown-percent-encode` / `--no-pymdown-percent-encode`. `--dry-run`: lista apenas alterações.

- `strip-md-bold-inline …`
**Sem API.** Exige pelo menos um bloco `documentations[]`. Remove `**` ao redor do código embutido em `.md` / `.mdx` sob o `contentPaths` de cada bloco (respeita `.translate-ignore`). `-p` / `--path` ou `-f` / `--file`, `--dry-run`, `--no-backup` (ignora `.backup.*` com carimbo de data antes da substituição).

- `check-markdown …`
**Sem API.** Analisa markdown/MDX sob o `contentPaths` de cada bloco `documentations[]` (mesma descoberta que `translate-docs`, respeita `.translate-ignore`): pares de delimitadores, código embutido não fechado e `STRONG_OUTSIDE_INLINE_CODE` / `STRONG_OUTSIDE_LINK` quando `**`/`__` envolvem um trecho `` `...` `` ou um link `[text](../url)`. `-p` / `--path` ou `-f` / `--file`: escopo opcional. Imprime linhas `relativePath:line: [ISSUE_CODE] message` no **stderr**; código de saída **1** se houver algum problema. `--json`: relatório JSON no **stdout**. Grava `markdown_source_issues` em `cacheDir` exceto se `--no-cache`. `-v` adiciona hashes de origem às linhas do stderr.
...
Executa `extract` **primeiro** (exige `features.extractUIStrings`) para que `strings.json` corresponda à origem, depois revisão por LLM das strings da interface **origem-localidade** (ortografia, gramática). **Dicas de terminologia** vêm apenas do CSV `glossary.userGlossary` (mesmo escopo que `translate-ui` — não `strings.json` / `uiGlossary`, para que texto ruim não seja reforçado como glossário). Usa OpenRouter (`OPENROUTER_API_KEY`). Apenas informativo (saída com **0** ao finalizar). Grava `lint-source-results_<timestamp>.log` sob `cacheDir` como relatório **legível por humanos** (resumo, problemas e linhas **OK** por string); o terminal imprime apenas contagens resumidas e problemas (sem linhas `[ok]` por string). Imprime o nome do arquivo de log na última linha. `--json`: relatório JSON totalmente legível por máquina apenas no stdout (o arquivo de log permanece legível por humanos). `--dry-run`: ainda executa `extract`, mas imprime apenas o plano do lote (sem chamadas à API). `--chunk`: strings por lote da API (padrão **50**). `-j`: número máximo de lotes em paralelo (padrão `concurrency`). Com `--json`, a saída no estilo humano vai para o stderr. Os links usam `path:line` como o botão “link” das strings da interface `editor`.
...
Exibe o cache de documentação e estatísticas do `strings.json` (agregações iguais às do Editor de Cache de Tradução → **Estatísticas**). `--max-columns`: número máximo de colunas de localidade por modelo × tabela de localidade (padrão corresponde ao editor).
...
Inicia um editor web local para o cache, `strings.json` e CSV do glossário. Com `--no-open`, o navegador padrão não é aberto automaticamente.  
**Observação:** Se você editar uma entrada no editor de cache, deverá executar um `sync --force-update` para reescrever os arquivos de saída com a entrada de cache atualizada. Além disso, se o texto de origem for alterado posteriormente, a edição manual será perdida, pois uma nova chave de cache será gerada.
