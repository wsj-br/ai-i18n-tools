...
**Prefiere `sync`** para cualquier cosa que antes fuera "ejecuta `translate-ui`, luego `translate-svg`, luego `translate-docs`": `ai-i18n-tools sync` ejecuta **extract** (cuando está habilitado), **translate-ui**, opcionalmente **translate-svg** y luego **translate-docs**—en el orden correcto y con banderas compartidas—de acuerdo con tu configuración. Encadenar manualmente esos tres comandos de traducción es fácil de hacer mal (orden, banderas de extracción, de idioma). Usa `i18n:translate:ui`, `i18n:translate:svg` y `i18n:translate:docs` solo cuando necesites un **único** paso de forma aislada.
...
**Validación:** Si el mensaje contiene **dos o más** marcadores de posición `{{…}}` distintos, **uno de ellos debe ser `{{count}}`** (el eje plural). De lo contrario, `extract` **falla** con un mensaje claro indicando archivo y línea.
...
**Ejemplo mínimo solo para README (`examples/console-app`)**
...
Esto inicia una interfaz web local respaldada por tu base de datos **`cacheDir`** SQLite configurada, la misma carpeta que la CLI usa para segmentos de documentación, registros y metadatos relacionados. Incluye las pestañas **Documentación** (segmentos de documento en caché), **Cadenas de interfaz**, **Plurales de interfaz**, **Glosario**, **Errores** y **Estadísticas**.
