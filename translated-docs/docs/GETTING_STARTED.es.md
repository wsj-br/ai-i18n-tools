---
translation_last_updated: '2026-04-13T00:28:19.751Z'
...
- `documentations[].description` - nota corta opcional para los mantenedores (lo que cubre este bloque). Cuando se establece, aparece en el encabezado de `translate-docs` (`🌐 …: traduciendo …`) y en los encabezados de la sección `status`.
...
| Campo                                       | Descripción                                                                                                                                                                                                               |
| -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `description`                                | Nota opcional legible por humanos para este bloque (no utilizada para traducción). Prefijada en el encabezado `🌐` de `translate-docs` cuando está configurada; también se muestra en los encabezados de sección de `status`.                                                     |
| `contentPaths`                               | Fuentes Markdown/MDX a traducir (`translate-docs` escanea estas para `.md` / `.mdx`). Las etiquetas JSON provienen de `jsonSource` en el mismo bloque.                                                                                  |
...
| `targetLocales`                              | Subconjunto opcional de locales solo para este bloque (de lo contrario, locales raíz `targetLocales`). Los locales de documentación efectivos son la unión a través de bloques.                                                                             |
| `jsonSource`                                 | Directorio fuente para los archivos de etiquetas JSON de Docusaurus para este bloque (por ejemplo, `"i18n/en"`).                                                                                                                                       |
...
| `markdownOutput.flatPreserveRelativeDir`     | Para el estilo `flat`, mantener subdirectorios fuente para que los archivos con el mismo nombre base no colisionen.                                                                                                                              |
...
| `markdownOutput.postProcessing` | Transformaciones opcionales en el **cuerpo** de markdown traducido (se preserva el front matter YAML). Se ejecuta después de la reensamblaje de segmentos y la reescritura de enlaces planos, y antes de `injectTranslationMetadata`. |
...
| `markdownOutput.postProcessing.languageListBlock` | `{ "start", "end", "separator" }` — el traductor encuentra la primera línea que contiene `start` y la línea `end` correspondiente, luego reemplaza ese segmento con un conmutador de idioma canónico. Los enlaces se construyen con rutas relativas al archivo traducido; las etiquetas provienen de `uiLanguagesPath` / `ui-languages.json` cuando están configuradas, de lo contrario, de `localeDisplayNames` y códigos de locales. |
...
| `translate-docs …`                                                        | Traduce archivos markdown/MDX y JSON para cada bloque `documentations` (`contentPaths`, opcional `jsonSource`). `-j`: número máximo de idiomas en paralelo; `-b`: número máximo de llamadas paralelas a la API por archivo. `--prompt-format`: formato del lote (`xml` \| `json-array` \| `json-object`). Consulta [Comportamiento de caché y banderas de `translate-docs`](#cache-behaviour-and-translate-docs-flags) y [Formato del prompt por lotes](#batch-prompt-format). |
| `translate-svg …`                                                         | Traduce recursos SVG independientes configurados en `config.svg` (separado de la documentación). Aplica las mismas ideas de caché que en la documentación; admite `--no-cache` para omitir lecturas/escrituras en SQLite durante esa ejecución. `-j`, `-b`, `--force`, `--force-update`, `-p` / `--path`, `--dry-run`.                                                    |
| `translate-ui [--locale <code>] [--force] [--dry-run] [-j <n>]`           | Traduce únicamente las cadenas de la interfaz de usuario. `--force`: vuelve a traducir todas las entradas por idioma (ignora las traducciones existentes). `--dry-run`: no realiza escrituras ni llamadas a la API. `-j`: número máximo de idiomas en paralelo. Requiere `features.translateUIStrings`.                                                                                 |
| `sync …`                                                                  | Extrae (si está habilitado), luego traduce la interfaz de usuario, luego `translate-svg` cuando existe `config.svg`, y finalmente traduce la documentación, a menos que se omita con `--no-ui`, `--no-svg` o `--no-docs`. Banderas compartidas: `-l`, `-p`, `--dry-run`, `-j`, `-b` (solo para lotes de documentación), `--force` / `--force-update` (solo para documentación; se excluyen mutuamente cuando se ejecuta la documentación).                         |
| `status`                                                                  | Muestra el estado de traducción del markdown por archivo × idioma (sin filtro `--locale`; los idiomas provienen de la configuración).                                                                                                                                                                                               |
| `cleanup [--dry-run] [--no-backup] [--backup <path>]`                  | Primero ejecuta `sync --force-update` (extrae, UI, SVG, docs), luego elimina filas de segmentos obsoletas (con `last_hit_at` nulo o ruta de archivo vacía); elimina filas de `file_tracking` cuya ruta de origen resuelta no existe en el disco; elimina filas de traducción cuya metainformación `filepath` apunta a un archivo inexistente. Registra tres conteos (obsoletos, `file_tracking` huérfanos, traducciones huérfanas). Crea una copia de seguridad de SQLite con marca de tiempo en el directorio de caché, a menos que se use `--no-backup`. |
