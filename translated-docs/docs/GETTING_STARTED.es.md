...
| `sync-ui [-l <codes>] [--force] [--dry-run] [-j <n>]`                                    | Extrae (si `features.extractUIStrings` está habilitado), luego traduce las cadenas de interfaz de usuario (si `features.translateUIStrings` está habilitado). Solo interfaz de usuario — sin documentación ni SVG. Mismas opciones `-l`, `--force`, `--dry-run` y `-j` que `translate-ui`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `lint-source [-l <code>] [--chunk <n>] [--dry-run] [--json] [-j <n>]`                    | Ejecuta `extract` **primero** (requiere `features.extractUIStrings`) para que `strings.json` coincida con el código fuente, luego realiza una revisión con LLM de las cadenas de interfaz de usuario **origen-configuración regional** (ortografía, gramática). Las **sugerencias de terminología** provienen únicamente del archivo CSV `glossary.userGlossary` (mismo ámbito que `translate-ui` — no `strings.json` / `uiGlossary`, para que textos incorrectos no se refuercen como glosario). Usa OpenRouter (`OPENROUTER_API_KEY`). Solo orientativo (finaliza con **0** al completarse). Genera `lint-source-results_<timestamp>.log` dentro de `cacheDir` como informe **legible por humanos** (resumen, problemas y filas **OK** por cadena); la terminal muestra solo recuentos del resumen y problemas (sin líneas `[ok]` por cadena). Imprime el nombre del archivo de registro en la última línea. `--json`: informe JSON completo y legible por máquina solo en stdout (el archivo de registro permanece legible por humanos). `--dry-run`: aún ejecuta `extract`, luego imprime solo el plan del lote (sin llamadas a API). `--chunk`: cadenas por lote de API (predeterminado **50**). `-j`: lotes máximos en paralelo (predeterminado `concurrency`). Con `--json`, la salida estilo humano va a stderr. Los enlaces usan `path:line` como el botón “link” de las cadenas de interfaz de usuario `dashboard`. |
