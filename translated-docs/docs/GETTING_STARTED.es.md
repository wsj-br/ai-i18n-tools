...
| `cleanup [--dry-run] [--no-backup] [--backup <path>]`                                                      | Ejecuta `sync --force-update` primero (extracción, interfaz de usuario, SVG, documentación), luego elimina filas de segmentos obsoletos (`last_hit_at` nulo / ruta de archivo vacía); elimina filas `file_tracking` cuya ruta de origen resuelta no existe en el disco; elimina filas de traducción cuyos metadatos `filepath` hacen referencia a un archivo inexistente; poda filas `translation_failures` huérfanas. Registra cuatro conteos (segmentos obsoletos, `file_tracking` huérfanos, traducciones huérfanas, errores huérfanos). Crea una copia de seguridad de SQLite con marca de tiempo en el directorio de caché, a menos que `--no-backup`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
