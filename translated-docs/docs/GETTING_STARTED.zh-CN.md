...
发布的包仅支持 **ESM**。请在 Node.js 或打包工具中使用 `import`/`import()`；不要使用 `require('ai-i18n-tools')`。
...
在本地安装包后，您可以在脚本中直接使用 CLI 命令（无需 `npx`）：
...
通过使用 `makeLocaleLoadersFromManifest` 从 `ui-languages.json` 派生值来保持 `localeLoaders` 与配置 **一致**（这会使用与 `makeLoadLocale` 相同的规范化方式过滤掉 `SOURCE_LOCALE`）。当你向 `targetLocales` 添加新语言环境并运行 `generate-ui-languages` 时，清单文件将自动更新，加载器也会自动跟踪更改——无需维护单独的硬编码映射。
...
- 分段行全局以 `(source_hash, locale)`（哈希值 = 归一化后的内容）为键。两个文件中相同的文本共享一行；`translations.filepath` 是元数据（最后写入者），并非每个文件的第二个缓存条目。
- `file_tracking.filepath` 使用命名空间键：每个 `documentations` 块对应 `doc-block:{index}:{relPath}`（`relPath` 是相对于项目根目录的 POSIX 路径：收集的 Markdown 路径；**JSON 标签文件使用相对于当前工作目录的源文件路径**，例如 `docs-site/i18n/en/code.json`，以便清理时可以解析实际文件），以及 `svg-assets:{relPath}` 用于 `translate-svg` 下的独立 SVG 资产。
- `translations.filepath` 存储 Markdown、JSON 和 SVG 分段相对于当前工作目录的 POSIX 路径（SVG 使用与其他资产相同的路径格式；`svg-assets:…` 前缀 **仅** 用于 `file_tracking`）。
- 运行后，`last_hit_at` 仅清除 **在同一翻译范围内**（遵循 `--path` 和启用的类型）且未被命中的分段行，因此过滤或仅文档的运行不会标记无关文件为陈旧。
...
`"flat"` - 将翻译后的文件与源文件并列放置，并添加区域设置后缀，或放在子目录中。页面之间的相对链接会自动重写。
...
| `markdownOutput.postProcessing.languageListBlock` | `{ "start", "end", "separator" }` — 翻译器查找包含 `start` 的第一行及对应的 `end` 行，然后将该片段替换为标准语言切换器。链接使用相对于翻译后文件的路径构建；标签来自 `uiLanguagesPath` / `ui-languages.json`（若已配置），否则来自 `localeDisplayNames` 和区域代码。                                                                                                                                                                                                                                                                                       |
...
| `editor [-p <port>] [--no-open]`                                            | 启动本地网页编辑器以编辑缓存、`strings.json` 和词汇表 CSV。`--no-open` 不会自动打开默认浏览器。<br><br>**注意：** 如果您在缓存编辑器中编辑了条目，您必须运行 `sync --force-update` 以使用更新的缓存条目重写输出文件。此外，如果源文本稍后发生更改，手动编辑将会丢失，因为会生成新的缓存键。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
