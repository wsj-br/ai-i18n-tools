...
`i18next-scanner` の `Parser.parseFuncFromString` を使用して、任意の JS/TS ファイル内の `t("literal")` および `i18n.t("literal")` 呼び出しを検出します。関数名およびファイル拡張子は構成可能です。`extract` **は、非スキャナー入力を同じカタログに統合することもできます。** つまり、`reactExtractor.includePackageDescription` が有効（デフォルト）の場合にはプロジェクトの `package.json` `description` を統合し、`reactExtractor.includeUiLanguageEnglishNames` が `true` かつ `uiLanguagesPath` が設定されている場合には `ui-languages.json` からの各 **`englishName`** を統合します（ソース内で既に見つかった文字列が優先されます）。セグメントのハッシュは、トリム済みのソース文字列の **MD5 の最初の8桁の16進数** であり、これが `strings.json` 内のキーになります。
...
`buildUIPromptMessages`はシステムとユーザーのメッセージを構築します:
- ソース言語とターゲット言語を特定します（`localeDisplayNames`または`ui-languages.json`からの表示名によって）。
- 文字列のJSON配列を送信し、翻訳のJSON配列を要求します。
