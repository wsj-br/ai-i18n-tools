...
プレーンHTMLアプリは、`t()`呼び出しの代わりにマーカー属性を使用して、同じカタログモデルに従います。詳細は[GETTING_STARTED — マークアップの翻訳用HTML](GETTING_STARTED.ja.md#marking-html-for-translation)を参照してください。
...
- **ロケール解決** (`resolveUiLocale` in `src/core/ui-locale.ts`): UI ロケールを `-L` / `--ui-lang` > `AI_I18N_LANG` > config `uiLanguage` > ホスト OS ロケール (`Intl.DateTimeFormat().resolvedOptions().locale`) の順に選択します。候補は正規化され、提供されたバンドルセットと完全一致または最も近いバリエーション (例: `pt-PT` → `pt-BR`、`en-US` → `en-GB`) に対して照合され、ソース ロケール (`en-GB`) にフォールバックします。CLI はヘルプを構築する前に 1 回 (事前解析 argv スキャン) と config ロード後に解決します。`uiLanguage` が適用されます (フラグと環境変数はまだ勝つ)。
- **ランタイム** (`src/i18n/index.ts`): `{{name}}` の補間を備えた最小の `t(source, vars)` で、英語のソース文字列に対して `src/i18n/locales/<code>.json` (ビルド時に `dist/i18n/locales` にコピー) の平坦なロケール バンドルでキー付けされます。キーまたはバンドルが見つからない場合、ソース テキストが返されます。これは、ワークフロー 1 と同じデフォルトとしてのキー モデルであり、ハッシュ ルックアップはありません。
- **ダッシュボード**: サーバーは解決された UI ロケールの `GET /api/ui-i18n` を返す `{ locale, dir, bundle }` を公開します。フロントエンドは `<html lang>` / `dir` を設定し、`data-i18n*` 属性を介して静的マークアップをローカライズします。
- **ドッグフーディング**: バンドルは、パッケージの独自の抽出 → `translate-ui` パイプラインを `ai-i18n-self.config.json` (`pnpm i18n:self`) に対して実行することによって生成されます。カタログ キーは、`t()` 呼び出し全体の `src/cli/` と `src/i18n/`、およびダッシュボードの `data-i18n*` マーカー `src/dashboard-app/index.html` から来ます。
