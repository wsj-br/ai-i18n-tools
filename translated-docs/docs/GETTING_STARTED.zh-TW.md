...
透過使用 **`makeLocaleLoadersFromManifest`** 從 **`ui-languages.json`** 衍生出值來使 `localeLoaders` **與設定保持一致**（使用與 **`makeLoadLocale`** 相同的正規化方式過濾掉 **`SOURCE_LOCALE`**）。當您在 **`targetLocales`** 中新增語系並執行 **`generate-ui-languages`** 後，清單會自動更新，您的載入器也會追蹤它們，無需維護單獨的硬編碼對應表。如果 JSON 匯捆檔案位於 **`public/`** 下（典型的 Next.js 結構），請在每個載入器中使用 **`fetch(\`/locales/${code}.json\`)`** 而非 **`import()`**，以便瀏覽器能從您的公開 URL 路徑載入靜態 JSON。對於沒有打包工具的 Node CLI，請在一個小型 **`makeFileLoader`** 輔助函式中使用 **`readFileSync`** 來載入語系檔案，該函式會為每個語系代碼回傳解析後的 JSON。
...
**驗證：** 如果訊息包含**兩個或更多**不同的 `{{…}}` 占位符，其中**必須有一個是 `{{count}}`**（複數軸）。否則 `extract` **將失敗**，並顯示明確的檔案/行號訊息。

**兩個獨立計數**（例如章節和頁面）不能共用同一個複數訊息 — 請使用**兩個** `t()` 呼叫（每個都包含 `plurals: true` 及其各自的 `count`），並在 UI 中串接。

`strings.json` **:** 複數群組使用 **每個雜湊一列** 的方式，其中包含 `"plural": true`、原始文字 **`source`**，以及將基數類別（`zero`、`one`、`two`、`few`、`many`、`other`）對應到該語系字串的物件 **`translated[locale]`**。
...
執行時期（`ai-i18n-tools/runtime` **）：** 呼叫 **`setupKeyAsDefaultT(i18n, { stringsJson, sourcePluralFlatBundle })`** — 它會執行 **`wrapI18nWithKeyTrim`**，註冊可選的 **`translate-ui`** `{sourceLocale}.json` 複數捆綁包，然後使用 **`buildPluralIndexFromStringsJson(stringsJson)`** 進行 **`wrapT`**。`wrapT` 會去除 `plurals` / `zeroDigit`，在需要時將鍵重寫為群組 ID，並轉發 **`count`**（可選：如果只有一個非 `{{count}}` 的佔位符，則 `count` 會從該數值選項複製）。
...
此清單由 `generate-ui-languages` 根據 `sourceLocale` + `targetLocales` 和捆綁的主目錄生成，並寫入 `ui.flatOutputDir`。若您變更設定中的任一語系，請執行 `generate-ui-languages` 以更新 `ui-languages.json` 檔案。
...
| `temperature`       | 取樣溫度。默認：`0.2` **。                                                    |**為何使用多個模型：** 不同供應商和模型的成本各異，且在不同語言和地區上的品質表現也有所不同。將 **`openrouter.translationModels` 設定為有序的備援鏈**（而非單一模型），如此一來當某個請求失敗時，CLI 便能嘗試下一個模型。
...
| `contentPaths`                               | 要翻譯的 Markdown/MDX 原始內容（`translate-docs` 會掃描這些內容以取得 `.md` / `.mdx`）。JSON 標籤來自此區塊中的 `jsonSource`。                                                                                  |
| `outputDir`                                  | 此區塊翻譯輸出的根目錄。                                                                                                                                                                      |
| `sourceFiles`                                | 可選的別名，載入時會合併到 `contentPaths` 中。                                                                                                                                                                        |
| `targetLocales`                              | 僅適用於此區塊的可選語言環境子集（否則使用根目錄的 `targetLocales`）。有效的文件語言環境是所有區塊的聯集。                                                                             |
| `jsonSource`                                 | 此區塊 Docusaurus JSON 標籤檔案的原始目錄（例如 `"i18n/en"`）。                                                                                                                                       |
| `markdownOutput.style`                       | `"nested"`（預設值）、`"docusaurus"` 或 `"flat"`。                                                                                                                                                                        |
...
| `markdownOutput.flatPreserveRelativeDir`     | 對於 `flat` 風格，保留原始子目錄以避免同檔名的檔案衝突。                                                                                                                              |
| `markdownOutput.rewriteRelativeLinks` | 翻譯後重寫相對連結（`flat` 風格會自動啟用此功能）。                                                                                                                                                 |
| `markdownOutput.linkRewriteDocsRoot` | 計算扁平連結重寫前綴時使用的儲存庫根目錄。通常保留為 `"."`，除非您的翻譯文件位於不同的專案根目錄下。 |
| `markdownOutput.postProcessing` **| 對翻譯後的 Markdown**本文 進行可選的轉換（YAML 前置資料會保留）。在片段重新組合和扁平連結重寫之後、`addFrontmatter` 之前執行。 |
| `markdownOutput.postProcessing.regexAdjustments` | `{ "description"?, "search", "replace" }` 的有序列表。`search` 是正規表示式模式（純字串使用標誌 `g` 或 `/pattern/flags`）。`replace` 支援諸如 `${translatedLocale}`、`${sourceLocale}`、`${sourceFullPath}`、`${translatedFullPath}`、`${sourceFilename}`、`${translatedFilename}`、`${sourceBasedir}`、`${translatedBasedir}` 等佔位符。 |
| `markdownOutput.postProcessing.languageListBlock` | `{ "start", "end", "separator" }` — 翻譯器會尋找包含 `start` 的第一行以及對應的 `end` 行，然後將該段落替換為標準的語言切換器。連結的路徑相對於翻譯後的檔案；標籤來自 `uiLanguagesPath` / `ui-languages.json`（若已設定），否則來自 `localeDisplayNames` 和語言代碼。 |
| `addFrontmatter`                  | 當設定為 `true`（省略時預設為此值）時，翻譯後的 Markdown 檔案會包含以下 YAML 欄位：`translation_last_updated`、`source_file_mtime`、`source_file_hash`、`translation_language`、`source_file_path`，以及當至少有一個片段具有模型中繼資料時的 `translation_models`（已排序的 OpenRouter 模型 ID 列表）。設定為 `false` 可跳過。 |
...
| 命令                                                                   | 說明                                                                                                                                                                                                                                                                                        |
| ------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `version`                                                                 | 印出 CLI 版本和建置時間戳（與根程式上的 `-V` / `--version` 提供的資訊相同）。                                                                                                                                                                                                  |
...
| `extract`                                                                 | 從 `t("…")` / `i18n.t("…")` 字面值、選擇性 `package.json` 說明和選擇性 manifest `englishName` 項目更新 `strings.json`（參見 `ui.reactExtractor`）。需要 `features.extractUIStrings`。                                                                                                                                                                                                    |
| `generate-ui-languages [--master <path>] [--dry-run]`                     | 使用 `sourceLocale` + `targetLocales` 和內建的 `data/ui-languages-complete.json`（或 `--master`）將 `ui-languages.json` 寫入 `ui.flatOutputDir`（或設定時的 `uiLanguagesPath`）。若主檔案中缺少某些語系，會發出警告並產生 `TODO` 佔位符。如果您現有的 manifest 中有自訂的 `label` 或 `englishName` 值，它們將被主目錄的預設值取代 — 完成後請審查並調整所產生的檔案。 |
| `translate-docs …`                                                        | 為每個 `documentations` 區塊翻譯 markdown/MDX 和 JSON（`contentPaths`，選擇性 `jsonSource`）。`-j`：最大並行語系數；`-b`：每檔案最大並行批次 API 請求數。`--prompt-format`：批次傳輸格式（`xml` \| `json-array` \| `json-object`）。參見 [快取行為與 `translate-docs` 標記](#cache-behaviour-and-translate-docs-flags) 和 [批次提示格式](#batch-prompt-format)。 |
| `translate-svg …`                                                         | 翻譯在 `config.svg` 中設定的獨立 SVG 資產（與文件分開）。需要 `features.translateSVG`。與文件具有相同的快取概念；支援 `--no-cache` 以跳過該次執行的 SQLite 讀寫操作。支援 `-j`、`-b`、`--force`、`--force-update`、`-p` / `--path`、`--dry-run`。                                                    |
| `translate-ui [--locale <code>] [--force] [--dry-run] [-j <n>]`           | 僅翻譯 UI 字串。`--force`：每語系重新翻譯所有項目（忽略現有翻譯）。`--dry-run`：不寫入、不呼叫 API。`-j`：最大並行語系數。需要 `features.translateUIStrings`。                                                                                 |
| `lint-source [-l <code>] [--chunk <n>] [--dry-run] [--json] [-j <n>]` **| 首先執行**`extract`**（需要 **`features.extractUIStrings`**），使 **`strings.json`** 與原始語系一致，然後由 LLM 審查 **原始語系** UI 字串（拼字、文法）。**術語提示**僅來自 **`glossary.userGlossary`** CSV（範圍與 **`translate-ui` 相同 — 不包含 `strings.json` / `uiGlossary`，因此不會將錯誤內容強化為術語表）。使用 OpenRouter（`OPENROUTER_API_KEY` **）。僅供參考（執行完成時以**0** 結束）。在 **`cacheDir`** 下寫入 **`lint-source-results_<timestamp>.log`** 作為 **人類可讀** 的報告（摘要、問題和每字串的 **OK** 列）；終端機僅列印摘要統計和問題（不列印每字串的 **`[ok]`** 行）。最後一行會印出日誌檔名。**`--json`**：僅在 stdout 輸出完整機器可讀的 JSON 報告（日誌檔保持人類可讀）。**`--dry-run`**：仍會執行 **`extract`**，然後僅列印批次計畫（不呼叫 API）。**`--chunk`**：每 API 批次的字串數（預設為 **50）。`-j` **：最大並行批次數（預設**`concurrency`**）。搭配 **`--json`** 時，人類風格輸出會導向 stderr。連結使用 **`path:line`**，如同 **`editor` UI 字串的「連結」按鈕。 |
| `export-ui-xliff [-l <codes>] [-o <dir>] [--untranslated-only] [--dry-run]` | 匯出 `strings.json` 為 XLIFF 2.0 格式（每個目標語系產生一個 `.xliff`）。`-o` / `--output-dir`：輸出目錄（預設：與目錄檔相同資料夾）。`--untranslated-only`：僅包含該語系缺少翻譯的單元。唯讀；不呼叫 API。                                                        |
| `sync …`                                                                  | 啟用時先提取，再進行 UI 翻譯，接著當 `features.translateSVG` 和 `config.svg` 設定時執行 `translate-svg`，最後進行文件翻譯 — 除非使用 `--no-ui`、`--no-svg` 或 `--no-docs` 跳過。共用標記：`-l`、`-p` / `-f`、`--dry-run`、`-j`、`-b`（僅文件批次化）、`--force` / `--force-update`（僅文件；文件執行時互斥）。文件階段也會轉發 `--emphasis-placeholders` 和 `--debug-failed`（意義與 `translate-docs` 相同）。`--prompt-format` 不是 `sync` 標記；文件步驟使用內建預設值（`json-array`）。                         |
| `status [--max-columns <n>]`                                   | 當 `features.translateUIStrings` 啟用時，列印每語系的 UI 覆蓋率（`Translated` / `Missing` / `Total`）。然後列印每檔案 × 每語系的 markdown 翻譯狀態（無 `--locale` 過濾；語系來自設定）。大型語系清單會分割成最多 `n` **欄語系的重複表格（預設為**9），以保持終端機行寬度適中。                                                                                                                                                                                               |
| `cleanup [--dry-run] [--no-backup] [--backup <path>]`                  | 首先執行 `sync --force-update`（提取、UI、SVG、文件），然後移除過時的段落列（`last_hit_at` 為 null / 檔案路徑為空）；刪除解析後原始路徑在磁碟上不存在的 `file_tracking` 列；移除其 `filepath` 元資料指向缺失檔案的翻譯列。記錄三項計數（過時、孤立的 `file_tracking`、孤立的翻譯）。除非使用 `--no-backup`，否則會在快取目錄下建立帶時間戳的 SQLite 備份。 |
| `editor [-p <port>] [--no-open]`                                          | 啟動本地網頁編輯器以編輯快取、`strings.json` 和術語表 CSV。`--no-open` **：不要自動開啟預設瀏覽器。<br><br>**注意： 如果您在快取編輯器中編輯了某個條目，必須執行 `sync --force-update` 才能使用更新的快取條目重寫輸出檔案。此外，如果原始文字日後變更，手動編輯將會遺失，因為會產生新的快取金鑰。 |
