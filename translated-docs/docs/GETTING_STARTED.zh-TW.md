...
當所有已配置的模型在同一個片段上都因 `AST mismatch` 失敗時，`translate-docs` 可以自動將該片段分割成較小的部分（先從清單中點開始，然後是單個清單項目或更短的段落片段），從第一個模型開始逐一重試，並在原始片段快取金鑰下重新合併結果。此功能預設啟用（`segmentSplitting.qualityRetrySplit`）；設定為 `false` 可在模型全部耗盡後停止。當此備援機制執行時，執行摘要會報告 `Quality split retries`。
...
預設監聽埠為 **8675**。若該埠不可用，伺服器會嘗試下一個埠（最多 1000 次），並記錄所選用的埠。已棄用的別名 `editor` 仍可運作但會列印警告——請優先使用 `dashboard`。
...
「**Markdown 問題**」標籤頁列出 `markdown_source_issues` SQLite 資料表中的資料列。每一列都是一項 **翻譯前**的發現：例如在 `translate-docs` 用於遮罩的相同 CommonMark 風格規則下，永遠無法配對成強調或刪除線的分隔符序列、以反引號開啟但未關閉的內聯程式碼區塊，或是當 `STRONG_OUTSIDE_LINK` 與 `**` / `__` 包住 `[text](../../docs/url)` 連結時（僅在連結文字內使用粗體）。這 **不等同於**「**失敗**」，後者記錄的是每區域設定的模型輸出與翻譯後驗證問題（`AST mismatch`、佔位符洩漏等類似問題）。
...
與 `docsOutput` 相同層級（依 `docs[]` 區塊而定）。預設 `true`：翻譯 Starlight/Docusaurus 的使用者介面 YAML 敘述（`title`、`description`、`sidebar.label`、`sidebar_label`、`keywords`、`hero.title`、`hero.tagline`、`hero.image.alt`、`hero.actions[].text`、`pagination_label`、`prev`/`next` 標籤）。設定 `false` 可保持整個前置資料區塊不變；傳入字串陣列可限制至特定的點路徑。
- `segmentSplitting`
與 `docsOutput` 相同層級（依 `docs[]` 區塊而定）。用於 `translate-docs` 提取的選擇性更細粒度片段：`{ "enabled", "maxCharsPerSegment"?, "splitPipeTables"?, "splitDenseParagraphs"?, "maxLinesPerParagraphChunk"?, "splitLongLists"?, "maxListItemsPerChunk"?, "qualityRetrySplit"?, "maxQualityRetrySplitDepth"? }`。當 `enabled` 為 `true` 時（`segmentSplitting` 省略時的預設值），密集段落、GFM 管線表格（第一個片段包含標頭、分隔線與第一筆資料列）以及長清單會被分割；子片段以單一換行重新合併（`tightJoinPrevious`）。設定 `"enabled": false` 則僅對每個以空白行分隔的主體區塊使用一個片段。當 `qualityRetrySplit` 為 `true` 時（預設值），在所有模型耗盡後仍未能通過 AST 驗證的 Markdown 片段會逐步分割並從第一個模型重新嘗試；`maxQualityRetrySplitDepth`（預設 `3`）限制遞迴分割次數。
- `warnMarkdownSourceIssues`
當 `true` 時（省略時的預設值），每次 `translate-docs` 執行會重新掃描 Markdown 片段中的高風險分隔符或未關閉的內聯程式碼，列印終端警告，並替換該檔案快取路徑對應的 `markdown_source_issues` 資料列。設定 `false` 可跳過此區塊的警告與 SQLite 更新。
- `addFrontmatter`
當 `true` 時（省略時的預設值），翻譯後的 Markdown 檔案會包含 YAML 欄位：`translation_last_updated`、`source_file_mtime`、`source_file_hash`、`translation_language`、`source_file_path`，以及至少有一個片段含有模型中繼資料時的 `translation_models`（已排序的 OpenRouter 模型 ID 清單）。設定為 `false` 可跳過。
...
| `check-markdown …`                                                                                         | 掃描每個 `docs[]` 區塊的 `contentPaths` 下的 Markdown/MDX（與 `translate-docs` 相同的發現機制，遵循 `.translate-ignore`）：分隔符配對、未關閉的內聯程式碼，以及當 `**`/`__` 包住 `[text](../../docs/url)` 連結時的 `STRONG_OUTSIDE_LINK`。`-p` / `--path` 或 `-f` / `--file`：選擇性範圍。將 `relativePath:line: [ISSUE_CODE] message` 行列印至 **stderr**；若有任何問題則以 **1** 為結束代碼。`--json`：於 **stdout** 輸出 JSON 報告。除非指定 `--no-cache`，否則會在 `cacheDir` 中寫入 `markdown_source_issues`。`-v` 會在 stderr 訊息中加入來源雜湊。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `cleanup [--dry-run] [--no-backup] [--backup <path>]`                                                      | 先執行 `sync --force-update`（提取、UI、SVG、文件），然後移除過時的片段列（`last_hit_at` 為 null 或檔案路徑為空）；刪除解析後原始路徑在磁碟上不存在的 `file_tracking` 列；移除其 `filepath` 中繼資料指向遺失檔案的翻譯列；清除孤立的 `translation_failures` 列。記錄四項計數（過時片段、孤立的 `file_tracking`、孤立的翻譯、孤立的失敗）。除非指定 `--no-backup`，否則會在快取目錄下建立帶有時間戳記的 SQLite 備份檔。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `dashboard [-p <port>] [--no-open]`                                                                        | 啟動翻譯儀表板（用於快取片段、`strings.json`、詞彙表、失敗項目和統計資料的本地網頁 UI）。預設通訊埠為 **8675**（若不可用則嘗試下一個通訊埠）。若加上 `--no-open`，則不會自動開啟預設瀏覽器。已棄用的別名 `editor` 仍可運作，但會顯示警告訊息。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
