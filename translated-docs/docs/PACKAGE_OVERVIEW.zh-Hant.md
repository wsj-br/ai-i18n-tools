...
- **地區設定解析**（`resolveUiLocale`於`src/core/ui-locale.ts`）：從`-L` / `--ui-lang` > `AI_I18N_LANG` > 設定檔`uiLanguage` > 主機作業系統地區設定（`Intl.DateTimeFormat().resolvedOptions().locale`）中選取使用者介面地區設定。候選地區設定會經過標準化，並與內建的套件集進行精確比對或依最接近的變體進行比對（例如 `pt-PT` → `pt-BR`、`en-US` → `en-GB`），最後回退至來源地區設定（`en-GB`）。命令列介面會在建構說明之前（預先剖析 argv 掃描）解析一次，並在載入設定檔後再次解析，以便套用`uiLanguage`（旗標和環境變數仍然具有優先權）。
- **執行階段**（`src/i18n/index.ts`）：一個最小化的`t(source, vars)`，包含`{{name}}`插值，以英文來源字串為索引鍵，對應至`src/i18n/locales/<code>.json`中的平面地區設定套件（建構時複製到`dist/i18n/locales`）。遺失的鍵或套件會傳回來源文字。這與工作流程 1 的鍵即預設值模型相同 — 沒有雜湊查找。
- **儀表板**：伺服器會公開`GET /api/ui-i18n`，傳回已解析使用者介面地區設定的`{ locale, dir, bundle }`；前端會設定`<html lang>` / `dir`，並透過`data-i18n*`屬性本地化靜態標記。
- **內部測試**：套件會透過對`ai-i18n-self.config.json`（`pnpm i18n:self`）執行套件自身的提取 → `translate-ui`流程來產生套件。目錄鍵來自`src/cli/`和`src/i18n/`中的`t()`呼叫，以及`src/dashboard-app/index.html`中的儀表板`data-i18n*`標記。
