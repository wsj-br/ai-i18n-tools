...
|----------|---------|--------|
| 1 — UI strings | `t("…")` / `i18n.t("…")` in source | `strings.json` catalog → flat per-locale JSON |
...
Everything is driven by `ai-i18n-tools.config.json`. Runtime helpers export from `ai-i18n-tools/runtime`. Public programmatic API exports from `src/index.ts`.
...
Run `pnpm test` and `pnpm lint` after every behavioral change. The CLI entry is `bin/ai-i18n-tools.mjs` (compiled to `dist/cli/index.js`).
...
examples/             console-app, nextjs-app (use the locally-built CLI)
...
- `data/ui-languages-complete.json` — hand-maintained master list; update only intentionally via `scripts/build-ui-languages-complete.mjs`.
